...
async function upsertSGRTStatePreservingEvaluator(nit, incomingPayload) {
  ...
  try {
    ...
    Object.keys(serverAnswers).forEach((c) => {
      const dst = mergedAnswers[c] && typeof mergedAnswers[c] === 'object' ? mergedAnswers[c] : {};
...
function hydrateEvaluatorResponses(payload) {
  ...
  const legacyContract = String(payload._respuestasContrato || payload.contratoRespuestas || payload.contratoEval || '').trim();
...
async function patchEvaluatorStateByContract(nit, contrato, patch) {
  ...
  try {
    ...
    if (patch && patch.replaceResponses === true && patch.respuestas && typeof patch.respuestas === 'object') {
      ...
      const currentAnswers = payload.respuestasACPorContrato[c] && typeof payload.respuestasACPorContrato[c] === 'object' ? payload.respuestasACPorContrato[c] : {};
    ...
    if (patch && patch.promContrato && typeof patch.promContrato === 'object') {
      payload.promPorContrato[c] = Object.assign({}, payload.promPorContrato[c] || {}, sgrtPlainClone(patch.promContrato));
    }
...
function pbiReadSavedConfig(){
  try{
    ...
    if(!workspaceId||!reportId) return null;
    return {workspaceId,reportId,embedUrl:String(data.embedUrl||''),updatedAt:data.updatedAt||''};
  }catch(e){
...
function pbiSaveReportConfig(cfg){
  ...
  if(!workspaceId||!reportId) throw new Error('Workspace o Report ID inválido');
  const embedUrl=String(cfg.embedUrl||`https://app.powerbi.com/reportEmbed?reportId=${encodeURIComponent(reportId)}&groupId=${encodeURIComponent(workspaceId)}`);
  fs.mkdirSync(PBI_CONFIG_DIR,{recursive:true});
  const payload={workspaceId,reportId,embedUrl,updatedAt:new Date().toISOString()};
...
app.post('/api/powerbi/report-config', (req,res)=>{
  ...
  try{
    ...
    const parsed=pbiParseEmbedUrl(req.body&&req.body.embedUrl);
    ...
    if(!workspaceId||!reportId) return res.status(400).json({ok:false,error:'Enlace de Power BI inválido: faltan groupId/workspaceId o reportId'});
    const saved=pbiSaveReportConfig({workspaceId,reportId,embedUrl:(parsed&&parsed.embedUrl)||(req.body&&req.body.embedUrl)});
    res.json({ok:true,workspaceId:saved.workspaceId,reportId:saved.reportId,embedUrl:saved.embedUrl,source:'server'});
  }catch(e){res.status(500).json({ok:false,error:e.message});}
});
