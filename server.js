...
app.post('/api/database/selective-delete', async (req, res) => {
  try {
    ...
    if (action === 'state_by_nit') {
      const rq = new sql.Request(pool); rq.input('nit', sql.NVarChar(50), nit);
      const rs = await rq.query(`DELETE FROM dbo.SGRT_Tercero_Estado WHERE NIT=@nit; SELECT @@ROWCOUNT AS Deleted;`);
      return res.json({ok:true,deleted:Number((rs.recordset&&rs.recordset[0]&&rs.recordset[0].Deleted)||0),message:'Estado SGRT eliminado para '+nit});
    }
    if (action === 'third_full') {
      ...
      return res.json({ok:true,message:'Tercero y estado SGRT eliminados para '+nit});
    }
...
async function openaiRespond(prompt,context,files,history){
  ...
  if(totalBytes>18*1024*1024) throw new Error('Los archivos adjuntos juntos deben ser de hasta 18 MB');
  const requestBody={model:AI_CFG.openaiModel,input:[{role:'user',content}]};
...
function aiProviderOrder(prompt,files){
  ...
  // En modo auto: OpenAI primero para conversación cotidiana y Gemini primero
  // cuando una pregunta conceptual puede beneficiarse de Google Search.
  if(AI_CFG.geminiApiKey&&AI_CFG.openaiApiKey){
    return aiShouldGroundWeb(prompt,{},files)?['gemini','openai']:['openai','gemini'];
  }
  const out=[];
...
function aiImageProviderOrder(){
  // Respeta AI_PROVIDER si se fijó explícitamente. En modo auto se intenta primero
  // Gemini (si existe) y luego OpenAI, igual que en el chat.
  if(AI_CFG.provider==='gemini') return AI_CFG.geminiApiKey?['gemini']:[];
  if(AI_CFG.provider==='openai') return AI_CFG.openaiApiKey?['openai']:[];
  const out=[];
  if(AI_CFG.geminiApiKey) out.push('gemini');
