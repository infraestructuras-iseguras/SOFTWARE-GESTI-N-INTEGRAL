...
function aiInstruction(prompt,context,history){
  ...
  return [
    'Actua como asistente profesional del Sistema de Gestion de Riesgos de Terceros (SGRT).',
    'Redacta en espanol claro, tecnico, corporativo y practico. No inventes datos.',
    'Usa exclusivamente la informacion disponible en el contexto, registros o archivos suministrados.',
    'Prioriza hallazgos, dimensiones de control, riesgos, acciones, responsables y siguientes pasos.',
    'Cuando se solicite un objeto, acta, informe o presentacion, redacta contenido listo para usar en un documento empresarial.',
...
async function geminiRespond(prompt,context,file,history){
  ...
  if(file&&file.dataUrl){
    ...
    if(!parsed) throw new Error('El archivo adjunto no tiene un formato valido');
    const bytes=Buffer.from(parsed.base64,'base64').length;
    if(bytes>18*1024*1024) throw new Error('Para analisis directo con Gemini, el archivo debe ser de hasta 18 MB');
...
async function openaiRespond(prompt,context,file,history){
  ...
  if(file&&file.dataUrl){
    ...
    if(/^image\//i.test(mime)) content.push({type:'input_image',image_url:String(file.dataUrl)});
      ...
      const parsed=parseDataUrl(file.dataUrl);
      content.push({type:'input_file',filename:String(file.name||'archivo'),file_data:parsed?parsed.base64:String(file.dataUrl||'')});
    }
  }
  const data=await aiFetchJson('https://api.openai.com/v1/responses',{
    method:'POST',
    headers:{'Authorization':'Bearer '+AI_CFG.openaiApiKey,'Content-Type':'application/json'},
...
async function aiRespond(prompt,context,file,history){
  ...
  if(provider==='gemini') return geminiRespond(prompt,context,file,history);
  if(provider==='openai') return openaiRespond(prompt,context,file,history);
