...
// ================================================================
// ASISTENTE IA / DOCUMENTOS / TRANSCRIPCION DE REUNIONES (OPCIONAL)
// ================================================================
// No hay secretos en el codigo. Para habilitarlo configura OPENAI_API_KEY en
// Azure App Service. Si no existe, el frontend conserva su guia local.
const AI_CFG = {
  apiKey: String(process.env.OPENAI_API_KEY || '').trim(),
  model: String(process.env.OPENAI_MODEL || 'gpt-5').trim(),
  transcriptionModel: String(process.env.OPENAI_TRANSCRIBE_MODEL || 'gpt-4o-transcribe').trim()
};

function aiConfigured(){ return !!AI_CFG.apiKey; }

async function aiFetchJson(url, options){
  if(typeof fetch!=='function') throw new Error('El servidor requiere Node 18+ para el asistente IA');
...
function aiHistoryText(history){
  ...
  return history.slice(-10).map(item=>{
    const role=String(item&&item.role||'user').toLowerCase()==='assistant'?'IA SGRT':'Usuario';
    const content=String(item&&item.content||'').trim();
    return content ? role+': '+content.slice(0,5000) : '';
  }).filter(Boolean).join('\n');
}

async function aiRespond(prompt, context, file, history){
  if(!aiConfigured()) throw new Error('OPENAI_API_KEY no esta configurada en Azure');
  const previous=aiHistoryText(history);
  const content=[{
    type:'input_text',
    text:[
      'Eres un asistente profesional del Sistema de Gestion de Riesgos de Terceros (SGRT).',
      'Redacta en espanol claro, tecnico y practico. No inventes datos.',
      'Prioriza: hallazgos, dimensiones de control, riesgos, acciones, responsables y siguientes pasos.',
      'Cuando el usuario pida objeto, acta o informe, entrega texto listo para usar en un documento corporativo.',
      'Mantén continuidad con la conversación previa cuando exista y respeta correcciones del usuario.',
  ...
  if(file&&file.dataUrl){
    ...
    if(/^image\//i.test(mime)) content.push({type:'input_image',image_url:String(file.dataUrl)});
    else { const raw=String(file.dataUrl||''); const m=raw.match(/^data:[^;]+;base64,(.+)$/s); content.push({type:'input_file',filename:String(file.name||'archivo'),file_data:m?m[1]:raw}); }
  }
  const data=await aiFetchJson('https://api.openai.com/v1/responses',{
    method:'POST',
    headers:{'Authorization':'Bearer '+AI_CFG.apiKey,'Content-Type':'application/json'},
    body:JSON.stringify({model:AI_CFG.model,input:[{role:'user',content}]})
  });
  return aiOutputText(data)||'El modelo no devolvio texto.';
}

app.get('/api/ai/status',(req,res)=>res.json({ok:true,configured:aiConfigured(),model:aiConfigured()?AI_CFG.model:'',transcriptionModel:aiConfigured()?AI_CFG.transcriptionModel:''}));

app.post('/api/ai/assist',async(req,res)=>{
  try{
    if(!aiConfigured()) return res.status(503).json({ok:false,code:'AI_NOT_CONFIGURED',error:'Configura OPENAI_API_KEY en Azure App Service'});
    const body=req.body||{};
    const text=await aiRespond(body.prompt||'',body.context||{},body.file||null,body.history||[]);
    res.json({ok:true,text});
  }catch(e){console.error('AI assist:',e.message);res.status(e.status||500).json({ok:false,error:e.message});}
});

app.post('/api/ai/transcribe',async(req,res)=>{
  try{
    if(!aiConfigured()) return res.status(503).json({ok:false,code:'AI_NOT_CONFIGURED',error:'Configura OPENAI_API_KEY en Azure App Service'});
    const body=req.body||{}, dataUrl=String(body.dataUrl||''), m=dataUrl.match(/^data:([^;,]+)?(?:;[^,]*)?;base64,(.+)$/s);
    if(!m) return res.status(400).json({ok:false,error:'Archivo de audio/video invalido'});
    const buffer=Buffer.from(m[2],'base64');
    if(buffer.length>35*1024*1024) return res.status(413).json({ok:false,error:'Para transcripcion directa, el archivo debe ser de hasta 35 MB'});
    ...
    form.append('file',new Blob([buffer],{type:String(body.mimeType||m[1]||'application/octet-stream')}),String(body.fileName||'reunion.mp4'));
    form.append('model',AI_CFG.transcriptionModel);
    const tr=await aiFetchJson('https://api.openai.com/v1/audio/transcriptions',{
      method:'POST',headers:{'Authorization':'Bearer '+AI_CFG.apiKey},body:form
    });
    const transcript=String(tr.text||'').trim();
    const summary=await aiRespond([
      'Genera un informe/acta practico de esta reunion.',
  ...
  }catch(e){console.error('AI transcribe:',e.message);res.status(e.status||500).json({ok:false,error:e.message});}
});
