...
// ================================================================
// ARCHIVOS ACTIVOS — GESTIÓN EXCLUSIVA ISEGURAS
// ================================================================
// Permite a ISEGURAS localizar evidencias/documentos que todavía están activos
// y enviarlos explícitamente a la papelera. No afecta respuestas, riesgos u
// otros módulos del SGRT.
function sgFileDisplayName(item) {
  item = item || {};
  return String(item.name || item.nombre || item.fileName || item.filename || item.titulo || item.title || item.id || 'Archivo');
}
function sgCollectFilesFromPack(pack, out, meta, path) {
  out = out || []; path = path || [];
  if (Array.isArray(pack)) {
    pack.forEach((item, idx) => {
      if (item && typeof item === 'object' && item.id && (item.name || item.nombre || item.fileName || item.filename || item.url || item.dataUrl || item.mime || item.type)) {
        out.push({
          id: String(item.id), type: meta.type, nit: meta.nit, tercero: meta.tercero || '',
          contractNo: String(item.contrato || item.contractNo || item.contratoNo || ''),
          name: sgFileDisplayName(item), folder: path.join(' / ') || 'Sin carpeta',
          size: item.size == null ? null : item.size, mime: String(item.mime || item.type || ''),
          date: item.fecha || item.createdAt || item.updatedAt || item.savedAt || null
        });
      } else if (item && typeof item === 'object') {
        sgCollectFilesFromPack(item, out, meta, path.concat(String(idx)));
      }
    });
    return out;
  }
  if (pack && typeof pack === 'object') {
    Object.keys(pack).forEach((k) => sgCollectFilesFromPack(pack[k], out, meta, path.concat(k)));
  }
  return out;
}
function sgRemoveFileFromPack(pack, targetId, found, path) {
  path = path || [];
  if (Array.isArray(pack)) {
    for (let i = pack.length - 1; i >= 0; i--) {
      const item = pack[i];
      if (item && typeof item === 'object' && String(item.id || '') === String(targetId)) {
        if (!found.item) { found.item = sgrtPlainClone(item); found.groupKey = path.join(' / '); }
        pack.splice(i, 1);
      } else if (item && typeof item === 'object') {
        sgRemoveFileFromPack(item, targetId, found, path.concat(String(i)));
      }
    }
    return pack;
  }
  if (pack && typeof pack === 'object') {
    Object.keys(pack).forEach((k) => {
      ...
      if (v && typeof v === 'object') {
        sgRemoveFileFromPack(v, targetId, found, path.concat(k));
        if (Array.isArray(v) && v.length === 0) delete pack[k];
        else if (!Array.isArray(v) && Object.keys(v).length === 0) delete pack[k];
      }
    });
  }
  return pack;
...
app.get('/api/iseguras/files', async (req,res)=>{
  try {
    if(!sgIsISEGURASRequest(req)) return res.status(403).json({ok:false,error:'Solo ISEGURAS puede gestionar archivos'});
    if(!sgrtStateReady) await ensureSGRTStateTable();
    const q=String(req.query.q||'').trim().toLowerCase();
    const limit=Math.max(1,Math.min(1000,Number(req.query.limit||500)||500));
    const rq=new sql.Request(pool);
    const rs=await rq.query(`SELECT s.NIT,s.Payload,s.UpdatedAt,t.Nombre_Tercero FROM dbo.SGRT_Tercero_Estado s LEFT JOIN dbo.Terceros t ON t.NIT=s.NIT ORDER BY s.UpdatedAt DESC`);
    const files=[];
    for (const row of rs.recordset||[]) {
      let st={}; try{st=JSON.parse(row.Payload||'{}');}catch(e){st={};}
      const metaBase={nit:String(row.NIT||''),tercero:String(row.Nombre_Tercero||'')};
      sgCollectFilesFromPack(st._evidenciasAC,files,{...metaBase,type:'evidence_ac'},['Evidencias Ambiente de Control']);
      sgCollectFilesFromPack(st._evidenciasRiesgo,files,{...metaBase,type:'evidence_risk'},['Evidencias Análisis de Riesgos']);
      (Array.isArray(st._documentosGenerados)?st._documentosGenerados:[]).forEach((d)=>{
        if(!d||!d.id)return;
        files.push({id:String(d.id),type:'document',nit:metaBase.nit,tercero:metaBase.tercero,contractNo:String(d.contrato||d.contractNo||''),name:sgFileDisplayName(d),folder:'Documentos Generados por IA',size:d.size==null?null:d.size,mime:String(d.mime||d.type||''),date:d.fecha||d.createdAt||d.updatedAt||d.savedAt||null});
      });
    }
    let out=files;
    if(q) out=out.filter((x)=>[x.name,x.nit,x.tercero,x.contractNo,x.folder,x.type].some((v)=>String(v||'').toLowerCase().includes(q)));
    out=out.slice(0,limit);
    res.json({ok:true,count:out.length,data:out});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});

app.delete('/api/iseguras/files/:nit/:type/:id', async (req,res)=>{
  try {
    if(!sgIsISEGURASRequest(req)) return res.status(403).json({ok:false,error:'Solo ISEGURAS puede eliminar archivos'});
    const nit=String(req.params.nit||''), type=String(req.params.type||''), id=String(req.params.id||'');
    if(!nit||!id||!['evidence_ac','evidence_risk','document'].includes(type)) return res.status(400).json({ok:false,error:'Archivo no válido'});
    if(!sgrtStateReady) await ensureSGRTStateTable(); await ensureSGRTRecycleTable();
    const tx=new sql.Transaction(pool); await tx.begin(sql.ISOLATION_LEVEL.SERIALIZABLE);
    try {
      const rr=new sql.Request(tx); rr.input('nit',sql.NVarChar(50),nit);
      const rs=await rr.query(`SELECT Payload FROM dbo.SGRT_Tercero_Estado WITH (UPDLOCK,HOLDLOCK) WHERE NIT=@nit`);
      if(!rs.recordset.length) throw new Error('No existe estado SGRT para este tercero');
      let st={}; try{st=JSON.parse(rs.recordset[0].Payload||'{}');}catch(e){st={};}
      st=hydrateEvaluatorResponses(st||{});
      let found={item:null,groupKey:''};
      if(type==='document') {
        const arr=Array.isArray(st._documentosGenerados)?st._documentosGenerados:[];
        const idx=arr.findIndex((x)=>String((x&&x.id)||'')===id);
        if(idx>=0){found.item=sgrtPlainClone(arr[idx]);found.groupKey='Documentos Generados por IA';arr.splice(idx,1);} st._documentosGenerados=arr;
      } else {
        const key=type==='evidence_ac'?'_evidenciasAC':'_evidenciasRiesgo';
        const pack=st[key]&&typeof st[key]==='object'?st[key]:{};
        sgRemoveFileFromPack(pack,id,found,[]); st[key]=pack;
        if(found.item){const tomb=new Set((Array.isArray(st._deletedEvidenceIds)?st._deletedEvidenceIds:[]).map(String));tomb.add(id);st._deletedEvidenceIds=Array.from(tomb).slice(-500);}
      }
      if(!found.item) throw new Error('El archivo ya no existe o fue eliminado por otro usuario');
      const contract=String(found.item.contrato||found.item.contractNo||found.item.contratoNo||'');
      await sgRecycleInsert({entityType:type,nit,contractNo:contract,itemId:id,label:(type==='document'?'Documento · ':'Evidencia · ')+sgFileDisplayName(found.item),payload:{groupKey:found.groupKey,item:sgrtPlainClone(found.item)}},sgActorFromRequest(req),tx);
      const wr=new sql.Request(tx);wr.input('nit',sql.NVarChar(50),nit);wr.input('payload',sql.NVarChar(sql.MAX),JSON.stringify(st));
      await wr.query(`UPDATE dbo.SGRT_Tercero_Estado SET Payload=@payload,UpdatedAt=SYSUTCDATETIME() WHERE NIT=@nit`);
      await tx.commit();
      await sgLogActivity({...sgActorFromRequest(req),eventType:'delete_file',nit,contractNo:contract,message:'ISEGURAS envió a papelera · '+sgFileDisplayName(found.item),meta:{type,id}});
      res.json({ok:true,message:'Archivo enviado a la papelera. Puede restaurarse desde ISEGURAS.'});
    } catch(e) { try{await tx.rollback();}catch(_){} throw e; }
...
app.delete('/api/recycle-bin/:id', async (req,res)=>{
  try{
    if(!sgIsISEGURASRequest(req)) return res.status(403).json({ok:false,error:'Solo ISEGURAS puede eliminar definitivamente elementos'});
    if(!sgrtRecycleReady) await ensureSGRTRecycleTable();
    const id=Number(req.params.id); if(!id) return res.status(400).json({ok:false,error:'ID inválido'});
    const rq=new sql.Request(pool);rq.input('id',sql.BigInt,id);
    const before=await rq.query(`SELECT TOP 1 ID,EntityType,NIT,ContractNo,Label,Status FROM dbo.SGRT_RecycleBin WHERE ID=@id`);
    if(!before.recordset.length) return res.status(404).json({ok:false,error:'Elemento no encontrado'});
    const row=before.recordset[0];
    await rq.query(`DELETE FROM dbo.SGRT_RecycleBin WHERE ID=@id`);
    await sgLogActivity({...sgActorFromRequest(req),eventType:'purge_recycle',nit:String(row.NIT||''),contractNo:String(row.ContractNo||''),message:'ISEGURAS eliminó definitivamente de la papelera · '+String(row.Label||row.EntityType),meta:{recycleId:id,entityType:row.EntityType}});
    res.json({ok:true,message:'Elemento eliminado definitivamente. Esta acción no se puede deshacer.'});
  }catch(e){res.status(400).json({ok:false,error:e.message});}
});
