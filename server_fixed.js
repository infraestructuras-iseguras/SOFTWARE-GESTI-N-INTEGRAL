...
app.post('/api/clasificacion', async (req, res) => {
  try {
    ...
    try {
      ...
      // ⭐ LÓGICA ROBUSTA: Garantiza siempre un valor válido
      let idRelGen = null;
      try {
        // Intenta buscar por nombre si existe
        if (tercero.NombreEntidad) {
          const rg = await new sql.Request(tx).input('ENT',sql.NVarChar(255),'%'+tercero.NombreEntidad+'%').query('SELECT TOP 1 ID_General FROM dbo.RELACION_GENERAL WHERE NombreEntidad LIKE @ENT AND Activo=1 ORDER BY ID_General');
          if (rg.recordset?.length) idRelGen = rg.recordset[0].ID_General;
        }
        // Si no encontró, usa el primer cliente disponible
        if (!idRelGen) {
          ...
          idRelGen = rgF.recordset?.[0]?.ID_General || 1;
        }
        console.log('  idRelGen assigned:', idRelGen);
      } catch(e) { 
        console.warn('  RELACION_GENERAL fallback to 1:', e.message);
        idRelGen = 1;  // ⭐ Fallback absoluto: evita null/undefined y problemas de sync Azure
      }
