...
(function() {
  ...
  window.clsRender = function(){
    ...
    // ⭐ Sincronizar CLS_DB desde TERCEROS_DB primero. Eliminar antes cualquier
    // fila derivada cuyo NIT ya no exista en la fuente central para impedir
    // registros fantasma después de un borrado.
    try{
      var cur = new Date().getFullYear().toString();
      var db = typeof TERCEROS_DB !== 'undefined' ? TERCEROS_DB : {};
      var validNits = new Set(Object.keys(db||{}).map(function(k){return String(k);}));
      Object.keys(window.CLS_DB||{}).forEach(function(y){
        if(Array.isArray(window.CLS_DB[y])) window.CLS_DB[y] = window.CLS_DB[y].filter(function(r){return r&&validNits.has(String(r.nit||''));});
      });
      Object.values(db).forEach(function(t) {
        var yr = t.yr || cur;
        if (!window.CLS_DB[yr]) window.CLS_DB[yr] = [];
...
document.addEventListener('DOMContentLoaded', function(){
    if(!window.SGRT_DISABLE_AUTO_DEMO && window.precargaDatosSimulados) window.precargaDatosSimulados();
