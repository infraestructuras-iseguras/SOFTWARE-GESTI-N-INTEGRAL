...
(function(){
  ...
  function normalizeNitCandidate(v){
    if(v==null)return '';
    var raw=String(v).trim().replace(/\.0+$/,'');
    raw=raw.replace(/^(?:nit|n\.i\.t\.?|identificaci[oó]n(?:\s+tributaria)?|tax\s*id|documento)\s*[:#-]?\s*/i,'').trim();
    var digits=raw.replace(/\D/g,'');
    if(digits.length<7||digits.length>15)return '';
    // NIT colombiano típico: 9 dígitos base + dígito de verificación.
    if(digits.length===10)return digits.slice(0,9)+'-'+digits.slice(9);
    return digits;
  }
  function extractNitFromText(text){
    var t=String(text||'').replace(/\u00a0/g,' '),m;
    m=t.match(/(?:N\.?I\.?T\.?|NIT(?:\s+(?:DEL?|DE)\s+TERCERO)?|IDENTIFICACI[ÓO]N(?:\s+TRIBUTARIA)?|N[ÚU]MERO\s+DE\s+IDENTIFICACI[ÓO]N|DOCUMENTO\s+TRIBUTARIO|TAX\s*ID)\s*[:#-]?\s*([0-9][0-9.\-\s]{5,24})/i);
    if(m&&m[1]){var direct=normalizeNitCandidate(m[1]);if(direct)return direct;}
    // Formato frecuente en PDF: base de 9 dígitos y DV separado después de guion/espacio.
    m=t.match(/(\d{3}[.\s]?\d{3}[.\s]?\d{3})\s*[-–—]?\s*(\d)/);
    if(m){var joined=normalizeNitCandidate(m[1]+m[2]);if(joined)return joined;}
    var found=(t.match(/\b\d[\d.\-\s]{7,14}\d\b/g)||[]).map(normalizeNitCandidate).filter(Boolean);
    var uniq=[];found.forEach(function(x){if(uniq.indexOf(x)<0)uniq.push(x);});
    return uniq.length===1?uniq[0]:'';
  }
  function extractNitFromRow(row){
    row=row||{};
    var direct=val(row,['nit','nit_del_tercero','nit_tercero','numero_nit','n_i_t','identificacion_tributaria','numero_identificacion','identificacion','documento_tributario','tax_id']);
    var n=normalizeNitCandidate(direct);if(n)return n;
    var keys=Object.keys(row);
    for(var i=0;i<keys.length;i++){
      var k=key(keys[i]);
      if(/(^|_)(nit|n_i_t|identificacion_tributaria|tax_id|numero_identificacion)(_|$)/.test(k)){
        n=normalizeNitCandidate(row[keys[i]]);if(n)return n;
      }
    }
    return extractNitFromText(keys.map(function(k){return k+': '+String(row[k]||'');}).join('\n'));
  }
  function fileDataUrl(file){return new Promise(function(resolve,reject){var r=new FileReader();r.onload=function(){resolve(r.result);};r.onerror=reject;r.readAsDataURL(file);});}
  async function aiExtractRecord(file,reason){
    try{
      var dataUrl=await fileDataUrl(file);
      var prompt='IMPORTACIÓN SGRT. Lee el archivo completo y extrae únicamente la información real del tercero. '+(reason||'')+' Devuelve SOLO JSON válido, sin markdown, con estas claves: organizacion, nit, nombre_tercero, domicilio, numero_contrato, inicio_contrato, fin_contrato, objeto_contrato, estado_contrato, valor_contrato, procesos_contrato, observaciones_contrato, nombre_supervisor, cargo_supervisor, proceso_supervision, contrato_asociado. Si un dato no existe usa cadena vacía. No inventes ningún valor.';
      var rr=await fetch('/api/ai/assist',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:prompt,context:{modulo:'Importación de terceros',alcance:'SGRT'},files:[{name:file.name,type:file.type||'application/pdf',dataUrl:dataUrl}],history:[]})});
      var jj=await rr.json().catch(function(){return{};});if(!rr.ok||!jj.ok||!jj.text)return null;
      var txt=String(jj.text||'').trim().replace(/^```(?:json)?/i,'').replace(/```$/,'').trim(),m=txt.match(/\{[\s\S]*\}/);if(m)txt=m[0];
      var obj=JSON.parse(txt);return rawToRecord(obj,file.name+' · extracción IA');
    }catch(e){console.warn('[SGRT21 AI import]',e.message);return null;}
  }
  ...
  function parseExcelFile(file){
    return sheetJS().then(function(XLSX){return readBuffer(file,false).then(function(buf){
      var wb=XLSX.read(buf,{type:'array',cellDates:true}),out=[];
      (wb.SheetNames||[]).forEach(function(name){
        var sh=wb.Sheets[name],grid=XLSX.utils.sheet_to_json(sh,{header:1,defval:'',raw:false,blankrows:false});
        if(!grid.length)return;
        var best=0,bestScore=-1;
        for(var i=0;i<Math.min(25,grid.length);i++){
          var line=(grid[i]||[]).map(function(x){return key(x);}).join(' '),score=0;
          if(/\bnit\b|n_i_t|identificacion_tributaria/.test(line))score+=7;
          if(/nombre.*tercero|razon_social|proveedor|contratista/.test(line))score+=4;
          if(/contrato/.test(line))score+=2;if(/domicilio|direccion|ciudad/.test(line))score+=1;if(/supervisor/.test(line))score+=1;
          if(score>bestScore){bestScore=score;best=i;}
        }
        var headers=(grid[best]||[]).map(function(h,idx){return key(h)||('col_'+idx);});
        for(var r=best+1;r<grid.length;r++){
          var arr=grid[r]||[],row={},non=false;headers.forEach(function(h,j){var v=arr[j]==null?'':arr[j];row[h]=v;if(String(v).trim())non=true;});
          if(non)out.push(rawToRecord(row,file.name+' · '+name+' · fila '+(r+1)));
        }
      });
      return out;
    });});
  ...
  function parsePdfFile(file){
    return pdfJS().then(function(pdf){return readBuffer(file,false).then(function(buf){
      return pdf.getDocument({data:buf}).promise.then(function(doc){
        var pages=[];function next(i){if(i>doc.numPages)return Promise.resolve(pages.join('\n'));return doc.getPage(i).then(function(pg){return pg.getTextContent().then(function(tc){pages.push(tc.items.map(function(x){return x.str;}).join(' '));return next(i+1);});});}
        return next(1);
      }).then(async function(text){
        var rec=pdfTextRecord(text,file.name);
        if(rec.nit&&rec.nombre)return [rec];
        var ai=await aiExtractRecord(file, text&&text.length>80?'El texto extraído no permitió identificar con certeza NIT/nombre.':'El PDF parece escaneado o sin texto seleccionable.');
        return [ai||rec];
      });
    });}).catch(async function(err){var ai=await aiExtractRecord(file,'PDF no pudo leerse con PDF.js: '+(err.message||err));if(ai)return [ai];throw err;});
  }
  ...
  function pdfTextRecord(text,source){
    ...
    var nit=extractNitFromText(normalized)||normalizeNitCandidate(labeled(normalized,['nit','nit\s+del\s+tercero','identificaci[oó]n\s+tributaria']));
  ...
  function normalizeRecord(r){
    r=clone(r)||{}; r.nit=normalizeNitCandidate(r.nit)||String(r.nit||'').trim(); r.nombre=String(r.nombre||'').trim(); r.domicilio=String(r.domicilio||'').trim();
  ...
  function validate(r,seen){
    ...
    else if((r.nit.replace(/\D/g,'').length<7)||(r.nit.replace(/\D/g,'').length>15)) errors.push('NIT con formato no reconocido');
    ...
    var nk=String(r.nit||'').replace(/\D/g,'');
  ...
  function mergeByContract(a,b){
    var out=[],seen={};[].concat(a||[],b||[]).forEach(function(c){var n=String(c&&c.num||c&&c.numero||c&&c.contrato||'').trim(),k=n.toLowerCase();if(!k)k='__'+out.length;if(seen[k]){Object.assign(seen[k],c||{});}else{var x=clone(c||{});seen[k]=x;out.push(x);}});return out;
  }
  function mergeSupervisors(a,b){var out=[],seen={};[].concat(a||[],b||[]).forEach(function(x){var k=norm((x&&x.nombre)||'')+'|'+norm((x&&x.contrato_asociado)||'');if(!k||k==='|')return;if(seen[k])Object.assign(seen[k],x||{});else{var y=clone(x||{});seen[k]=y;out.push(y);}});return out;}
  async function fetchJsonSafe(url,opts){var r=await fetch(url,opts||{}),j=await r.json().catch(function(){return{};});if(!r.ok||j.ok===false)throw new Error(j.error||('HTTP '+r.status));return j;}
  async function syncImportedThirdAzure(t){
    await fetchJsonSafe('/api/terceros',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tercero:t})});
    var current={};try{var got=await fetchJsonSafe('/api/sgrt-state/'+encodeURIComponent(t.nit));current=(got.data&&got.data.estado_sgrt)||{};}catch(e){if(!/404/.test(String(e.message)))console.warn('[SGRT21 estado previo]',e.message);}
    var merged=Object.assign({},current,{nit:t.nit,nombre:t.nombre,nombre_tercero:t.nombre,entidad:t.entidad||'colpensiones',entidadLabel:t.entidadLabel||entityLabel(),domicilio:t.domicilio||current.domicilio||'',servicio:t.servicio||current.servicio||'',contratos:mergeByContract(current.contratos,t.contratos),supervisores:mergeSupervisors(current.supervisores,t.supervisores),origenImportacion:t.origenArchivo||'',ultimaImportacion:new Date().toISOString()});
    await fetchJsonSafe('/api/sgrt-state/'+encodeURIComponent(t.nit),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({estado_sgrt:merged})});
    return true;
  }
  window.sgrtConfirmarImportacion=async function(){
    if(!isAdmin()||state.processing)return;
    var db=window.TERCEROS_DB||localDB(),resp=window.CUEST_RESPUESTAS||localResponses(), valid=(state.records||[]).filter(function(r){return r.valid;}),added=0,updated=0,synced=0,failed=0;
    if(!valid.length){toast('No hay registros válidos para importar','warning');return;}
    var b=document.getElementById('sgrt-bulk-confirm');if(b){b.disabled=true;b.style.opacity='.45';b.textContent='Sincronizando…';}
    setStatus('Guardando en Azure SQL. Esta importación quedará visible desde otros equipos/enlaces del sistema…','warning');
    for(var i=0;i<valid.length;i++){
      var r=valid[i],digits=String(r.nit||'').replace(/\D/g,''),existingKey=Object.keys(db).find(function(k){return String(k).replace(/\D/g,'')===digits;}),old=existingKey?db[existingKey]:null;
      ...
      r.supervisores.forEach(function(s){var c=cons.find(function(x){return x.num&&String(x.num)===String(s.contrato_asociado);});if(c){c.supervisor=s.nombre;c.supervisorCargo=s.cargo;c.procesoSupervision=s.proceso;c.supervisor_asociado=s.nombre;}});
      var t=Object.assign({},old||{}, {nit:r.nit,nombre:r.nombre,nombre_tercero:r.nombre,entidad:'colpensiones',entidadLabel:entityLabel(),domicilio:r.domicilio||(old&&old.domicilio)||'',servicio:(cons[0]?(cons[0].objeto||cons[0].procesos||''):'')||(old&&old.servicio)||'',supervisor:r.supervisores[0]?r.supervisores[0].nombre:((old&&old.supervisor)||''),supervisores:mergeSupervisors(old&&old.supervisores,r.supervisores),contratos:mergeByContract(old&&old.contratos,cons),estado:(old&&old.estado)||'Activo',fecha_creacion:(old&&old.fecha_creacion)||new Date().toISOString(),origen:'importacion_asistida_azure',origenArchivo:r.source,demo:false,localOnly:false,sincronizado:false});
      if(existingKey&&existingKey!==r.nit)delete db[existingKey];db[r.nit]=t;if(old)updated++;else added++;
      try{await syncImportedThirdAzure(t);t.sincronizado=true;synced++;}catch(e){failed++;t.sincronizado=false;t._errorSincronizacion=e.message;console.warn('[SGRT21 import Azure]',r.nit,e.message);}
    }
    window.TERCEROS_DB=db;window.CUEST_RESPUESTAS=resp;window.MATRIZ_DB=window.MATRIZ_DB||localMatrix();persistLocalOnly();
    // Recargar el estado compartido antes de dibujar la tabla: así el registro
    // confirmado aparece de inmediato y queda igual en otro navegador/enlace.
    try{if(typeof window.sgrtCargarDesdeServidor==='function')await window.sgrtCargarDesdeServidor({forceServer:true,silentUi:false});}catch(_refreshErr){console.warn('[SGRT21 refresh Azure]',_refreshErr.message);}
    refreshViews();setTimeout(refreshViews,180);
    var msg='Importación terminada: '+added+' nuevo(s), '+updated+' actualizado(s), '+synced+' sincronizado(s) en Azure'+(failed?' · '+failed+' pendiente(s) por error':'')+'.';
    setStatus(msg,failed?'warning':'success');toast(msg,failed?'warning':'success');
    try{if(typeof window.sendNotification==='function')window.sendNotification('importacion','Importación de terceros completada',msg,{'Nuevos':added,'Actualizados':updated,'Azure':synced,'Errores':failed});}catch(e){}
    state.records=[];renderPreview();if(b){b.textContent='Confirmar importación válida';}
  };
  ...
  function adminHtml(s){var rows=exportRows(s),head=Object.keys(rows[0]||{Entidad:'',NIT:'',Tercero:''});return '<!doctype html><html lang="es"><head><meta charset="utf-8"><title>SGRT · Power BI · Organización actual</title><style>body{font-family:Arial,Calibri,sans-serif;margin:28px;color:#243447;background:#f7fafc}h1{color:#1a3a5c}h2{color:#1e6bb8;border-bottom:2px solid #bfdbfe;padding-bottom:6px}table{width:100%;border-collapse:collapse;background:white;font-size:11px;margin:10px 0 22px}th{background:#1a3a5c;color:white;padding:8px;text-align:left}td{border:1px solid #dbe3ea;padding:7px}p{color:#64748b;font-size:12px}</style></head><body><h1>SGRT · Tablas para Power BI · Organización actual</h1><p>Alcance del Administrador de Riesgos para la organización actual. Generado localmente el '+new Date().toLocaleString('es-CO')+'.</p><h2>Indicadores</h2><p><b>Terceros:</b> '+s.total+' · <b>Riesgos:</b> '+s.riskRows.length+' · <b>Documentos:</b> '+s.documentos.total+' · <b>AC:</b> '+s.acTotal+'</p><h2>Detalle</h2><table><thead><tr>'+head.map(function(k){return'<th>'+esc(k)+'</th>';}).join('')+'</tr></thead><tbody>'+rows.map(function(r){return'<tr>'+head.map(function(k){return'<td>'+esc(r[k])+'</td>';}).join('')+'</tr>';}).join('')+'</tbody></table></body></html>';}
  function renderPowerBI(){
    ...
    w.insertAdjacentHTML('afterbegin','<div id="admin-powerbi-graficas" class="card" style="margin-bottom:16px;border-left:4px solid #1e6bb8;"><div class="card-hdr" style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;"><div><h3 style="margin:0;color:#1a3a5c;">📊 Tablero Power BI · Organización actual</h3><div class="sub">Datos consolidados del Evaluador y del Administrador de Riesgos para la organización actual.</div></div><div style="display:flex;gap:6px;flex-wrap:wrap;"><button onclick="window.descargarDatosPowerBIAdmin(\'csv\')" class="btn btn-outline btn-sm">⬇ CSV para Power BI</button><button onclick="window.descargarDatosPowerBIAdmin(\'json\')" class="btn btn-outline btn-sm">⬇ JSON</button><button onclick="window.descargarDatosPowerBIAdmin(\'html\')" class="btn btn-outline btn-sm">⬇ Tablas HTML</button></div></div><div style="padding:14px;display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px;"><div style="border:1px solid #e2e8f0;border-radius:8px;padding:12px;min-height:290px;"><div style="font-size:12px;font-weight:700;color:#1a3a5c;margin-bottom:8px;">Gráfica de torta · exposición</div><div style="height:245px;position:relative;"><canvas id="admin-chart-exposicion"></canvas></div></div><div style="border:1px solid #e2e8f0;border-radius:8px;padding:12px;min-height:290px;"><div style="font-size:12px;font-weight:700;color:#1a3a5c;margin-bottom:8px;">Terceros y riesgos · organización actual</div><div style="height:245px;position:relative;"><canvas id="admin-chart-entidades"></canvas></div></div></div><div style="font-size:11px;color:#64748b;padding:0 14px 13px;">La torta usa la calificación de matriz cuando existe y la clasificación del tercero cuando no existe matriz. La exportación respeta el alcance de la organización actual.</div></div>');
  ...
  function hook(){
    ...
    wrapOnce('doLogin',function(old){return function(){var r=old.apply(this,arguments);setTimeout(function(){applyScope();refreshViews();},260);return r;};},'_sgrt21Login');
  ...
  function boot(){ panel();applyScope();hook(); }
