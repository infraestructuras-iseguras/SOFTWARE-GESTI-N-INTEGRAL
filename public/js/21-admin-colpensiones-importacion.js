...
(function(){
  ...
  function extractNitFromText(text){
    ...
    m=t.match(/(?:N\.?I\.?T\.?|NIT(?:\s+DEL?\s+TERCERO)?|IDENTIFICACI[ÓO]N(?:\s+TRIBUTARIA)?|N[ÚU]MERO\s+DE\s+IDENTIFICACI[ÓO]N)\s*[:#-]?\s*([0-9][0-9.\-\s]{6,20})/i);
    if(m&&m[1])return normalizeNitCandidate(m[1]);
