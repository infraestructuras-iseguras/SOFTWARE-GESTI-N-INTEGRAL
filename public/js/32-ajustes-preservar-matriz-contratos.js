...
(function(){
  ...
  if(typeof oldServer==='function')window.sgrtCargarDesdeServidor=async function(opts){var silent=!!(opts&&opts.silentUi);if(!silent)captureRegistryOpen();var r=await oldServer.apply(this,arguments);if(!silent){setTimeout(applyRegistry,0);setTimeout(applyRegistry,120);}return r;};
  ...
  function calcVal(r){try{if(typeof window._calcCtrlValoracion==='function')return window._calcCtrlValoracion(r);}catch(e){}var yes=function(v){return norm(v)==='Si';};var sum=(yes(r.a1)?0.15:(norm(r.a1)==='Parcial'?0.05:0))+(yes(r.a2)?0.15:0)+(yes(r.a3)?0.15:0)+(yes(r.a4)?0.15:0)+(yes(r.a5)?0.20:0)+(yes(r.a6)?0.20:0);var pct=Math.round(sum*100);var m=pct>=91?['OPTIMIZADO',5,'#15803D','#DCFCE7']:pct>=71?['ADMINISTRADO',4,'#16A34A','#F0FDF4']:pct>=41?['DEFINIDO',3,'#CA8A04','#FEFCE8']:pct>=21?['REPETIBLE',2,'#EA580C','#FFF7ED']:pct>0?['INICIAL',1,'#DC2626','#FEF2F2']:['NO EXISTE',0,'#6B7280','#F3F4F6'];if(norm(r.a1)==='No Aplica')return{pct:0,nivelCumpl:'NO APLICA',madurez:'NO APLICA',valorMad:null,color:'#6B7280',bgColor:'#F3F4F6'};return{pct:pct,nivelCumpl:pct+'%',madurez:m[0],valorMad:m[1],color:m[2],bgColor:m[3]};}
