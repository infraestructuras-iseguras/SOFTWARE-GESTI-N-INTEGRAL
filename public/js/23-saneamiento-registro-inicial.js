/*
 * SGRT — Módulo 23: saneamiento inicial y eliminación específica de IBM.
 *
...
(function(){
  ...
  // Limpieza inicial única de la entrega V8 solicitada para pruebas desde cero.
  var LOCAL_MIGRATION='sgrt_cero_total_v8';
  var REMOTE_MIGRATION='sgrt_cero_total_remoto_v8';
  var PENDING_DELETE='sgrt_v8_nits_pendientes_borrado';
  var IBM_MIGRATION='sgrt_ibm_eliminado_v11';
  ...
  function isIBM(nit,record){
    var name=String((record&&(record.nombre||record.Nombre_Tercero||record.NombreTercero))||'').toUpperCase();
    return rawNit(nit)===IBM_NIT||name.indexOf('IBM')>=0;
  }
  function addRemoteExclusions(ids){
    var seen={},result=[];
    (window.SGRT_EXCLUDED_REMOTE_NITS||[]).concat(ids||[]).forEach(function(id){
      id=rawNit(id);if(id&&!seen[id]){seen[id]=true;result.push(id);}
    });
    window.SGRT_EXCLUDED_REMOTE_NITS=result;
  }
  function removeLocalIBM(ids){
    var remove={};(ids||[]).concat([IBM_NIT]).forEach(function(id){id=rawNit(id);if(id)remove[id]=true;});
    function cleanMap(map){
      if(!map||typeof map!=='object'||Array.isArray(map))return map||{};
      Object.keys(map).forEach(function(key){if(remove[rawNit(key)]||isIBM(key,map[key]))delete map[key];});
      return map;
    }
    window.TERCEROS_DB=cleanMap(window.TERCEROS_DB||{});
    ['sgrt_terceros_db_shared','sgrt_terceros_db','sgrt_terceros','sgrt_terceros_pending',
      ...
      .forEach(function(key){writeJSON(key,cleanMap(readJSON(key,{})));});
    var snapshot=readJSON('sgrt_v8',{});
    ['TERCEROS_DB','CUEST_RESPUESTAS','RESULTADO_EVALUACION','CLS_DB'].forEach(function(key){snapshot[key]=cleanMap(snapshot[key]||{});});
    writeJSON('sgrt_v8',snapshot);
    try{if(typeof TERCEROS_DB!=='undefined')TERCEROS_DB=window.TERCEROS_DB;}catch(e){}
  }
  ...
  async function deleteIBMOnce(){
    if(localStorage.getItem(IBM_MIGRATION)==='1')return true;
    var ids=[IBM_NIT],listed=false;
    Object.keys(window.TERCEROS_DB||{}).forEach(function(nit){if(isIBM(nit,window.TERCEROS_DB[nit]))ids.push(nit);});
    try{
      var response=await fetch(apiBase()+'/api/terceros',{headers:{'Accept':'application/json','Cache-Control':'no-cache'}});
      var data=await response.json();
      if(!response.ok||!data.ok)throw new Error(data.error||('HTTP '+response.status));
      listed=true;
      (Array.isArray(data.data)?data.data:[]).forEach(function(row){if(isIBM(rawNit(row),row))ids.push(rawNit(row));});
    }catch(e){console.warn('No fue posible verificar IBM en SQL; se mantendrá excluido y se reintentará:',e.message);}
    var seen={};ids=ids.map(rawNit).filter(function(id){if(!id||seen[id])return false;seen[id]=true;return true;});
    addRemoteExclusions(ids);removeLocalIBM(ids);
    if(typeof window._remoteDeleteTercero!=='function')return false;
    var failed=[];
    for(var i=0;i<ids.length;i++){
      try{await window._remoteDeleteTercero(ids[i],{allowNotFound:true});}catch(e2){failed.push(ids[i]);}
    }
    if(listed&&!failed.length){localStorage.setItem(IBM_MIGRATION,'1');return true;}
    return false;
  }
