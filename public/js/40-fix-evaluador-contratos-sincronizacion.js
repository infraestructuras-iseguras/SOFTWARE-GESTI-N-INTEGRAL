...
(function(){
  ...
  function mergeRemoteState(nit,remote,rerender){
    nit=norm(nit);if(!remote||typeof remote!=='object')return false;remote=clone(remote);remote.nit=remote.nit||nit;if(!legacyOwner[nit])legacyOwner[nit]=norm(remote._respuestasContrato||remote.contratoRespuestas||(remote._ultimaEdicionEvaluador&&remote._ultimaEdicionEvaluador.contrato)||'');rememberCanonicalDims(remote,true);migrateLegacyResponses(remote);var t=db()[nit]||{};
    // Los campos maestros no deben borrarse por un payload extendido antiguo o parcial.
    ['nombre','entidad','domicilio','servicio','servicio_contratado'].forEach(function(k){
      if(remote[k]!==undefined&&norm(remote[k]))t[k]=clone(remote[k]);
    });
    ['contratos','dimsPorContrato','tipologiasPorContrato','aprobadoPorContrato','_persHiddenControls','_tipologiasDBCustom','_configPreguntas','promPorContrato'].forEach(function(k){if(remote[k]!==undefined)t[k]=clone(remote[k]);});
    migrateLegacyResponses(t);
  ...
  async function lightweightServerLoad(opts){
    ...
    try{
      ...
      var changed=[];
      (states.data||[]).forEach(function(row){var nit=norm(row.nit||row.NIT),remote=row.estado_sgrt&&typeof row.estado_sgrt==='object'?row.estado_sgrt:{};if(!nit)return;remote=clone(remote);remote.nit=remote.nit||nit;var before='';try{before=JSON.stringify({d:(db()[nit]||{}).dimsPorContrato||{},r:(db()[nit]||{}).respuestasACPorContrato||{},c:(db()[nit]||{}).contratos||[]});}catch(e){}mergeRemoteState(nit,remote,false);var t=db()[nit]||{};var m=masterBy[nit]||{};
        // El maestro SQL es la fuente principal de los datos básicos cuando trae valor.
        t.nombre=norm(m.nombre||m.Nombre_Tercero)||t.nombre||nit;
        t.domicilio=norm(m.domicilio||m.Domicilio)||t.domicilio||'';
        t.servicio_contratado=norm(m.servicio_contratado||m.Servicio_Contratado)||t.servicio_contratado||'';
        if(!t.servicio&&t.servicio_contratado)t.servicio=t.servicio_contratado;
        rememberCanonicalDims(t);var after='';try{after=JSON.stringify({d:t.dimsPorContrato||{},r:t.respuestasACPorContrato||{},c:t.contratos||[]});}catch(e2){}if(before!==after)changed.push(nit);});
