...
(function(){
  ...
  function renderEvaluatorQuestionnaire(nit,contract){
    var t=db()[norm(nit)];if(!t||typeof original.cargarCuestionarioTercero!=='function')return;
    // Estos filtros pertenecen a fases antiguas y podían ocultar una tipología que
    // sí quedó asignada al contrato. Se suspenden solo durante este render y luego
    // se restauran sin alterar la información original del tercero.
  ...
  window.acVerTodoCuestionario=function(){
    ...
    setTimeout(function(){enhanceQuestionnaireTipologies(nit,contract,'');renderCopyBox();removeEvaluatorIcons();},120);
  };
  ...
  function switchEvaluatorContract(contract,source){
    ...
    if(source==='ac'){
      ...
      setTimeout(function(){enhanceQuestionnaireTipologies(nit,contract,'');renderCopyBox();removeEvaluatorIcons();},60);pullRemoteThird(nit,true);
    }
  }
  ...
  if(typeof original.cargarCuestionarioTercero==='function'){
    window.cargarCuestionarioTercero=function(){
      ...
      var r=renderEvaluatorQuestionnaire(nit,c);setTimeout(function(){enhanceQuestionnaireTipologies(nit,c,'');renderCopyBox();removeEvaluatorIcons();},60);return r;
    };
  }
  ...
  function openTipologyInQuestionnaire(key,name){
    ...
    setTimeout(function(){enhanceQuestionnaireTipologies(nit,contract,key);renderCopyBox();removeEvaluatorIcons();},220);
  }
  ...
  function mergeRemoteState(nit,remote,rerender){
    ...
    if(rerender&&!blocked&&Date.now()-lastLocalEditAt>700){if(panelIsInstructions()){populateContractSelect(nit,'ac',c);populateStrictTipologies();renderStrictProgress(nit);}else{populateContractSelect(nit,'q',c);try{window.cargarCuestionarioTercero&&window.cargarCuestionarioTercero();}catch(e2){}setTimeout(function(){enhanceQuestionnaireTipologies(nit,c,'');},70);}renderCopyBox();removeEvaluatorIcons();}
    return true;
  }
  ...
  if(typeof original.serverLoad==='function')window.sgrtCargarDesdeServidor=async function(opts){
    var silent=!!(opts&&opts.silentUi),r=await original.serverLoad.apply(this,arguments);if(isEvaluator()){Object.keys(db()).forEach(function(n){migrateLegacyResponses(db()[n]);});ensureAllRegistryRows();var nit=currentNit(panelIsInstructions()?'ac':'q'),c=currentContract(panelIsInstructions()?'ac':'q',nit);if(nit&&c&&activeResponseCtx[nit]===c)activateExact(nit,c);if(!silent){try{window.clsRender&&window.clsRender();}catch(e){}if(panelIsInstructions()&&nit){populateContractSelect(nit,'ac',c);populateStrictTipologies();renderStrictProgress(nit);}else if(nit&&c){setTimeout(function(){enhanceQuestionnaireTipologies(nit,c,'');},50);}}}return r;
  };
