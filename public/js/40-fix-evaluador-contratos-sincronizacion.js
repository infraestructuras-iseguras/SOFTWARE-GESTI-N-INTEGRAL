...
(function(){
  ...
  function strictDims(t,contract){
    ...
    if(nit&&canonicalDims[nit]&&Object.prototype.hasOwnProperty.call(canonicalDims[nit],contract))arr=canonicalDims[nit][contract];
    if(!Array.isArray(arr)&&t.dimsPorContrato&&Object.prototype.hasOwnProperty.call(t.dimsPorContrato,contract))arr=t.dimsPorContrato[contract];
  ...
  function migrateLegacyResponses(t){
    ...
    if(nit&&!legacyOwner[nit]){
      legacyOwner[nit]=norm(t._respuestasContrato||t.contratoRespuestas||(t._ultimaEdicionEvaluador&&t._ultimaEdicionEvaluador.contrato)||'');
      if(!legacyOwner[nit]&&!Object.keys(t.respuestasACPorContrato||{}).length)legacyOwner[nit]=norm(t.contratoEval);
    }
    if(nit&&legacyOwner[nit]&&t._respuestas)absorb(legacyOwner[nit],t._respuestas);
    return changed;
  }
  ...
  function renderEvaluatorQuestionnaire(nit,contract){
    ...
    // El renderer antiguo necesita t.dims, pero solo durante esta llamada. Se restaura
    // inmediatamente para que ningún módulo heredado lo guarde dentro de otro contrato.
    t.dims=clone(strictDims(t,contract));t._infDimsFilter=[];t._infDimsNA=[];
  ...
  window.acCambiarTerceroInstruc=function(){
    ...
    rememberCanonicalDims(db()[nit]);var c=populateContractSelect(nit,'ac');if(c){populateContractSelect(nit,'q',c);setContext(nit,c);populateStrictTipologies();renderStrictProgress(nit);pullRemoteThird(nit,true);}removeEvaluatorIcons();
  };
  ...
  function copyResponses(sourceContract,targetContract){
    var nit=currentNit('q')||currentNit('ac'),t=db()[nit];if(!t)return 0;var src=exactResponses(t,sourceContract),dims=strictDims(t,targetContract),out=exactResponses(t,targetContract),copied=0;
    dims.forEach(function(d){var key=dimKey(d);if(!key||!src[key])return;var allowed=activeControlNumbers(nit,key,targetContract);if(!out[key])out[key]={};allowed.forEach(function(n){if(src[key]&&src[key][n]){out[key][n]=clone(src[key][n]);copied++;}});});
  ...
  function sourceContracts(nit,target){var t=db()[nit];if(!t)return[];return evaluatorContracts(t).map(contractNum).filter(function(c){return c!==target;});
  }
  function renderCopyBox(){
    var old=document.getElementById('sgrt-eval-copy-box');if(old)old.remove();if(!isEvaluator())return;
    var nit=currentNit('q')||currentNit('ac'),target=currentContract(panelIsInstructions()?'ac':'q',nit);if(!nit||!target)return;
    ...
    // El check pertenece al cuestionario; en Instrucciones se mostrará al entrar a responder.
    ...
    box.innerHTML='<label style="display:flex;align-items:center;gap:8px;font-weight:700;cursor:pointer;"><input id="sgrt-eval-copy-check" type="checkbox"> Usar respuestas ya diligenciadas de otro contrato</label><div id="sgrt-eval-copy-options" style="display:none;margin-top:9px;gap:8px;align-items:center;flex-wrap:wrap;"><select id="sgrt-eval-copy-source" style="min-width:220px;padding:7px 9px;border:1px solid #cbd5e1;border-radius:6px;background:white;">'+sources.map(function(c){var n=answerWeight(exactResponses(db()[nit],c));return '<option value="'+esc(c)+'">Contrato '+esc(c)+(n?' · '+n+' respuesta(s)':' · sin respuestas guardadas')+'</option>';}).join('')+'</select><button type="button" id="sgrt-eval-copy-apply" style="padding:7px 12px;border:1px solid #1e6bb8;background:#1e6bb8;color:white;border-radius:6px;font-weight:700;cursor:pointer;">Rellenar respuestas</button><span style="color:#64748b;">Solo copia preguntas coincidentes. No borra preguntas distintas del contrato actual.</span></div>';
    ...
    btn.onclick=async function(){
      ...
      var srcWeight=answerWeight(exactResponses(db()[nit],source));if(!srcWeight){try{window.showToast&&window.showToast('El contrato '+source+' todavía no tiene respuestas guardadas para copiar.','warning',3000);}catch(e){}return;}
      var existing=answerWeight(exactResponses(db()[nit],target)),msg='¿Rellenar el contrato '+target+' con las respuestas del contrato '+source+'?';if(existing)msg+=' Se conservarán las respuestas no coincidentes y se actualizarán únicamente las preguntas coincidentes.';if(!window.confirm(msg))return;
      var n=copyResponses(source,target);lastLocalEditAt=Date.now();await syncEvaluatorContract(nit,target,{replaceResponses:true,respuestas:clone((db()[nit].respuestasACPorContrato||{})[target]||{})});
      try{applyRemoteAnswersToVisible(nit,target);renderStrictProgress(nit);window.showToast&&window.showToast(n?'Se copiaron '+n+' respuesta(s) al contrato '+target+'.':'No había preguntas coincidentes para copiar.',n?'success':'warning',3200);}catch(e2){}
    };
  }
  ...
  function applyRemoteAnswersToVisible(nit,contract){
    nit=norm(nit);contract=norm(contract);if(!nit||!contract)return;var rr=exactResponses(db()[nit]||{},contract);responses()[nit]=clone(rr);var nk=safeNit(nit);
  ...
  function mergeRemoteState(nit,remote,rerender){
    nit=norm(nit);if(!remote||typeof remote!=='object')return false;remote=clone(remote);remote.nit=remote.nit||nit;if(!legacyOwner[nit])legacyOwner[nit]=norm(remote._respuestasContrato||remote.contratoRespuestas||(remote._ultimaEdicionEvaluador&&remote._ultimaEdicionEvaluador.contrato)||((!Object.keys(remote.respuestasACPorContrato||{}).length)?remote.contratoEval:''));rememberCanonicalDims(remote,true);migrateLegacyResponses(remote);var t=db()[nit]||{};
  ...
  async function lightweightServerLoad(opts){
    ...
    }catch(e){console.warn('[SGRT40v4] Refresco silencioso:',e.message);return {ok:false,error:e.message};}
