...
(function(){
  ...
  window.bdLimpiarBD=async function(){
    if(!requireISRemote())return;
    if(!confirm('¿Limpiar completamente los registros locales y eliminarlos también del servidor? Esta acción no se puede deshacer.'))return;
    var word=prompt('Escribe LIMPIAR para confirmar:');if(word!=='LIMPIAR'){toastRemote('Operación cancelada','warning');return;}

    // Unir los NIT locales con los NIT que realmente existen en dbo.Terceros.
    // Antes solo se recorrían los locales, por lo que un registro existente en la BD
    // pero ausente de este navegador podía quedarse vivo y reaparecer al iniciar sesión.
    var idsMap={};Object.keys(localDbRemote()).forEach(function(n){if(n)idsMap[String(n).trim()]=true;});
    try{
      var base=String(window.API_BASE_URL||window.API_BASE||'http://localhost:3000').replace(/\/$/,'');
      var rr=await fetch(base+'/api/terceros',{headers:{'Accept':'application/json'},cache:'no-store'});
      var xx=await rr.json().catch(function(){return{};});
      if(rr.ok&&xx.ok&&Array.isArray(xx.data))xx.data.forEach(function(r){var n=String(r.nit||r.NIT||'').trim();if(n)idsMap[n]=true;});
    }catch(e){console.warn('No se pudo enumerar la BD antes de limpiar:',e.message);}

    var ids=Object.keys(idsMap),ok=0,fail=0;
