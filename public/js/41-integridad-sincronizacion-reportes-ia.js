...
(function(){
  ...
  function riskApiBase(){return String(window.API_BASE_URL||window.API_BASE||'').replace(/\/$/,'');}
  function syncRiskForName(name,deleteIds){
    var t=Object.values(db()).find(function(x){return x&&(norm(x.nombre)===norm(name)||norm(x.nit)===norm(name));});
    if(!t)return;
    t._matrizRiesgos=risksForThird(t);t._changed=true;t.sincronizado=false;t.savedAt=new Date().toISOString();saveLocal();
    var actor=window.currentUser||{},body={upserts:t._matrizRiesgos,deleteIds:Array.isArray(deleteIds)?deleteIds:[],actor:{login:actor.login||'',nombre:actor.nombre||actor.name||'',rol:actor.rol||''}};
    setTimeout(async function(){
      try{
        var base=riskApiBase();if(!base)throw new Error('API_BASE no configurado');
        var rr=await fetch(base+'/api/sgrt-state/'+encodeURIComponent(t.nit)+'/riesgos',{method:'PATCH',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(body)});
        var x=await rr.json().catch(function(){return{};});
        if(!rr.ok||!x.ok)throw new Error(x.error||('HTTP '+rr.status));
        t.sincronizado=true;t._changed=false;saveLocal();
      }catch(e){
        console.warn('[SGRT41 riesgos] PATCH atómico no disponible:',e.message||e);
        try{window._sgrtUpsertEstadoCompleto&&window._sgrtUpsertEstadoCompleto(t);}catch(e2){}
      }
    },80);
  }
  ...
  if(typeof oldEliminarRiesgo==='function')window.eliminarRiesgo=function(id){var row=(window.MATRIZ_DB||[]).find(function(x){return norm(x&&x.id)===norm(id);});var name=row&&(row.nit||row.tercero);var r=oldEliminarRiesgo.apply(this,arguments);setTimeout(function(){if(name)syncRiskForName(name,[String(id)]);},120);return r;};
