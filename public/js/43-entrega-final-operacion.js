...
(function(){
  ...
  function usagePage(){
    ...
    p=document.createElement('div');p.id='admin-pg-uso-recursos';p.className='page';p.innerHTML='\
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;margin-bottom:18px;">\
        <div><h2 style="font-family:Montserrat,sans-serif;font-size:18px;font-weight:800;color:var(--navy);margin:0;">Uso y Recursos</h2><div style="font-size:12px;color:var(--muted);margin-top:4px;">Lectura operativa por usuario: quién entró, cuántas sesiones abrió, cuántas llamadas hizo al backend y cuánto tráfico generó. La memoria/CPU se mide a nivel global del servidor, no por persona.</div></div>\
  ...
  async function loadUsage(){
    ...
    try{var j=await api('/api/telemetry/summary?days='+encodeURIComponent(days)),t=j.totals||{},sv=j.server||{},users=j.users||[];
      ...
      var kpis=[
        ['Usuarios únicos',t.UniqueUsers||0,'Personas distintas que generaron actividad durante el periodo elegido.'],
        ['Activos 24h',t.Active24h||0,'Usuarios que registraron al menos una actividad en las últimas 24 horas.'],
        ['Sesiones',t.Sessions||0,'Sesiones de navegación identificadas. Un usuario puede abrir más de una sesión.'],
        ['Solicitudes API',t.ApiRequests||0,'Llamadas realizadas desde la interfaz al backend: cargar, guardar, consultar o sincronizar datos.'],
        ['Acciones',t.Actions||0,'Eventos funcionales registrados: cambios de módulo, guardados, exportaciones, pruebas u otras acciones auditables.'],
        ['Pruebas ejecutadas',t.UserTests||0,'Sets de prueba de usuarios ejecutados desde ISEGURAS durante el periodo.'],
        ['Tráfico',mb(Number(t.BytesIn||0)+Number(t.BytesOut||0)),'Volumen aproximado de datos recibido y enviado por la aplicación. No representa costo de Azure ni consumo de Internet facturado.']
      ];
      body.innerHTML='<div style="padding:12px 14px;margin-bottom:12px;background:#f0f7ff;border:1px solid #bfdbfe;border-radius:9px;color:#334155;font-size:10.5px;line-height:1.55;"><b>Cómo leer este módulo:</b> la tabla por usuario muestra <b>actividad y tráfico atribuibles a cada sesión</b>. La RAM y el heap aparecen aparte porque Node/Azure ejecutan un proceso compartido: no es técnicamente correcto adjudicar MB de RAM exactos a una persona. Para comparar usuarios mira sesiones, API, acciones, interacciones y tráfico.</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;margin-bottom:14px;">'+kpis.map(function(x){return '<div title="'+esc(x[2])+'" style="background:white;border:1px solid #dbe5ee;border-radius:8px;padding:13px;"><div style="font-size:9.5px;color:#64748b;text-transform:uppercase;font-weight:800;">'+esc(x[0])+'</div><div style="font-family:Montserrat,sans-serif;font-size:23px;font-weight:900;color:#173b5f;margin-top:5px;">'+esc(x[1])+'</div><div style="font-size:9.2px;color:#64748b;line-height:1.35;margin-top:6px;">'+esc(x[2])+'</div></div>';}).join('')+'</div>\
      <div style="display:grid;grid-template-columns:2fr 1fr;gap:12px;align-items:start;">\
        <div class="card"><div style="padding:11px 14px;border-bottom:1px solid #e2e8f0;font-weight:800;color:#173b5f;">Actividad y recursos estimados por usuario <span style="font-size:9px;font-weight:600;color:#64748b;">(incluye usuarios configurados sin actividad)</span></div><div style="overflow:auto;max-height:430px;"><table style="width:100%;border-collapse:collapse;font-size:11px;"><thead><tr style="background:#f8fafc;position:sticky;top:0;"><th style="padding:8px;text-align:left;">Usuario</th><th>Rol</th><th>Entidad</th><th title="Sesiones distintas detectadas">Sesiones</th><th title="Solicitudes al backend">API</th><th title="Acciones funcionales registradas">Acciones</th><th title="API + acciones; sirve como referencia de actividad, no como CPU">Interacciones</th><th title="Bytes de entrada + salida registrados">Tráfico</th><th>Última actividad</th></tr></thead><tbody>'+users.map(function(u){var interactions=Number(u.ApiRequests||0)+Number(u.Actions||0);return '<tr style="border-top:1px solid #eef2f7;"><td style="padding:8px;"><b>'+esc(u.UserName||u.UserLogin)+'</b><div style="font-size:9px;color:#94a3b8;">'+esc(u.UserLogin)+'</div></td><td style="padding:8px;text-align:center;">'+esc(u.UserRole||'—')+'</td><td style="padding:8px;text-align:center;">'+esc(u.EntityId||'—')+'</td><td style="padding:8px;text-align:center;">'+Number(u.Sessions||0)+'</td><td style="padding:8px;text-align:center;">'+Number(u.ApiRequests||0)+'</td><td style="padding:8px;text-align:center;">'+Number(u.Actions||0)+'</td><td style="padding:8px;text-align:center;font-weight:800;color:#173b5f;">'+interactions+'</td><td style="padding:8px;text-align:center;">'+mb(Number(u.BytesIn||0)+Number(u.BytesOut||0))+'</td><td style="padding:8px;white-space:nowrap;">'+esc(dt(u.LastSeen))+'</td></tr>';}).join('')+'</tbody></table></div><div style="padding:9px 12px;border-top:1px solid #eef2f7;font-size:9.5px;color:#64748b;line-height:1.5;"><b>Interpretación:</b> más API/acciones/tráfico significa mayor uso operativo del sistema en el periodo. Esto no equivale directamente a CPU, RAM o costo monetario por usuario.</div></div>\
        <div class="card"><div style="padding:11px 14px;border-bottom:1px solid #e2e8f0;font-weight:800;color:#173b5f;">Recursos globales del servidor</div><div style="padding:13px;font-size:11px;line-height:1.75;"><b>RAM proceso (RSS):</b> '+esc(sv.rssMB||0)+' MB<div style="font-size:9px;color:#64748b;">Memoria total ocupada por el proceso Node, incluyendo heap y memoria nativa.</div><b>Heap usado:</b> '+esc(sv.heapUsedMB||0)+' MB<div style="font-size:9px;color:#64748b;">Memoria JavaScript que está en uso en este momento.</div><b>Heap total:</b> '+esc(sv.heapTotalMB||0)+' MB<div style="font-size:9px;color:#64748b;">Memoria que V8 tiene reservada para objetos JavaScript.</div><b>Uptime:</b> '+Math.round(Number(sv.uptimeSeconds||0)/60)+' min<div style="font-size:9px;color:#64748b;">Tiempo desde el último inicio/reinicio del backend.</div><b>Node:</b> '+esc(sv.node||'—')+'<div style="margin-top:9px;padding:8px;background:#fff7ed;border:1px solid #fed7aa;border-radius:6px;color:#9a3412;">'+esc(j.note||'El uso por usuario es una estimación operacional.')+'</div></div></div>\
      </div>';
      ...
      p.innerHTML='\
        ...
        <div class="card" style="margin-bottom:14px;border-left:4px solid #1e6bb8;"><div style="padding:14px;">\
          <div style="display:grid;grid-template-columns:2fr 1.4fr 1fr auto;gap:9px;align-items:end;">\
            ...
            <div><label style="display:block;font-size:10px;font-weight:800;color:#475569;margin-bottom:5px;">Set de prueba</label><select id="sgrt43-test-set" onchange="window.sgrt43ShowTestSet&&window.sgrt43ShowTestSet()" style="width:100%;padding:8px;border:1px solid #cbd5e1;border-radius:6px;"><option value="full">Completo</option><option value="access">Rol y módulos</option><option value="data">Datos compartidos / Azure</option><option value="concurrency">Concurrencia ligera</option></select></div>\
          ...
          <div id="sgrt43-test-user-detail" style="margin-top:10px;padding:9px 11px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:7px;font-size:10.5px;color:#334155;"></div><div id="sgrt43-test-set-help" style="margin-top:9px;padding:10px 12px;background:#f0f7ff;border:1px solid #bfdbfe;border-radius:7px;font-size:10px;color:#334155;line-height:1.5;"></div><div style="font-size:10px;color:#64748b;margin-top:9px;line-height:1.5;"><b>Importante:</b> no inicia sesión como el usuario seleccionado, no usa su contraseña y no modifica sus datos. La prueba de concurrencia solo realiza lecturas seguras para medir respuesta y errores; no es una prueba de estrés ni representa la capacidad máxima de producción.</div>\
        </div></div>\
  ...
  function populateTestUsers(){
    ...
    if(prev&&users[prev])el.value=prev;else if(rows.length)el.value=rows[0].login;showTestUser();showTestSet();
  }
  function showTestSet(){var el=document.getElementById('sgrt43-test-set'),box=document.getElementById('sgrt43-test-set-help');if(!el||!box)return;var v=s(el.value)||'full',m={full:'<b>Completo:</b> ejecuta rol/módulos + conexión y datos compartidos + telemetría + concurrencia ligera. Es el set recomendado para una demostración general.',access:'<b>Rol y módulos:</b> verifica que el usuario exista, tenga rol y que las pantallas esperadas para ese rol estén disponibles. No entra con su contraseña.',data:'<b>Datos compartidos / Azure:</b> verifica backend, conexión SQL, maestro de terceros, estado SGRT compartido y telemetría. Sirve para demostrar que distintos equipos ven la misma fuente.',concurrency:'<b>Concurrencia ligera:</b> lanza 1/5/10/20 lecturas simultáneas seguras y mide promedio, máximo y errores. No modifica datos y no sustituye una prueba de carga profesional.'};box.innerHTML=m[v]||m.full;}
  window.sgrt43ShowTestSet=showTestSet;
  ...
  async function genExcel(){try{var X=await ensureXLSX(),sel=selectedReport(),db=thirdDb(),t=sel.nit?db[sel.nit]:null,all=sel.nit&&t?[[sel.nit,t]]:Object.entries(db),wb=X.utils.book_new(),now=new Date();
      ...
      wb.SheetNames.forEach(function(n){var ws=wb.Sheets[n];ws['!cols']=Array.from({length:18},function(_,i){return {wch:i===0?22:i<4?26:34};});});var name='SGRT_Informe_'+(sel.nit||'General')+'_'+now.toISOString().slice(0,10)+'.xlsx',arr=X.write(wb,{bookType:'xlsx',type:'array'}),blob=new Blob([arr],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});try{if(window.sgrt45RegistrarDocumentoGenerado)await window.sgrt45RegistrarDocumentoGenerado(blob,name,{nit:sel.nit||'',contract:sel.contract||''});}catch(_reg){}if(window.sgrt25OpenArtifactPreview)window.sgrt25OpenArtifactPreview(blob,name,{kind:'xlsx',title:'Excel SGRT',workbook:wb,data:(window.sgrt42CollectReport?window.sgrt42CollectReport():null)});else X.writeFile(wb,name);toast('Excel SGRT generado, guardado en Documentación y listo para abrir/descargar.','success');event('report_export','Excel SGRT generado',{nit:sel.nit,contractNo:sel.contract,meta:{archivo:name}});
