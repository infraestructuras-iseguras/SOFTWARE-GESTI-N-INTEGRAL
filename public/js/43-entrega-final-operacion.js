...
(function(){
  ...
  async function loadUsage(){
    ...
    try{var j=await api('/api/telemetry/summary?days='+encodeURIComponent(days)),t=j.totals||{},sv=j.server||{},users=j.users||[];
      ...
      <div style="display:grid;grid-template-columns:2fr 1fr;gap:12px;align-items:start;">\
        <div class="card"><div style="padding:11px 14px;border-bottom:1px solid #e2e8f0;font-weight:800;color:#173b5f;">Consumo estimado por usuario</div><div style="overflow:auto;max-height:430px;"><table style="width:100%;border-collapse:collapse;font-size:11px;"><thead><tr style="background:#f8fafc;position:sticky;top:0;"><th style="padding:8px;text-align:left;">Usuario</th><th>Rol</th><th>Sesiones</th><th>API</th><th>Acciones</th><th>Tráfico</th><th>Última actividad</th></tr></thead><tbody>'+users.map(function(u){return '<tr style="border-top:1px solid #eef2f7;"><td style="padding:8px;"><b>'+esc(u.UserName||u.UserLogin)+'</b><div style="font-size:9px;color:#94a3b8;">'+esc(u.UserLogin)+'</div></td><td style="padding:8px;text-align:center;">'+esc(u.UserRole||'—')+'</td><td style="padding:8px;text-align:center;">'+Number(u.Sessions||0)+'</td><td style="padding:8px;text-align:center;">'+Number(u.ApiRequests||0)+'</td><td style="padding:8px;text-align:center;">'+Number(u.Actions||0)+'</td><td style="padding:8px;text-align:center;">'+mb(Number(u.BytesIn||0)+Number(u.BytesOut||0))+'</td><td style="padding:8px;white-space:nowrap;">'+esc(dt(u.LastSeen))+'</td></tr>';}).join('')+'</tbody></table></div></div>\
      ...
      p.innerHTML='\
        ...
        <div class="card" style="margin-bottom:14px;border-left:4px solid #1e6bb8;"><div style="padding:14px;">\
          <div style="display:grid;grid-template-columns:2fr 1.4fr 1fr auto;gap:9px;align-items:end;">\
            <div><label style="display:block;font-size:10px;font-weight:800;color:#475569;margin-bottom:5px;">Usuario a validar</label><select id="sgrt43-test-user" style="width:100%;padding:8px;border:1px solid #cbd5e1;border-radius:6px;"></select></div>\
          ...
          <div style="font-size:10px;color:#64748b;margin-top:9px;line-height:1.5;"><b>Importante:</b> no inicia sesión como el usuario seleccionado, no usa su contraseña y no modifica sus datos. La prueba de concurrencia solo realiza lecturas seguras para medir respuesta y errores; no es una prueba de estrés.</div>\
        </div></div>\
  ...
  function populateTestUsers(){
    ...
    el.innerHTML=rows.map(function(x){return '<option value="'+esc(x.login)+'">'+esc(x.u.name||x.login)+' · '+esc(x.u.rol||'Sin rol')+(x.u.entidad?' · '+esc(x.u.entidad):'')+'</option>';}).join('');
    if(prev&&users[prev])el.value=prev;else if(rows.length)el.value=rows[0].login;
  }
  ...
  async function seedAportes(force){var status=document.getElementById('sgrt43-demo-status');if(status)status.textContent='Verificando…';try{var tj=await api('/api/terceros'),rows=tj.data||tj.terceros||tj.rows||[],found=rows.find(function(x){return low(x.nombre||x.razonSocial||x.tercero).indexOf('aportes')>=0;});
      if(!found){if(status)status.textContent='No se encontró Aportes en Línea.';if(force)toast('No existe todavía un tercero llamado Aportes en Línea','warning');return false;}
  ...
  function wrapNav(){
    ...
    var nav=window.navTo;if(typeof nav==='function'&&!nav._sgrt43){var nn=function(el,pg){var r=nav.apply(this,arguments);event('page_view','Abrió '+pg,{meta:{page:pg}});return r;};nn._sgrt43=true;window.navTo=nn;}
    var dl=window.doLogin;if(typeof dl==='function'&&!dl._sgrt43){var nd=function(){var r=dl.apply(this,arguments);setTimeout(function(){wrapEvents();usagePage();testPage();injectDbSafe();event('login','Inicio de sesión');hydrateRisks();pollActivity();if(isIS())seedAportes(false);},700);return r;};nd._sgrt43=true;window.doLogin=nd;}
  }

  function init(){wrapEvents();wrapNav();usagePage();testPage();injectDbSafe();if(login()){event('session_start','Sesión de interfaz activa');hydrateRisks();pollActivity();}setInterval(function(){wrapEvents();if(login()){hydrateRisks();pollActivity();}},60000);setInterval(pollActivity,25000);}
