...
(function(){
  ...
  function normalizeThird(t){
    ...
    t.contratos.forEach(function(c){if(c.num&&(c.estado_aprobacion==='APROBADO'||c.estado==='Aprobado'))t.aprobadoPorContrato[c.num]=true;});
    return t;
  }
  ...
  function refreshAll(nit){
    ...
    if(nit){try{var s=document.getElementById('cls-tip-tercero-sel');if(s){s.value=nit;}}catch(e){}}
  }
  function payload(t){
  ...
  if(typeof oldApr==='function'){
    window._aprToggleContrato=function(nit,num){
      ...
      if(ahora){t.aprobado_clasif=t.aprobado_clasif||new Date().toISOString();t.habilitado_ac=true;}
