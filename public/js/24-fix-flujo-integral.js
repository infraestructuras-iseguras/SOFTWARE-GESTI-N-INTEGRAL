...
(function(){
  ...
  function saveDb(){
    ...
    // _lsSave también puede disparar sincronización automática. Cuando este módulo
    // ya está persistiendo contra la API, la pausamos para evitar bucles/reintentos
    // que podían volver a subir una versión antigua de TERCEROS_DB.
    var hadSkip=Object.prototype.hasOwnProperty.call(window,'_SGRT_SKIP_AUTOSYNC');
    var prevSkip=window._SGRT_SKIP_AUTOSYNC;
    window._SGRT_SKIP_AUTOSYNC=true;
    try{if(window._lsSave)window._lsSave();}catch(e4){}
    finally{if(hadSkip)window._SGRT_SKIP_AUTOSYNC=prevSkip;else try{delete window._SGRT_SKIP_AUTOSYNC;}catch(e5){window._SGRT_SKIP_AUTOSYNC=false;}}
  }
  ...
  function scheduleRemote(t){
    var nit=String(t&&t.nit||'').trim();if(!nit)return;
    if(syncTimers[nit])clearTimeout(syncTimers[nit]);
    syncTimers[nit]=setTimeout(function(){delete syncTimers[nit];upsertRemote(t);},250);
  }
  window._sgrtUpsertEstadoCompleto=upsertRemote;

  // ── Cargar desde servidor. La BD es la fuente de verdad para la existencia del tercero. ──
  // Solo se conserva temporalmente un registro local ausente en servidor cuando tiene
  // cambios pendientes (_changed=true y sincronizado!=true); así no reaparecen registros
  // borrados de la base de datos por culpa de cachés/localStorage antiguos.
  function purgeRemovedState(removedNits,validNits){
    var removed={};(removedNits||[]).forEach(function(n){removed[String(n)]=true;});
    var valid={};(validNits||[]).forEach(function(n){valid[String(n)]=true;});
    try{
      window.CUEST_RESPUESTAS=window.CUEST_RESPUESTAS||{};
      window.CUEST_CTRL_CUSTOM=window.CUEST_CTRL_CUSTOM||{};
      // Limpiar por el conjunto válido completo, no solo por los NIT que estaban en
      // TERCEROS_DB. Así también desaparecen cuestionarios huérfanos de fases antiguas.
      Object.keys(window.CUEST_RESPUESTAS).forEach(function(n){if(!valid[String(n)])delete window.CUEST_RESPUESTAS[n];});
      Object.keys(window.CUEST_CTRL_CUSTOM).forEach(function(n){if(!valid[String(n)])delete window.CUEST_CTRL_CUSTOM[n];});
      localStorage.setItem('sgrt_cuest_respuestas',JSON.stringify(window.CUEST_RESPUESTAS));
      localStorage.setItem('sgrt_cuest_custom',JSON.stringify(window.CUEST_CTRL_CUSTOM));
    }catch(e){}
    try{
      if(window.CLS_DB&&typeof window.CLS_DB==='object')Object.keys(window.CLS_DB).forEach(function(y){
        if(Array.isArray(window.CLS_DB[y]))window.CLS_DB[y]=window.CLS_DB[y].filter(function(r){var n=String((r&&r.nit)||(r&&r.NIT)||'').trim();return !n||!!valid[n];});
      });
    }catch(e){}
    try{
      if(Array.isArray(window.tercerosPendientesCuestionario)){
        var kept=window.tercerosPendientesCuestionario.filter(function(r){var n=String((r&&r.nit)||(r&&r.NIT)||r||'').trim();return !n||!!valid[n];});
        window.tercerosPendientesCuestionario.splice(0,window.tercerosPendientesCuestionario.length);
        Array.prototype.push.apply(window.tercerosPendientesCuestionario,kept);
      }
    }catch(e){}
    try{
      if(Array.isArray(window.MATRIZ_DB))window.MATRIZ_DB=window.MATRIZ_DB.filter(function(r){
        if(!r||typeof r!=='object')return true;
        var raw=r.nit!=null?r.nit:(r.NIT!=null?r.NIT:(r.terceroNit!=null?r.terceroNit:(r.tercero_nit!=null?r.tercero_nit:r.nitTercero)));
        if(raw==null||String(raw).trim()==='')return true;
        return !!valid[String(raw).trim()];
      });
    }catch(e){}
    try{
      var raw=localStorage.getItem('sgrt_terceros_pending');
      if(raw){
        var p=JSON.parse(raw);
        if(Array.isArray(p))p=p.filter(function(r){var n=String((r&&r.nit)||(r&&r.NIT)||r||'').trim();return !n||!!valid[n];});
        else if(p&&typeof p==='object')Object.keys(p).forEach(function(n){if(!valid[String(n)])delete p[n];});
        localStorage.setItem('sgrt_terceros_pending',JSON.stringify(p));
      }
    }catch(e){}
  }
  ...
  window.sgrtCargarDesdeServidor=async function(opts){
    ...
    try{
      ...
      var states={};
      if(pair[1].ok&&sx.ok&&Array.isArray(sx.data))sx.data.forEach(function(r){var n=String(r.nit||r.NIT||'').trim();if(n)states[n]=(r.estado_sgrt&&typeof r.estado_sgrt==='object')?r.estado_sgrt:{};});

      var d=db();
      var previous={};Object.keys(d).forEach(function(n){previous[n]=clone(d[n]);});
      var serverNits={};
      var next={};

      x.data.forEach(function(row){
        var rn=String(row.nit||row.NIT||'').trim();if(!rn)return;
        serverNits[rn]=true;
        var local=previous[rn]||{};
        var serverState=(states[rn]&&typeof states[rn]==='object')?states[rn]:null;
        var dirty=local&&local._changed===true&&local.sincronizado!==true;
        var merged;

        // Cuando existe estado extendido en servidor, ese estado manda. Únicamente una
        // edición local aún no sincronizada puede ponerse encima para no perder trabajo.
        if(serverState){merged=dirty?Object.assign({},serverState,local):Object.assign({},serverState);}
        else{
          // dbo.Terceros contiene datos básicos; conservar detalles locales del mismo NIT
          // (contratos/supervisores) si el estado extendido todavía no existe.
          merged=Object.assign({},local,row);
        }
        merged.nit=rn;
        merged.nombre=merged.nombre||row.nombre||row.Nombre_Tercero||row.Nombre||local.nombre||rn;
        if(row.domicilio!=null||row.Domicilio!=null)merged.domicilio=row.domicilio!=null?row.domicilio:row.Domicilio;
        if(row.servicio_contratado!=null||row.ServicioContratado!=null)merged.servicio_contratado=row.servicio_contratado!=null?row.servicio_contratado:row.ServicioContratado;
        next[rn]=normalizeThird(merged);
      });

      // Si alguien eliminó directamente el maestro dbo.Terceros, limpiar también
      // estados extendidos huérfanos para que ese NIT no reviva al ser creado de nuevo.
      Object.keys(states).forEach(function(n){
        if(serverNits[n])return;
        fetch(apiBase()+'/api/sgrt-state/'+encodeURIComponent(String(n)),{method:'DELETE'}).catch(function(){});
      });

      // Preservar únicamente altas/ediciones locales realmente pendientes que todavía
      // no alcanzaron la API. forceServer se usa cuando se requiere espejo 100% servidor.
      var pendientes=[];
      if(!opts.forceServer)Object.keys(previous).forEach(function(n){
        var t=previous[n];
        if(!serverNits[n]&&t&&t._changed===true&&t.sincronizado!==true){next[n]=normalizeThird(t);pendientes.push(n);}
      });

      // Reemplazar el objeto EN EL MISMO lugar para que todos los módulos que guardaron
      // una referencia a TERCEROS_DB vean inmediatamente altas y borrados.
      Object.keys(d).forEach(function(n){delete d[n];});
      Object.keys(next).forEach(function(n){d[n]=next[n];});
      window.TERCEROS_DB=d;

      var validNits=Object.keys(next);
      var removed=Object.keys(previous).filter(function(n){return !next[n];});
      purgeRemovedState(removed,validNits);

      // Restaurar cuestionarios solo de terceros válidos actuales.
      Object.keys(next).forEach(function(nit){
        var t=next[nit]||{};
        try{if(t._configPreguntas){window.CUEST_CTRL_CUSTOM=window.CUEST_CTRL_CUSTOM||{};window.CUEST_CTRL_CUSTOM[nit]=clone(t._configPreguntas);}}catch(e){}
        try{if(t._respuestas){window.CUEST_RESPUESTAS=window.CUEST_RESPUESTAS||{};window.CUEST_RESPUESTAS[nit]=clone(t._respuestas);}}catch(e){}
      });
      try{if(window.CUEST_CTRL_CUSTOM)localStorage.setItem('sgrt_cuest_custom',JSON.stringify(window.CUEST_CTRL_CUSTOM));}catch(e){}
      try{if(window.CUEST_RESPUESTAS)localStorage.setItem('sgrt_cuest_respuestas',JSON.stringify(window.CUEST_RESPUESTAS));}catch(e){}

      saveDb();
      // En sincronizaciones de fondo solo se actualiza la fuente de datos. No se
      // reconstruyen selectores, tablas ni formularios: evita parpadeos al cambiar
      // de módulo y conserva exactamente la vista que el usuario está utilizando.
      if(!opts.silentUi) refreshAll(opts.selectNit||'');
      // Reintentar altas pendientes sin permitir que una cancele la sincronización de otra.
      pendientes.forEach(function(n){scheduleRemote(d[n]);});
      return {ok:true,count:x.data.length,localPendientes:pendientes.length};
  ...
  document.addEventListener('DOMContentLoaded',function(){setTimeout(function(){window.sgrtCargarDesdeServidor({forceServer:true});},500);});
})();
