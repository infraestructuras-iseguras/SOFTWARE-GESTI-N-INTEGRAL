...
(function(){
  ...
  function answer(v){
    ...
    if(s==='no'||s==='no implementado'||s==='no implementada'||s==='no existe') return 'No';
  ...
  function persistSoon(nit){
    // El consolidado 42 hace un único autoguardado liviano por pausa del usuario.
    // Evita duplicar JSON.stringify(Terceros + evidencias) y sincronizaciones por cada selector.
  ...
  /* -------------------------------------------------------------
   * Guardado reforzado (hotfix): borrador/controles no dependen del debounce.
   * - Copia los valores visibles al estado en memoria antes de guardar.
   * - Escribe una instantánea síncrona en localStorage.
   * - En pagehide/visibilitychange hace un último flush local.
   * ------------------------------------------------------------- */
  function syncVisibleToMemory(nit){
    nit=norm(nit||window.nitActual||((document.getElementById('q-tercero')||{}).value));
    if(!nit)return '';
    var nitKey=nit.replace(/[^a-z0-9]/gi,'_');
    var r=getResponses();if(!r[nit])r[nit]={};

    Array.prototype.forEach.call(document.querySelectorAll('select[id^="qa_'+nitKey+'_"]'),function(el){
      var oc=el.getAttribute('onchange')||'';
      var m=oc.match(/onChangeAtribCuest\('([^']*)','([^']*)',(\d+),(\d+)\)/);
      if(!m)return;
      var key=m[2],ctrl=parseInt(m[3],10),ai=parseInt(m[4],10);
      ensureResp(nit,key,ctrl)['a'+ai]=answer(el.value);
    });
    Array.prototype.forEach.call(document.querySelectorAll('textarea[id^="obs_'+nitKey+'_"]'),function(el){
      var oc=el.getAttribute('onchange')||'';
      var m=oc.match(/onChangeObsCuest\('([^']*)','([^']*)',(\d+),/);
      if(!m)return;
      ensureResp(nit,m[2],parseInt(m[3],10)).obs=el.value||'';
    });
    stashContract(nit);
    return nit;
  }

  function immediateLocalSnapshot(nit){
    nit=norm(nit||window.nitActual||((document.getElementById('q-tercero')||{}).value));
    try{
      var db=getDB(),r=getResponses();
      ...
      localStorage.setItem('sgrt_last_change_v1',JSON.stringify({kind:'guardado-inmediato-ac',nit:nit||'',contrato:activeContract(nit||''),ts:Date.now()}));
      return true;
    }catch(e){
      console.error('[SGRT] Error guardando instantánea local:',e);
      return false;
    }
  }

  function remoteSaveNow(nit){
    nit=norm(nit);if(!nit)return;
    var t=getDB()[nit];
    if(!t||typeof window._sgrtUpsertEstadoCompleto!=='function')return;
    try{Promise.resolve(window._sgrtUpsertEstadoCompleto(t)).catch(function(e){console.warn('[SGRT] Sync remota AC pendiente:',e);});}catch(e){}
  }

  function saveDraftNow(){
    var nit=syncVisibleToMemory();
    if(!nit){try{window.showToast&&window.showToast('Selecciona un tercero primero','error',2500);}catch(e){}return false;}
    try{
      var fecha=new Date().toLocaleString('es-CO',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'});
      var borrador={
        nit:nit,fecha:fecha,
        respuestas:JSON.parse(JSON.stringify(getResponses()[nit]||{})),
        customCtrls:JSON.parse(JSON.stringify((window.CUEST_CTRL_CUSTOM||{})[nit]||{})),
        contrato:activeContract(nit)||''
      };
      localStorage.setItem('cuest_borrador_'+nit,JSON.stringify(borrador));
      immediateLocalSnapshot(nit);
      var info=document.getElementById('q-borrador-info'),ts=document.getElementById('q-borrador-ts');
      if(info&&ts){info.style.display='block';ts.textContent='Borrador guardado el '+fecha+' — puedes actualizar o continuar después.';}
      try{window._flashGuardadoCuest&&window._flashGuardadoCuest();}catch(e){}
      try{window.showToast&&window.showToast('✅ Borrador guardado correctamente','success',3000);}catch(e){}
      remoteSaveNow(nit);
      return true;
    }catch(e){
      console.error('[SGRT] Error guardar borrador:',e);
      try{window.showToast&&window.showToast('❌ No se pudo guardar el borrador','error',3000);}catch(ex){}
      return false;
    }
  }

  function installSaveHardening(){
    // Borrador: reemplazo puntual para garantizar persistencia inmediata.
    window.guardarBorradorCuestionario=saveDraftNow;

    // Guardar cuestionario: conservar validaciones/cálculos existentes, pero
    // sincronizar primero el DOM y persistir inmediatamente al terminar.
    if(typeof window.guardarCuestionarioCompleto==='function'&&!window.guardarCuestionarioCompleto.__sgrtSafeSave){
      var oldFull=window.guardarCuestionarioCompleto;
      var wrapped=function(){
        var nit=syncVisibleToMemory();
        immediateLocalSnapshot(nit);
        var ret=oldFull.apply(this,arguments);
        immediateLocalSnapshot(nit);
        remoteSaveNow(nit);
        return ret;
      };
      wrapped.__sgrtSafeSave=true;
      window.guardarCuestionarioCompleto=wrapped;
    }

    // Configuración de controles del Administrador: tras cualquier cambio,
    // forzar la instantánea local para que un refresh no revierta el catálogo.
    ['ctrlAgregar','ctrlTgl','ctrlTodas','ctrlDel'].forEach(function(name){
      var old=window[name];
      if(typeof old!=='function'||old.__sgrtSafeSave)return;
      var w=function(){
        var ret=old.apply(this,arguments);
        try{window._lsSave&&window._lsSave();}catch(e){}
        immediateLocalSnapshot(window.nitActual||'');
        return ret;
      };
      w.__sgrtSafeSave=true;window[name]=w;
    });
  }

  function flushBeforeLeave(){
    if(window._sgrt42Active)return;
    try{var nit=syncVisibleToMemory();immediateLocalSnapshot(nit);}catch(e){}
  }

  function init(){
    removeSmallGoButtons();
    if(!window._sgrt42Active){
      installSaveHardening();
      setTimeout(installSaveHardening,700); // por si un módulo anterior reinstala handlers
    }
    ...
    document.addEventListener('visibilitychange',function(){if(document.hidden)flushBeforeLeave();});
  }
