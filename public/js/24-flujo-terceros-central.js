...
(function(){
  ...
  function allRecords(){
    var out={};
    // La clave compartida es la fuente canónica. Su valor {} significa cero
    // registros y debe ganar sobre cualquier copia antigua que quede en memoria.
    var sharedRaw=null;try{sharedRaw=localStorage.getItem('sgrt_terceros_db_shared');}catch(e){}
    if(sharedRaw!==null){
      try{out=JSON.parse(sharedRaw)||{};}catch(e2){out={};}
    }else{
      try{var snap=JSON.parse(localStorage.getItem('sgrt_v8')||'{}');Object.assign(out,snap.TERCEROS_DB||{});}catch(e3){}
      try{Object.assign(out,window.TERCEROS_DB||{});}catch(e4){}
    }
