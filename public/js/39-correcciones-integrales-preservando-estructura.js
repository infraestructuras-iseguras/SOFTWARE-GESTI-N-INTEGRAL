...
(function(){
  ...
  window._AUTOREFRESH_INTERVAL=setInterval(function(){
    ...
    try{if(typeof window.sgrtCargarDesdeServidor==='function')window.sgrtCargarDesdeServidor({silentUi:true});}catch(e){}
  },60000);
  ...
  function rebuildEvidenceRepository(){
    ...
    var ev=evidence();Object.keys(ev).forEach(function(sk){(ev[sk]||[]).forEach(function(item){
      ...
      else cf.children.push({id:'repo_'+item.id,name:item.name,type:'file',size:item.size||0,fecha:item.fecha||'',mime:item.type||'',_evidId:item.id,_origen:'Ambiente de Control',_contrato:c.contract,_tip:c.key,_ctrl:c.ctrl,sharePointUrl:item.sharePointUrl||'',sharePointPath:item.sharePointPath||''});
    });});
  ...
  window.registrarEvidenciaCuest=async function(nitKey,key,ctrlN,inputEl){
    ...
    try{window.showToast&&window.showToast((saved?saved+' evidencia(s) guardada(s) en Documentación y Evidencias. ':'')+(tooLarge.length?'Se omitieron '+tooLarge.length+' archivo(s) mayores de 20 MB.':''),saved?'success':'warning',3500);}catch(e){}
  };
  ...
  if(typeof oldServerLoad==='function')window.sgrtCargarDesdeServidor=async function(opts){
    var silent=!!(opts&&opts.silentUi);
    ...
    await importRemoteEvidence();if(!silent)scheduleReportPostProcess(false);return r;
  };
