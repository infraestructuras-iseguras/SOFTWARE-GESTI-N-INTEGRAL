...
(function(){
  ...
  var STORAGE_COMPACTED_KEY='sgrt_storage_compacted_v4';
  var LAST_REMOTE_LOGIN_KEY='sgrt_last_login_remote_v4';
  var snapshotTimer=null;
  var reportTimer=null;
  var sharePointStatus={checkedAt:0,connected:false,pending:null};
  var DOCUMENT_MAX_BYTES=20*1024*1024;
  var storageRecoveryPending=false;
  ...
  function responseWeight(obj){
    var count=0,seen=[];
    function walk(x){
      if(!x||typeof x!=='object'||seen.indexOf(x)>=0)return;seen.push(x);
      if(Object.prototype.hasOwnProperty.call(x,'a1')&&answered(x)){count++;return;}
      Object.keys(x).forEach(function(k){if(k.indexOf('__')!==0)walk(x[k]);});
    }
    walk(obj);return count;
  }
  function bestContractResponses(t,contract){
    contract=norm(contract);var candidates=[];
    function add(x){if(x&&typeof x==='object')candidates.push(x);}
    add(t&&t.respuestasACPorContrato&&t.respuestasACPorContrato[contract]);
    add(t&&t.acPorContrato&&t.acPorContrato[contract]&&t.acPorContrato[contract].respuestas);
    add(t&&t.borradoresACPorContrato&&t.borradoresACPorContrato[contract]&&t.borradoresACPorContrato[contract].respuestas);
    add(t&&t._respuestasPorContrato&&t._respuestasPorContrato[contract]);
    add(t&&t.respuestasPorContrato&&t.respuestasPorContrato[contract]);
    if(norm(t&&t.contratoEval)===contract)add(t&&t._respuestas);
    var best={},weight=-1;candidates.forEach(function(x){var w=responseWeight(x);if(w>weight){best=x;weight=w;}});
    return {responses:best||{},weight:Math.max(0,weight)};
  }
  ...
  function responseForContract(t,nit,contract){
    var best=bestContractResponses(t,contract);if(best.weight>0)return best.responses;
  ...
  function sanitizedEvidence(){
    var out={};
    Object.keys(evidence()).forEach(function(sk){
      out[sk]=(evidence()[sk]||[]).map(function(item){var x=Object.assign({},item);delete x.dataUrl;delete x.dataURL;delete x._dataURL;return x;});
    });
    return out;
  }
  ...
  function flushLocal(){
    ...
    try{localStorage.setItem(SNAPSHOT_KEY,JSON.stringify(buildSnapshot()));return true;}
      ...
      // No mostrar avisos de cuota al usuario. Primero compactamos automáticamente
      // archivos heredados que todavía estén incrustados como Base64 y reintentamos.
      if(!storageRecoveryPending){
        storageRecoveryPending=true;
        setTimeout(function(){
          Promise.resolve(compactLegacyStorage()).then(function(){
            try{localStorage.setItem(SNAPSHOT_KEY,JSON.stringify(buildSnapshot()));}catch(x){}
          }).finally(function(){storageRecoveryPending=false;});
        },0);
      }
      console.warn('SGRT: el guardado local se recuperará mediante compactación automática.',e);
      return false;
    }
  }
  ...
  function legacyFileId(prefix){return 'legacy_'+prefix+'_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,9);}
  async function compactInlineTree(node,prefix){
    if(!node||typeof node!=='object')return 0;var changed=0;
    var data=node.dataUrl||node.dataURL||node._dataURL;
    if(typeof data==='string'&&data.indexOf('data:')===0){
      var id=node._evidId||node.id||legacyFileId(prefix);node._evidId=id;
      try{await putFile(id,data);delete node.dataUrl;delete node.dataURL;delete node._dataURL;changed++;}catch(e){}
    }
    var kids=Array.isArray(node.children)?node.children:[];
    for(var i=0;i<kids.length;i++)changed+=await compactInlineTree(kids[i],prefix);
    return changed;
  }
  async function compactStorageTree(key,prefix){
    var tree=null;try{tree=JSON.parse(localStorage.getItem(key)||'null');}catch(e){}if(!tree)return 0;
    var changed=await compactInlineTree(tree,prefix);
    try{localStorage.setItem(key,JSON.stringify(tree));}catch(e2){}
    if(key==='rpt_fs_v2')window.RPT_FS=tree;
    return changed;
  }
  function updateStorageLabels(){
    // El usuario no necesita ver cuotas ni mensajes de capacidad. Los binarios
    // se guardan fuera de localStorage y aquí solo se indica disponibilidad.
    var text='Repositorio documental disponible';
    ['od-uso-lbl','admin-od-uso-lbl'].forEach(function(id){var el=document.getElementById(id);if(el&&!/SharePoint empresarial/i.test(el.textContent||''))el.textContent=text;});
  }
  async function compactLegacyStorage(){
    if(!window.indexedDB)return 0;var total=0;
    total+=await compactStorageTree('od_sgrt_v8','od');
    total+=await compactStorageTree('rpt_fs_v2','rpt');
    try{localStorage.setItem(STORAGE_COMPACTED_KEY,JSON.stringify({at:new Date().toISOString(),files:total}));}catch(e){}
    updateStorageLabels();return total;
  }
  function installReportStorageBridge(){
    if(window._sgrt39ReportBridge)return;window._sgrt39ReportBridge=true;
    var oldSave=window._saveRPT,oldOpen=window._rptOpenFile,oldDownload=window._rptDownload;
    if(typeof oldSave==='function')window._saveRPT=function(){
      var tree=window.RPT_FS;if(!tree)return oldSave.apply(this,arguments);
      compactInlineTree(tree,'rpt').then(function(){try{oldSave();}catch(e){}updateStorageLabels();});
    };
    if(typeof oldOpen==='function')window._rptOpenFile=async function(id){
      var item=typeof window._findById==='function'?window._findById(id):null;
      if(item&&item._evidId){var data=await getFile(item._evidId);if(data){var a=document.createElement('a');a.href=data;a.target='_blank';a.click();return;}}
      if(item&&item.sharePointUrl){window.open(item.sharePointUrl,'_blank','noopener');return;}
      return oldOpen.apply(this,arguments);
    };
    if(typeof oldDownload==='function')window._rptDownload=async function(id){
      var item=typeof window._findById==='function'?window._findById(id):null;
      if(item&&item._evidId){var data=await getFile(item._evidId);if(data){var a=document.createElement('a');a.href=data;a.download=item.name||'documento';a.click();return;}}
      if(item&&item.sharePointUrl){window.open(item.sharePointUrl+(item.sharePointUrl.indexOf('?')>=0?'&':'?')+'download=1','_blank','noopener');return;}
      return oldDownload.apply(this,arguments);
    };
  }
  window.sgrtCompactarAlmacenamiento=compactLegacyStorage;
  ...
  async function migrateInlineFiles(){
    ...
    Object.keys(ev).forEach(function(sk){(ev[sk]||[]).forEach(function(item){
      ...
      var data=item.dataUrl||item.dataURL||item._dataURL;if(data){jobs.push(putFile(item.id,data).then(function(){delete item.dataUrl;delete item.dataURL;delete item._dataURL;}).catch(function(){}));}
    });});
  ...
  function tipologyLabel(nit,contract,key){
    key=norm(key)||'general';var t=db()[nit]||{},label='',dims=dimsFor(t,contract)||[];
    var d=dims.find(function(x){return dimKey(x)===key.toLowerCase();});if(d)label=tipologyName(d);
    try{if(!label&&window.TIPOLOGIA_CATALOG&&window.TIPOLOGIA_CATALOG[key])label=norm(window.TIPOLOGIA_CATALOG[key].nombre||window.TIPOLOGIA_CATALOG[key].label);}catch(e){}
    try{if(!label&&window.SECCIONES_INFO&&window.SECCIONES_INFO[key])label=norm(window.SECCIONES_INFO[key].label);}catch(e2){}
    return label||key||'General';
  }
  function sharePointFolder(origin,nit,contract,riskId,tipKey){
    ...
    if(tipKey)folder+='/Tipología '+pathName(tipologyLabel(nit,contract,tipKey));
  ...
  async function storeInSharePoint(dataUrl,folder,file){
    ...
    try{
      ...
      var up=await fetch(base+'/api/sharepoint/upload',{method:'POST',headers:headers,body:JSON.stringify({path:folder,name:file.name,mimeType:file.type||'application/octet-stream',contentBase64:String(dataUrl).split(',').pop()})});
      if(!up.ok)return false;var result=await up.json().catch(function(){return{};});return result.item||true;
    }catch(e){return false;}
  }
  function findEvidenceMeta(id){var found=null;Object.keys(evidence()).some(function(sk){return (evidence()[sk]||[]).some(function(x){if(x.id===id){found=x;return true;}return false;});});if(found)return found;Object.keys(riskEvidence()).some(function(rid){return (riskEvidence()[rid]||[]).some(function(x){if(x.id===id){found=x;return true;}return false;});});return found;}
  async function downloadEvidence(id,name,fallback){var data=fallback||await getFile(id),meta=findEvidenceMeta(id)||{};if(!data&&meta.sharePointUrl){window.open(meta.sharePointUrl+(meta.sharePointUrl.indexOf('?')>=0?'&':'?')+'download=1','_blank','noopener');return;}if(!data){try{window.showToast&&window.showToast('No se encontró el contenido del archivo.','error',2500);}catch(e){}return;}var a=document.createElement('a');a.href=data;a.download=name||'evidencia';document.body.appendChild(a);a.click();a.remove();}
  ...
  function importLegacyRepo(fs){
    ...
    found.forEach(function(x){
      ...
      if(!ev[sk])ev[sk]=[];ev[sk].push(item);var inline=f.dataUrl||f.dataURL||f._dataURL;if(inline)putFile(id,inline).catch(function(){});
    });
  }
  function rebuildEvidenceRepository(){
    ...
    var ev=evidence();Object.keys(ev).forEach(function(sk){(ev[sk]||[]).forEach(function(item){
      var c=inferEvidenceContext(sk,item),t=db()[c.nit]||{},thirdName=(t.nombre||c.nit)+' ('+c.nit+')',tipName=tipologyLabel(c.nit,c.contract,c.key);
      var tf=ensureFolder(ac,'f_ac_'+safeNit(c.nit),thirdName),cf=ensureFolder(tf,'f_ac_'+safeNit(c.nit)+'_'+safeNit(c.contract),'Contrato '+c.contract),pf=ensureFolder(cf,'f_ac_'+safeNit(c.nit)+'_'+safeNit(c.contract)+'_'+safeNit(c.key),'Tipología '+tipName);
      var duplicate=(pf.children||[]).find(function(x){return x._evidId===item.id||(x.name===item.name&&Number(x.size||0)===Number(item.size||0)&&norm(x._ctrl)===norm(c.ctrl));});
      if(duplicate){duplicate.sharePointUrl=item.sharePointUrl||duplicate.sharePointUrl||'';duplicate.sharePointPath=item.sharePointPath||duplicate.sharePointPath||'';duplicate._verificada=true;}
      else pf.children.push({id:'repo_'+item.id,name:item.name,type:'file',size:item.size||0,fecha:item.fecha||'',mime:item.type||'',_evidId:item.id,_origen:'Ambiente de Control',_contrato:c.contract,_tip:c.key,_tipNombre:tipName,_ctrl:c.ctrl,_verificada:true,sharePointUrl:item.sharePointUrl||'',sharePointPath:item.sharePointPath||''});
    });});
    Object.keys(riskEvidence()).forEach(function(riskId){(riskEvidence()[riskId]||[]).forEach(function(item){
      ...
      if(duplicate){duplicate.sharePointUrl=item.sharePointUrl||duplicate.sharePointUrl||'';duplicate.sharePointPath=item.sharePointPath||duplicate.sharePointPath||'';}
      else rf.children.push({id:'repo_'+item.id,name:item.name,type:'file',size:item.size||0,fecha:item.fecha||'',mime:item.type||'',_evidId:item.id,_origen:'Análisis de Riesgos',_contrato:c.contract,_riesgo:riskId,sharePointUrl:item.sharePointUrl||'',sharePointPath:item.sharePointPath||''});
    });});
    persistRiskEvidence();
  ...
  async function previewEvidence(item){
    ...
    if(!data&&item.sharePointUrl){window.open(item.sharePointUrl,'_blank','noopener');return;}
  ...
  if(typeof previousOdRender==='function')window.odRender=function(){
    var r=previousOdRender.apply(this,arguments),grid=document.getElementById('od-grid');updateStorageLabels();if(!grid)return r;
    Array.from(grid.querySelectorAll('[onclick*="window.odDescargar"]')).forEach(function(clickArea){
      ...
      var card=clickArea.parentElement;if(!card)return;
      if(!card.querySelector('.sgrt-evidence-verified')){var badge=document.createElement('div');badge.className='sgrt-evidence-verified';badge.style.cssText='display:inline-flex;align-items:center;gap:3px;margin-top:6px;padding:2px 7px;background:#ecfdf5;border:1px solid #86efac;border-radius:999px;color:#15803d;font-size:9px;font-weight:800;';badge.textContent='✓ Evidencia cargada';card.appendChild(badge);}
      if(card.querySelector('.sgrt-evidence-actions'))return;
  ...
  if(typeof previousAdminOdRender==='function')window.adminOdRender=function(){
    ...
    Array.from(grid.querySelectorAll('[data-item-id]')).forEach(function(card){
      var id='';try{id=decodeURIComponent(card.getAttribute('data-item-id')||'');}catch(e){}var item=findRepoItem(parseRepo(),id);if(!item||item.type==='folder'||!item._evidId)return;
      if(!card.querySelector('.sgrt-evidence-verified')){var badge=document.createElement('div');badge.className='sgrt-evidence-verified';badge.style.cssText='display:inline-flex;align-items:center;gap:3px;margin-top:6px;padding:2px 7px;background:#ecfdf5;border:1px solid #86efac;border-radius:999px;color:#15803d;font-size:9px;font-weight:800;';badge.textContent='✓ Evidencia cargada';card.appendChild(badge);}
      if(card.querySelector('.sgrt-evidence-actions'))return;
      var actions=document.createElement('div');actions.className='sgrt-evidence-actions';actions.style.cssText='display:flex;justify-content:center;gap:5px;margin-top:7px;';
  ...
  window.registrarEvidenciaCuest=async function(nitKey,key,ctrlN,inputEl){
    ...
    var tooLarge=files.filter(function(f){return f.size>DOCUMENT_MAX_BYTES;});files=files.filter(function(f){return f.size<=DOCUMENT_MAX_BYTES;});
    ...
    for(var i=0;i<files.length;i++){
      ...
      try{
        var data=await readFile(f),id=evidenceId(),item={id:id,name:f.name,size:f.size,type:f.type,fecha:new Date().toISOString(),nit:nit,contrato:contract,tipologia:key,control:ctrlN,subidoPor:(window.currentUser||{}).name||'—',estadoCarga:'Cargada',verificada:true,sincronizadaServidor:false};
        await putFile(id,data);
        var folder=sharePointFolder('Evidencias Ambiente de Control',nit,contract,null,key),cloud=await storeInSharePoint(data,folder,f);
        if(cloud){item.sharePointPath=folder+'/'+f.name;item.sharePointUrl=cloud.webUrl||'';item.sharePointId=cloud.id||'';}
        ev[sk].push(item);savedItems.push(item);saved++;
      }catch(e){}
    }
    rebuildEvidenceRepository();flushLocal();try{window.renderEvCuest&&window.renderEvCuest(nitKey,key,ctrlN);}catch(e){}
    var syncOk=false;if(saved)syncOk=await syncOneThird(nit);
    savedItems.forEach(function(item){item.sincronizadaServidor=syncOk!==false;item.fechaVerificacion=new Date().toISOString();});rebuildEvidenceRepository();flushLocal();try{window.renderEvCuest&&window.renderEvCuest(nitKey,key,ctrlN);}catch(e2){}
    var tip=tipologyLabel(nit,contract,key);
    try{window.showToast&&window.showToast((saved?saved+' evidencia(s) cargada(s) ✓ · Documentación > Contrato '+contract+' > Tipología '+tip+'. ':'')+(tooLarge.length?'Se omitieron '+tooLarge.length+' archivo(s) mayores de 20 MB.':''),saved?'success':'warning',4200);}catch(e){}
  };
  window.renderEvCuest=function(nitKey,key,ctrlN){
    var wrap=document.getElementById('evw_'+nitKey+'_'+key+'_'+ctrlN);if(!wrap)return;var sk=nitKey+'_'+key+'_'+ctrlN,items=evidence()[sk]||[],nit=findNitFromSafe(nitKey),contract=currentContract(nit)||'Sin contrato',tip=tipologyLabel(nit,contract,key);
    wrap.style.width='100%';
    wrap.innerHTML=items.map(function(x,i){
      var repoId='repo_'+x.id,server=x.sincronizadaServidor===false?'<span style="color:#b45309;font-size:9.5px;">Pendiente de sincronización</span>':'<span style="color:#15803d;font-size:9.5px;">Sincronizada</span>';
      return '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:7px 9px;background:#f8fffb;border:1px solid #bbf7d0;border-radius:7px;font-size:11px;min-width:300px;max-width:100%;"><div style="min-width:0;flex:1;"><div style="display:flex;align-items:center;gap:6px;min-width:0;"><span style="color:#15803d;font-weight:900;">✓ Cargada</span><span style="font-weight:700;color:#1e6bb8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+esc(x.name)+'</span><span style="color:var(--muted);font-size:9.5px;white-space:nowrap;">'+Math.max(1,Math.round((x.size||0)/1024))+' KB</span></div><div style="margin-top:2px;color:#64748b;font-size:9.5px;">Documentación · Contrato '+esc(x.contrato||contract)+' · Tipología '+esc(tip)+' · '+server+'</div></div><div style="display:flex;gap:4px;flex-shrink:0;"><button type="button" onclick="window.odVer(\''+esc(repoId)+'\')" style="padding:3px 7px;border:1px solid #93c5fd;border-radius:5px;background:white;color:#1d4ed8;font-size:9.5px;font-weight:700;cursor:pointer;">Ver</button><button type="button" onclick="window.sgrtDescargarEvidencia(\''+esc(x.id)+'\',\''+esc(x.name).replace(/&#39;/g,"\\'")+'\')" style="padding:3px 7px;border:1px solid #d7dee8;border-radius:5px;background:white;color:#475569;font-size:9.5px;font-weight:700;cursor:pointer;">Descargar</button><button type="button" data-sk="'+esc(sk)+'" data-idx="'+i+'" onclick="window.elimEvCuest(this)" style="background:none;border:0;color:var(--red);cursor:pointer;font-size:14px;">×</button></div></div>';
    }).join('');
  };
  ...
  window.registrarEvRiesgo=function(inputEl){
    ...
    files.forEach(function(file){
      if(file.size>DOCUMENT_MAX_BYTES){rejected++;return;}
      var reader=new FileReader();reader.onload=function(e){
        ...
        if(saved+rejected===files.length)try{window.showToast&&window.showToast(saved+' evidencia(s) preparadas para el riesgo'+(rejected?' · '+rejected+' omitida(s) por superar 20 MB':''),'success',3000);}catch(x2){}
      };reader.onerror=function(){rejected++;};reader.readAsDataURL(file);
    });
    if(!saved&&rejected===files.length)try{window.showToast&&window.showToast('Los archivos superan el máximo permitido de 20 MB.','error',3000);}catch(e){}
  };
  ...
  async function exportEvidenceForNit(nit){
    ...
    for(var sk of Object.keys(ev)){
      ...
      for(var item of items){var x=clone(item),data=x.dataUrl||await getFile(x.id),ctx=inferEvidenceContext(sk,x),folder=sharePointFolder('Evidencias Ambiente de Control',ctx.nit,ctx.contract,null,ctx.key),cloud=x.sharePointUrl?false:(data?await storeInSharePoint(data,folder,x):false);if(cloud){x.sharePointPath=folder+'/'+x.name;x.sharePointUrl=cloud.webUrl||x.sharePointUrl||'';x.sharePointId=cloud.id||x.sharePointId||'';item.sharePointPath=x.sharePointPath;item.sharePointUrl=x.sharePointUrl;item.sharePointId=x.sharePointId;}if(!x.sharePointUrl&&data)x.dataUrl=data;out[sk].push(x);}
    }
    return out;
  }
  ...
  async function syncOneThird(nit){
    ...
    // La copia de transporte puede incluir archivos para Azure; el objeto vivo del
    // tercero nunca los recibe, evitando que saveDb() llene localStorage con Base64.
    var remoteThird=Object.assign({},t),pack=await exportEvidenceForNit(nit),riskPack=await exportRiskEvidenceForNit(nit);
    if(Object.keys(pack).length)remoteThird._evidenciasAC=pack;if(Object.keys(riskPack).length)remoteThird._evidenciasRiesgo=riskPack;
    try{
      if(typeof window._sgrtUpsertEstadoCompleto!=='function')return false;
      var ok=await window._sgrtUpsertEstadoCompleto(remoteThird);t.sincronizado=ok!==false;t._changed=ok===false;return ok!==false;
    }catch(e){t.sincronizado=false;t._changed=true;return false;}finally{flushLocal();}
  }

  /* ---------------- Respuestas, borrador y guardado completo ---------------- */
  function responseContracts(t){
    var seen={};function add(v){v=norm(v);if(v)seen[v]=1;}
    (t&&t.contratos||[]).forEach(function(c){add(contractNum(c));});add(t&&t.contratoEval);
    ['respuestasACPorContrato','acPorContrato','borradoresACPorContrato','_respuestasPorContrato','respuestasPorContrato'].forEach(function(k){Object.keys(t&&t[k]||{}).forEach(add);});
    return Object.keys(seen);
  }
  function normalizeRemoteResponses(){
    Object.keys(db()).forEach(function(nit){var t=db()[nit];if(!t)return;if(!t.respuestasACPorContrato||typeof t.respuestasACPorContrato!=='object')t.respuestasACPorContrato={};
      responseContracts(t).forEach(function(contract){var best=bestContractResponses(t,contract);if(best.weight>0)t.respuestasACPorContrato[contract]=clone(best.responses);});
    });
  }
  function activateAuthoritativeResponses(nit,contract){
    nit=norm(nit);contract=norm(contract);var t=db()[nit];if(!t||!contract)return 0;normalizeRemoteResponses();
    var best=bestContractResponses(t,contract),rr=clone(best.responses||{});responses()[nit]=rr;t.contratoEval=contract;
    try{var shared=JSON.parse(localStorage.getItem('sgrt_cuest_respuestas')||'{}');shared[nit]=rr;localStorage.setItem('sgrt_cuest_respuestas',JSON.stringify(shared));}catch(e){}
    return best.weight;
  }
  var previousQuestionnaireLoad=window.cargarCuestionarioTercero;
  if(typeof previousQuestionnaireLoad==='function')window.cargarCuestionarioTercero=function(){
    var nit=norm((document.getElementById('q-tercero')||{}).value),contract=currentContract(nit);if(nit&&contract)activateAuthoritativeResponses(nit,contract);
    return previousQuestionnaireLoad.apply(this,arguments);
  };
  var previousContractChange=window.qCambiarContrato;
  if(typeof previousContractChange==='function')window.qCambiarContrato=function(contract){
    var r=previousContractChange.apply(this,arguments),nit=currentNit(),selected=norm(contract)||currentContract(nit);
    if(nit&&selected){activateAuthoritativeResponses(nit,selected);refreshEvaluatorProgress(nit,selected);}return r;
  };
  ...
  if(typeof oldServerLoad==='function')window.sgrtCargarDesdeServidor=async function(){
    var r=await oldServerLoad.apply(this,arguments);normalizeRemoteResponses();
    var nit=currentNit(),contract=currentContract(nit);if(nit&&contract)activateAuthoritativeResponses(nit,contract);
    await importRemoteEvidence();scheduleReportPostProcess(false);return r;
  };
  var previousLogin=window.doLogin;
  if(typeof previousLogin==='function')window.doLogin=function(){
    var login=norm((document.getElementById('li-user')||{}).value).toLowerCase(),last='';try{last=norm(localStorage.getItem(LAST_REMOTE_LOGIN_KEY)).toLowerCase();}catch(e){}
    var r=previousLogin.apply(this,arguments);
    setTimeout(function(){
      if(!window.currentUser||!login||typeof window.sgrtCargarDesdeServidor!=='function')return;
      var changed=!!last&&last!==login;try{localStorage.setItem(LAST_REMOTE_LOGIN_KEY,login);}catch(e){}
      Promise.resolve(window.sgrtCargarDesdeServidor({forceServer:changed,selectNit:currentNit()})).then(function(){
        normalizeRemoteResponses();var nit=currentNit(),contract=currentContract(nit);if(nit&&contract){activateAuthoritativeResponses(nit,contract);refreshEvaluatorProgress(nit,contract);}scheduleReportPostProcess(true);
      }).catch(function(){});
    },80);return r;
  };

  /* -------- Repositorio documental local con capacidad ampliada --------
     La estructura de carpetas sigue guardándose exactamente en od_sgrt_v8,
     pero el contenido pesado de cada archivo se mueve a IndexedDB. Así el
     árbol conserva su forma y localStorage deja de llenarse con Base64. */
  function repositoryTree(){
    var live=window._odFS;
    if(live&&typeof live==='object')return live;
    return parseRepo();
  }
  function repositoryCurrentFolder(){
    try{if(typeof window.odCF==='function')return window.odCF();}catch(e){}
    var cur=repositoryTree(),path=window._odPath||[];
    path.forEach(function(seg){cur=(cur.children||[]).find(function(c){return c.id===seg.id;})||cur;});
    return cur;
  }
  function adminCurrentFolder(fs){
    var cur=fs,path=window._adminOdPath||[];
    path.forEach(function(seg){cur=(cur.children||[]).find(function(c){return c.id===seg.id;})||cur;});
    return cur;
  }
  function metadataClone(node){
    if(!node||typeof node!=='object')return node;
    if(Array.isArray(node))return node.map(metadataClone);
    var out={};Object.keys(node).forEach(function(k){
      if((k==='dataURL'||k==='dataUrl'||k==='_dataURL')&&node._evidId)return;
      out[k]=k==='children'?metadataClone(node[k]||[]):node[k];
    });return out;
  }
  function saveRepositoryMetadata(fs){
    fs=fs||repositoryTree();
    try{localStorage.setItem('od_sgrt_v8',JSON.stringify(metadataClone(fs)));updateStorageLabels();return true;}
    catch(e){
      if(!storageRecoveryPending){
        storageRecoveryPending=true;
        Promise.resolve(compactInlineTree(fs,'od')).then(function(){
          try{localStorage.setItem('od_sgrt_v8',JSON.stringify(metadataClone(fs)));}catch(x){}
        }).then(compactLegacyStorage).finally(function(){storageRecoveryPending=false;});
      }
      console.warn('SGRT: metadatos del repositorio pendientes de compactación.',e);return false;
    }
  }
  function collectStoredFileIds(node,out){
    out=out||[];if(!node)return out;
    if(node._evidId)out.push(node._evidId);
    (node.children||[]).forEach(function(c){collectStoredFileIds(c,out);});return out;
  }
  async function addFilesToFolder(files,folder,prefix){
    files=Array.from(files||[]);if(!files.length||!folder)return {saved:0,rejected:0,failed:0};
    folder.children=folder.children||[];var saved=0,rejected=0,failed=0;
    for(var i=0;i<files.length;i++){
      var file=files[i];
      if(file.size>DOCUMENT_MAX_BYTES){rejected++;continue;}
      try{
        var data=await readFile(file),id=legacyFileId(prefix||'doc');
        await putFile(id,data);
        folder.children.push({id:'file_'+Date.now()+'_'+Math.random().toString(36).slice(2),name:file.name,type:'file',size:file.size,fecha:new Date().toLocaleDateString('es-CO'),mime:file.type||'',_evidId:id,_storage:'indexeddb'});
        saved++;
      }catch(e){failed++;console.warn('SGRT: no se pudo guardar el archivo '+file.name,e);}
    }
    return {saved:saved,rejected:rejected,failed:failed};
  }
  function uploadResultToast(r){
    var bits=[];if(r.saved)bits.push(r.saved+' archivo(s) guardado(s)');if(r.rejected)bits.push(r.rejected+' omitido(s) por superar 20 MB');if(r.failed)bits.push(r.failed+' no se pudieron guardar');
    if(!bits.length)return;try{window.showToast&&window.showToast((r.failed||r.rejected&&!r.saved?'⚠️ ':'✅ ')+bits.join(' · '),r.saved?'success':'warning',3500);}catch(e){}
  }
  function installDocumentStorageBridge(){
    if(window._sgrt39DocumentBridge)return;window._sgrt39DocumentBridge=true;
    var oldSave=window.odSave,oldUpload=window.odSubirArchivos,oldAdminUpload=window.adminOdSubir,oldDelete=window.odDel,oldDeleteSel=window.odDelSel,oldDlSel=window.odDlSel;

    window.odSave=function(){
      if(sharePointConnected()&&typeof oldSave==='function')return oldSave.apply(this,arguments);
      var fs=repositoryTree();window._odFS=fs;return saveRepositoryMetadata(fs);
    };

    window.odSubirArchivos=async function(files){
      if(sharePointConnected()&&typeof oldUpload==='function')return oldUpload.apply(this,arguments);
      if(!files||!files.length)return;
      var folder=repositoryCurrentFolder(),result=await addFilesToFolder(files,folder,'doc');
      saveRepositoryMetadata(repositoryTree());try{window.odRender&&window.odRender();}catch(e){}uploadResultToast(result);
    };

    window.adminOdSubir=async function(files){
      if(sharePointConnected()&&typeof oldAdminUpload==='function')return oldAdminUpload.apply(this,arguments);
      if(!files||!files.length)return;
      var fs=parseRepo(),folder=adminCurrentFolder(fs),result=await addFilesToFolder(files,folder,'adoc');
      window._odFS=fs;saveRepositoryMetadata(fs);try{window.adminOdInit&&window.adminOdInit();}catch(e){}uploadResultToast(result);
    };

    window.odDel=function(id){
      if(sharePointConnected()&&typeof oldDelete==='function')return oldDelete.apply(this,arguments);
      var item=findRepoItem(repositoryTree(),id);collectStoredFileIds(item,[]).forEach(function(fid){deleteFile(fid);});
      return typeof oldDelete==='function'?oldDelete.apply(this,arguments):undefined;
    };

    window.odDelSel=function(){
      if(sharePointConnected()&&typeof oldDeleteSel==='function')return oldDeleteSel.apply(this,arguments);
      var ids=window._odSel||new Set(),folder=repositoryCurrentFolder();(folder.children||[]).forEach(function(item){if(ids.has(item.id))collectStoredFileIds(item,[]).forEach(function(fid){deleteFile(fid);});});
      return typeof oldDeleteSel==='function'?oldDeleteSel.apply(this,arguments):undefined;
    };

    window.odDlSel=async function(){
      if(sharePointConnected()&&typeof oldDlSel==='function')return oldDlSel.apply(this,arguments);
      var ids=window._odSel||new Set(),files=(repositoryCurrentFolder().children||[]).filter(function(it){return ids.has(it.id)&&it.type==='file';});
      if(!files.length){try{window.showToast&&window.showToast('Selecciona archivos primero','warning',2000);}catch(e){}return;}
      for(var i=0;i<files.length;i++){
        var fi=files[i];if(fi._evidId)await downloadEvidence(fi._evidId,fi.name,fi.dataURL||fi.dataUrl||'');else if(fi.dataURL||fi.dataUrl)await downloadEvidence(fi.id,fi.name,fi.dataURL||fi.dataUrl);
      }
    };
  }

  window._sgrt39Test={responseWeight:responseWeight,bestContractResponses:bestContractResponses,normalizeRemoteResponses:normalizeRemoteResponses,activateAuthoritativeResponses:activateAuthoritativeResponses,syncOneThird:syncOneThird,compactLegacyStorage:compactLegacyStorage};

  function init(){
    applyTextCorrections();hydrateRiskEvidence();installReportStorageBridge();installDocumentStorageBridge();
    try{if(navigator.storage&&navigator.storage.persist)navigator.storage.persist().catch(function(){});}catch(e){}
    Promise.resolve().then(compactLegacyStorage).then(migrateInlineFiles).then(importRemoteEvidence).then(function(){rebuildEvidenceRepository();updateStorageLabels();});
    var css=document.getElementById('sgrt39-style');if(!css){css=document.createElement('style');css.id='sgrt39-style';css.textContent='#q-secciones-wrap textarea[id^="obs_"],#q-secciones-wrap textarea[id^="qo_"],#q-secciones-wrap textarea[id$="_obs"]{min-height:125px!important;resize:vertical!important;line-height:1.5!important;padding:9px 10px!important;font-size:12px!important;box-sizing:border-box!important;}';document.head.appendChild(css);}
