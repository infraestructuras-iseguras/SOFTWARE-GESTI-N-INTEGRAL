...
// Sincronización ligera: los eventos locales son inmediatos; sondeo de respaldo cada 60 s
...
function iniciarSincronizacionTiempoReal() {
  ...
  SYNC_INTERVAL=setInterval(()=>{
    ...
    cargarNotificacionesSistema();cargarReportesSistema();actualizarPanelNotificaciones();
  },60000);
}
  ...
  if(USERS[u] && USERS[u].pass === p){
    ...
    if(isAdminRole){
      ...
      if(appDiv){
        ...
        if(currentUser.rol === 'Cliente' || currentUser.rol === 'evaluador') setTimeout(function(){
          try{applyRoleRestrictions();}catch(e){}
          try{window.clsInitDash&&window.clsInitDash();window.clsRender&&window.clsRender();}catch(e){}
        },90);
      }
      try{applyRoleRestrictions();}catch(e){}
...
(function syncTercerosFromAPI(){
  if(document.hidden)return;
  const apiUrl = API_BASE + '/api/terceros';
  fetch(apiUrl, {headers: {'Content-Type': 'application/json'}})
    .then(r=>{if(!r.ok) throw new Error('API error');return r.json();})
    .then(data=>{
      if(data && data.data && Array.isArray(data.data)){
        // Actualizar TERCEROS_DB globalmente con datos de la BD
        ...
        data.data.forEach(t=>{
          TERCEROS_DB[t.nit] = {
            ...
            nombre: t.nombre,
            entidad: t.entidad,
            entidadLabel: t.entidadLabel,
            servicio: t.servicio || '',
            supervisor: t.supervisor || '',
            prom: parseFloat(t.prom || 0),
            zona: t.zona || 'BAJO',
            periodicidad: t.periodicidad || 'Sin evaluación',
        ...
        console.log('✅ Sincronización exitosa: '+Object.keys(TERCEROS_DB).length+' terceros desde Azure SQL');
      }
    })
    .catch(e=>{
      console.warn('⚠️ Sync: No se pudo conectar a Azure SQL; se conserva el respaldo local', e.message);
      // Fallback seguro: recuperar la copia local existente; nunca borrarla por una falla de red.
      try{
        var localCache = JSON.parse(localStorage.getItem('sgrt_terceros_db_shared')||'{}');
        if(typeof TERCEROS_DB !== 'object' || !TERCEROS_DB) TERCEROS_DB = {};
        Object.keys(localCache||{}).forEach(function(nit){
          if(localCache[nit] && !TERCEROS_DB[nit]) TERCEROS_DB[nit] = localCache[nit];
        });
        console.log('📦 Respaldo local conservado: '+Object.keys(TERCEROS_DB||{}).length+' terceros');
      }catch(cacheErr){
        if(typeof TERCEROS_DB !== 'object' || !TERCEROS_DB) TERCEROS_DB = {};
        console.warn('⚠️ No fue posible leer el respaldo local', cacheErr.message);
      }
    });
  // Re-sincronizar como respaldo cada 3 minutos; otros módulos ya sincronizan cambios puntuales.
  setInterval(syncTercerosFromAPI, 180000);
})();
...
async function cargarTercerosDesdeAPI(opts){
  opts = opts || {};
  const tbody = document.getElementById('tbody-terceros');
  if(!tbody) return;

  // El spinner se muestra solo en una carga iniciada por navegación. En el
  // refresco automático se conserva la tabla visible para evitar parpadeos.
  if(!opts.silent){
  ...
  let apiOk = false;
  try {
    const resp = await fetch((typeof API_BASE!=='undefined'?API_BASE:'http://localhost:3000')+'/api/terceros', {cache:'no-store'});
    if(resp.ok){
      const data = await resp.json();
      if(data.ok && Array.isArray(data.data)){
        apiOk = true;
        rows = data.data.slice();
      }
    }
  ...
  const nitsApi = new Set(rows.map(function(r){return String(r.NIT||r.nit||'').trim();}).filter(Boolean));

  // Si el servidor respondió correctamente, él es la fuente de verdad. Solo se
  // conserva temporalmente un registro local que todavía esté pendiente de
  // sincronización. Así un registro borrado en BD no reaparece desde caché.
  dbRows.forEach(function(t){
    var tn = String((t&&t.nit)||'').trim();
    if(!tn || nitsApi.has(tn)) return;
    var pendiente = t && t._changed === true && t.sincronizado !== true;
    if(apiOk && !pendiente) return;
    rows.push({
      ID_RelacionTerceros:'—', NIT:tn, NombreTercero:t.nombre,
      ServicioContratado:t.servicio_contratado||t.servicio, _supervisor:t.supervisor||(t.supervisores&&t.supervisores[0]&&t.supervisores[0].nombre)||'',
  ...
  tbody.innerHTML = rows.map(r=>{
    const nit      = r.NIT||r.nit||'—';
    var _estadoCompleto = (typeof TERCEROS_DB!=='undefined' && TERCEROS_DB[String(nit)]) ? TERCEROS_DB[String(nit)] : null;
    const nombre   = (_estadoCompleto&&_estadoCompleto.nombre)||r.NombreTercero||r.Nombre_Tercero||r.nombre||'—';
    const servicio = (_estadoCompleto&&(_estadoCompleto.servicio_contratado||_estadoCompleto.servicio))||r.ServicioContratado||r.Servicio_Contratado||r.servicio||'—';
    const super_   = (_estadoCompleto&&(_estadoCompleto.supervisor||(_estadoCompleto.supervisores&&_estadoCompleto.supervisores[0]&&_estadoCompleto.supervisores[0].nombre)))||r._supervisor||r.SupervisorNombre||'—';
    const prom     = parseFloat((_estadoCompleto&&_estadoCompleto.prom)!=null?_estadoCompleto.prom:(r.PromedioCriticidad||r.prom||0));
    const zona     = (_estadoCompleto&&_estadoCompleto.zona)||r.Zona_Riesgo||r.zona||'—';
    const per      = (_estadoCompleto&&_estadoCompleto.periodicidad)||r.Periodicidad||r.periodicidad||(prom>=3?'Se evalúa':'No se evalúa');
    ...
    if(typeof TERCEROS_DB!=='undefined' && !TERCEROS_DB[nit]){
      ...
      TERCEROS_DB[nit]={nit,nombre,entidad:entRaw,entidadLabel:eLabel,
        servicio,supervisor:super_,prom,zona,periodicidad:per,estado:activo?'Activo':'Inactivo',
        dims:savedT.dims||[],contratos:savedT.contratos||[],supervisores:savedT.supervisores||[],
        sincronizado:true,_changed:false};
...
async function saveClasifForm(){
  ...
  // Registro inicial: NO inventar tipologías. Se califican después, por contrato.

  // ── Guardar en TERCEROS_DB ─────────────────────────
  // ✅ INCLUIR CONTRATOS CON SUPERVISORES DESDE EL BUFFER
  var contratosDelBuffer = (window._cfContratosBuffer || []).filter(c => c.num && c.num.trim());
  ...
  */
  ...
  var contratosGuardados = (window._cfContratosBuffer || []).filter(function(c){ return c.num && c.num.trim(); });
...
function showToast(msg, type='info', duration=3500){
  ...
  if(!wrap) return;
  // Compatibilidad: varios módulos usan "warning" y el CSS histórico usa "warn".
  // Sin esta normalización el toast conserva texto blanco, pero no recibe fondo y parece vacío.
  type = String(type||'info').toLowerCase();
  if(type==='warning') type='warn';
  const t = document.createElement('div');
  t.className = 'toast t-'+type;
  // Fallback visual inline: aunque el navegador conserve un CSS viejo en caché,
  // la alerta nunca volverá a verse blanca sobre fondo blanco.
  const fondosToast={success:'#198754',error:'#dc3545',info:'#0d6efd',warn:'#f59e0b'};
  t.style.background=fondosToast[type]||fondosToast.info;
  t.style.color='#fff';
  const texto = (msg===undefined || msg===null || String(msg).trim()==='') ? 'Operación realizada.' : String(msg);
  t.textContent = texto;
  wrap.appendChild(t);
...
const CUEST_DATA = {
  op: [
    ...
    {id:'op6',ctrl:'Protocolo de comunicación por eventos de riesgo',q:'¿Se cuenta con protocolo de comunicación con Colpensiones ante la identificación de eventos de riesgo?',doc:'Protocolo de comunicación; canales definidos; divulgación'},
  ...
  si: [
    ...
    {id:'si23',ctrl:'Restricción de acceso a áreas de procesamiento de información',q:'¿El acceso a instalaciones de procesamiento de información está restringido físicamente?',doc:'Listado instalaciones de procesamiento'},
    ...
    {id:'si29',ctrl:'Seguridad de la red – Firewall',q:'¿Se cuenta con firewalls en todas las conexiones con redes externas o DMZ?',doc:'Diagrama topología red con firewalls; directrices configuración'},
    {id:'si30',ctrl:'Seguridad de la red – pruebas de penetración',q:'¿Se realizan periódicamente pruebas de penetración sobre la infraestructura tecnológica?',doc:'Informes evaluación intrusión; cronograma pruebas'},
...
const CUESTIONARIO_CONTROLES = {
  "op": [
    ...
    {
      "n": 6,
      "ctrl": "Protocolo de comunicación por eventos de riesgo",
  ...
  "si": [
    ...
    {
      "n": 23,
      "ctrl": "Restricción de acceso a áreas de procesamiento de información",
    ...
    {
      "n": 29,
      "ctrl": "Seguridad de la red – Firewall",
    ...
    {
      "n": 30,
      "ctrl": "Seguridad de la red – pruebas de penetración",
    ...
    {
      "n": 45,
      "ctrl": "Gestión de vulnerabilidades – parches de seguridad",
...
function cargarCuestionarioTercero(){
  ...
  if(nit && typeof API_BASE !== 'undefined' && API_BASE && !window._sgrtCargandoRemotoCuestionario){
    // IMPORTANTE: el maestro dbo.Terceros NO contiene el estado completo del flujo.
    // Antes se reemplazaba TERCEROS_DB[nit] por una versión reducida del maestro y se
    // perdían contratos, aprobadoPorContrato y dimsPorContrato justo al entrar a AC.
    // Ahora se MEZCLA maestro + SGRT_Tercero_Estado + estado local sin borrar campos.
    var _baseApi=String(API_BASE).replace(/\/$/,'');
    var _masterReq=fetch(_baseApi + '/api/terceros/' + encodeURIComponent(nit), {cache:'no-store'})
      .then(function(r){return r.json();}).catch(function(){return null;});
    var _stateReq=fetch(_baseApi + '/api/sgrt-state/' + encodeURIComponent(nit), {cache:'no-store'})
      .then(function(r){return r.json();}).catch(function(){return null;});
    Promise.all([_masterReq,_stateReq]).then(function(rs){
      var m=rs[0]&&rs[0].ok&&rs[0].data?rs[0].data:{};
      var sx=rs[1]&&rs[1].ok&&rs[1].data?rs[1].data:{};
      // GET /api/sgrt-state/:nit devuelve data=<payload> directamente.
      // También toleramos la forma {estado_sgrt:<payload>} por compatibilidad.
      var remoto=(sx.estado_sgrt&&typeof sx.estado_sgrt==='object')?sx.estado_sgrt:
        ((sx&&typeof sx==='object')?sx:{});
      var local=(window.TERCEROS_DB&&window.TERCEROS_DB[nit])||{};
      var maestro={};
      if(m && Object.keys(m).length){
        maestro.nit=nit;
        maestro.nombre=m.nombre||m.Nombre||m.Nombre_Tercero||local.nombre||nit;
        maestro.domicilio=m.domicilio||m.Domicilio||local.domicilio||'';
        maestro.servicio=m.servicio||m.servicio_contratado||m.Servicio_Contratado||local.servicio||'';
        maestro.servicio_contratado=maestro.servicio;
      }
      // Si hay cambios locales pendientes (por ejemplo, se acaba de aprobar), gana local.
      // En otro navegador/rol, donde no hay cambio pendiente, gana el estado extendido remoto.
      var merged=local&&local._changed
        ? Object.assign({},maestro,remoto,local)
        : Object.assign({},local,maestro,remoto);
      merged.nit=nit;
      merged.nombre=merged.nombre||maestro.nombre||nit;
      if(!Array.isArray(merged.contratos)) merged.contratos=[];
      if(!merged.dimsPorContrato||typeof merged.dimsPorContrato!=='object') merged.dimsPorContrato={};
      if(!merged.promPorContrato||typeof merged.promPorContrato!=='object') merged.promPorContrato={};
      if(!merged.aprobadoPorContrato||typeof merged.aprobadoPorContrato!=='object') merged.aprobadoPorContrato={};
      merged.contratos.forEach(function(c){
        if(!c)return;
        var num=String(c.num||c.numero||'').trim();
        if(num && (c.estado_aprobacion==='APROBADO'||c.estado==='Aprobado'||c.aprobado===true)) merged.aprobadoPorContrato[num]=true;
      });
      window.TERCEROS_DB=window.TERCEROS_DB||{};
      window.TERCEROS_DB[nit]=merged;
      try{ localStorage.setItem('sgrt_terceros_db_shared',JSON.stringify(window.TERCEROS_DB)); }catch(e){}
      console.log('[SGRT] ✅ Estado completo preservado para Ambiente de Control:', nit, merged);

      // Volver a renderizar una sola vez con el estado ya hidratado, sin disparar
      // otra consulta remota recursiva.
      if(((document.getElementById('q-tercero')||{}).value||'')===nit){
        window._sgrtCargandoRemotoCuestionario=true;
        try{ cargarCuestionarioTercero(); }
        finally{ window._sgrtCargandoRemotoCuestionario=false; }
        try{ window.acPoblarSelectorTerceroInstruc && window.acPoblarSelectorTerceroInstruc(); }catch(e){}
        try{ window.acPoblarContratos && window.acPoblarContratos(nit); }catch(e){}
        try{ window.qPoblarContratos && window.qPoblarContratos(nit); }catch(e){}
        try{ window.poblarSelectorACTipologia && window.poblarSelectorACTipologia(); }catch(e){}
      }
    }).catch(function(e){
      console.warn('[SGRT] ⚠️ No se pudo hidratar estado remoto; se conserva el local:', e&&e.message?e.message:e);
    });
  }
...
function registrarEvidenciaCuest(nitKey, key, ctrlN, inputEl){
  ...
  files.forEach(function(f){
    ...
    r.onload=function(e){
      ...
      // ── Guardar evidencia en Documentación y Evidencias / Ambiente de Control ──
      // Hotfix 41 la asocia al tercero + contrato y la sincroniza para que
      // el Administrador de Riesgos pueda verla desde otro navegador/rol.
      try{
        if(window._sgrt41RegistrarEvidenciaAC){
          window._sgrt41RegistrarEvidenciaAC(f, e.target.result, nitKey, key, ctrlN);
        }
      }catch(exRepo){ console.warn('No se pudo registrar evidencia AC compartida:', exRepo); }
    };
    r.readAsDataURL(f);
  });
  inputEl.value='';
...
function _nivelRiesgoValor(v){
  if(v===null || v===undefined || v==='') return 0;
  var txt=String(v).trim().toUpperCase();
  var porNombre={
    'MUY BAJA':0.2,'BAJA':0.4,'MEDIA':0.6,'ALTA':0.8,'MUY ALTA':1,
    'LEVE':0.2,'MENOR':0.4,'MODERADO':0.6,'MAYOR':0.8,'CATASTRÓFICO':1,'CATASTROFICO':1
  };
  if(Object.prototype.hasOwnProperty.call(porNombre,txt)) return porNombre[txt];
  var n=parseFloat(v); if(!isFinite(n)||n<=0) return 0;
  // Excel agrupa el residual por intervalos: <=20%, <=40%, <=60%, <=80%, <=100%.
  if(n<=0.2) return 0.2;
  if(n<=0.4) return 0.4;
  if(n<=0.6) return 0.6;
  if(n<=0.8) return 0.8;
...
function _labelProbRiesgo(v){
  return ({'0.2':'Muy Baja','0.4':'Baja','0.6':'Media','0.8':'Alta','1':'Muy Alta'})[String(_nivelRiesgoValor(v))]||'—';
}
function _labelImpactoRiesgo(v){
  return ({'0.2':'Leve','0.4':'Menor','0.6':'Moderado','0.8':'Mayor','1':'Catastrófico'})[String(_nivelRiesgoValor(v))]||'—';
}
function zonaFromProb(p, imp){
  var pv=_nivelRiesgoValor(p), iv=_nivelRiesgoValor(imp);
  if(!pv || !iv) return '—';
  // Matriz 5×5 copiada de la hoja Parámetros del Excel (Probabilidad × Impacto).
  var matriz={
    '0.2':{'0.2':'BAJO','0.4':'BAJO','0.6':'MODERADO','0.8':'ALTO','1':'EXTREMO'},
    '0.4':{'0.2':'BAJO','0.4':'MODERADO','0.6':'MODERADO','0.8':'ALTO','1':'EXTREMO'},
    '0.6':{'0.2':'MODERADO','0.4':'MODERADO','0.6':'MODERADO','0.8':'ALTO','1':'EXTREMO'},
    '0.8':{'0.2':'MODERADO','0.4':'MODERADO','0.6':'ALTO','0.8':'ALTO','1':'EXTREMO'},
    '1':  {'0.2':'ALTO','0.4':'ALTO','0.6':'ALTO','0.8':'ALTO','1':'EXTREMO'}
  };
  return (matriz[String(pv)]||{})[String(iv)]||'—';
}
...
function calcZonaMatriz(){
  // Probabilidad se muestra en DIV y se conserva numéricamente en inputs hidden.
  var pInh = (document.getElementById('nr-prob-inh-val')||{}).value || '';
  var iInh = (document.getElementById('nr-imp-inh')||{}).value || '';
  var pRes = (document.getElementById('nr-prob-res-val')||{}).value || '';
  var iRes = (document.getElementById('nr-imp-res')||{}).value || '';

  function setZona(elId,p,i){
    var el=document.getElementById(elId); if(!el)return;
    if(!p||!i){el.textContent='—';el.style.background='#F3F4F6';el.style.color='var(--muted)';return;}
    var z=zonaFromProb(p,i),zc=ZONA_COLOR[z]||ZONA_COLOR['BAJO'];
    el.textContent=z;el.style.background=zc.bg;el.style.color=zc.text;
  }
  setZona('nr-zona-inh',pInh,iInh);
  setZona('nr-zona-res',pRes,iRes);
}

// ── Frecuencia / criterio → Probabilidad Inherente (Excel) ─────────
function calcProbInherenteAuto(){
  var frecuencia=document.getElementById('nr-frecuencia');
  var criterio=document.getElementById('nr-criterio-prob');
  var tipo=(document.getElementById('nr-tipo-impl-val')||{}).value||'';
  var raw='';
  // En modalidad "Por Frecuencia", el criterio del Excel tiene prioridad.
  if(tipo==='frecuencia') raw=(criterio&&criterio.value)||'';
  else if(frecuencia) raw=frecuencia.value;

  var n=parseFloat(raw||0), valor=0;
  if(n){
    // El Excel trabaja únicamente con cinco niveles (20/40/60/80/100%).
    // Los valores intermedios históricos del selector se normalizan a ese esquema.
    if(n>=0.95) valor=1;
    else if(n>=0.8) valor=0.8;
    else if(n>=0.6) valor=0.6;
    else if(n>=0.4) valor=0.4;
    else valor=0.2;
  }
  var div=document.getElementById('nr-prob-inh'),hidden=document.getElementById('nr-prob-inh-val');
  if(hidden)hidden.value=valor?String(valor):'';
  if(div){div.textContent=valor?_labelProbRiesgo(valor):'—';div.style.color=valor?'#1a3a5c':'var(--muted)';}
  try{window.calcEfectividadAuto&&window.calcEfectividadAuto();}catch(e){}
  calcZonaMatriz();
}

// ── Impacto Económico/Reputacional → Impacto Inherente (máximo, Excel) ─
function calcImpactoInherenteAuto(){
  var ce=document.getElementById('nr-criterio-imp-econ'),cr=document.getElementById('nr-criterio-imp-reput');
  var de=document.getElementById('nr-imp-econ'),dr=document.getElementById('nr-imp-reput');
  var tipo=(document.getElementById('nr-tipo-impl-val')||{}).value||'';
  var e=parseFloat(tipo==='frecuencia'?((ce&&ce.value)||0):((de&&de.value)||0));
  var r=parseFloat(tipo==='frecuencia'?((cr&&cr.value)||0):((dr&&dr.value)||0));
  var max=Math.max(e||0,r||0);
  var hidden=document.getElementById('nr-imp-inh'),div=document.getElementById('nr-imp-inh-calc');
  if(hidden)hidden.value=max?String(_nivelRiesgoValor(max)):'';
  if(div){div.textContent=max?_labelImpactoRiesgo(max):'—';div.style.color=max?'#1a3a5c':'var(--muted)';}
  try{window.calcEfectividadAuto&&window.calcEfectividadAuto();}catch(e){}
  calcZonaMatriz();
}
...
// Cada control se aplica secuencialmente, tal como en las filas AE/AG del Excel.
window.calcEfectividadAuto = function(){
  var arr=window._nrCtrlsBuffer||[];
  arr.forEach(function(c,idx){window.calcEfectividadControl(idx);});

  var pInh=parseFloat((document.getElementById('nr-prob-inh-val')||{}).value||0);
  var iInh=parseFloat((document.getElementById('nr-imp-inh')||{}).value||0);
  // Una fila de control vacía no debe reducir el riesgo por usar valores por defecto del formulario.
  var activos=arr.filter(function(c){return String(c.desc||'').trim()!=='';});

  var prom=activos.length?activos.reduce(function(a,c){return a+(parseFloat(c.efec)||0);},0)/activos.length:0;
  var elE=document.getElementById('nr-efec-prom');
  if(elE){elE.textContent=activos.length?Math.round(prom*100)+' %':'— %';
    elE.style.background=prom>=0.7?'#dcfce7':prom>=0.4?'#fef3c7':'#F3F4F6';
    elE.style.color=prom>=0.7?'#166534':prom>=0.4?'#92400e':'var(--muted)';}

  // Excel: cada control se aplica sobre el residual dejado por el control anterior.
  // PREVENTIVO/DETECTIVO reduce probabilidad; CORRECTIVO reduce impacto.
  var probRaw=pInh,impRaw=iInh;
  activos.forEach(function(c){
    var ef=Math.max(0,Math.min(1,parseFloat(c.efec)||0));
    if((c.tipo==='PREVENTIVO'||c.tipo==='DETECTIVO') && probRaw>0) probRaw=Math.max(probRaw-(probRaw*ef),0);
    if(c.tipo==='CORRECTIVO' && impRaw>0) impRaw=Math.max(impRaw-(impRaw*ef),0);
  });

  // El Excel convierte el valor residual continuo al nivel inmediatamente superior de la escala.
  var pNivel=pInh?_nivelRiesgoValor(probRaw):0;
  var iNivel=iInh?_nivelRiesgoValor(impRaw):0;
  var pDiv=document.getElementById('nr-prob-res'),pHidden=document.getElementById('nr-prob-res-val');
  if(pHidden)pHidden.value=pNivel?String(pNivel):'';
  if(pDiv){pDiv.textContent=pNivel?_labelProbRiesgo(pNivel):'—';pDiv.style.color=pNivel?'#1a3a5c':'var(--muted)';}
  var iSel=document.getElementById('nr-imp-res');if(iSel)iSel.value=iNivel?String(iNivel):'';

  // Guardar valores continuos para trazabilidad sin cambiar la interfaz.
  window._nrResidualRaw={probabilidad:probRaw||0,impacto:impRaw||0};
  calcZonaMatriz();
...
function abrirNuevoRiesgo(){
  ...
  ['nr-ref','nr-tercero','nr-tipo','nr-factor','nr-clasificacion','nr-desc','nr-causa',
   ...
   'nr-contrato','nr-frecuencia','nr-tipo-impl-val','nr-criterio-prob','nr-criterio-imp-econ','nr-criterio-imp-reput','nr-imp-reput','nr-imp-econ'].forEach(id=>{
    const el = document.getElementById(id);
    if(el){
      if(id === 'nr-prob-inh' || id === 'nr-prob-res'){
        el.textContent='—';
  ...
  inp = document.getElementById('nr-imp-inh'); if(inp) inp.value='';
  inp = document.getElementById('nr-prob-res-val'); if(inp) inp.value='';
  var impCalc=document.getElementById('nr-imp-inh-calc'); if(impCalc) impCalc.textContent='—';
  ...
  window._nrCtrlsBuffer = [{desc:'',tipo:'PREVENTIVO',automatizacion:'AUTOMÁTICO',documentacion:'DOCUMENTADO',frecuencia:'CONTINUA',registro:'CON REGISTRO',responsable:'',efec:0.5}];
  window.nrCtrlRender && window.nrCtrlRender();
...
function editarRiesgo(id){
  ...
  const set = (elId,val) => {
    var el=document.getElementById(elId);if(!el)return;
    if(elId==='nr-prob-inh'){
      var v=_nivelRiesgoValor(val),h=document.getElementById('nr-prob-inh-val');if(h)h.value=v?String(v):'';
      el.textContent=v?_labelProbRiesgo(v):'—';
    }else if(elId==='nr-prob-res'){
      var vr=_nivelRiesgoValor(val),hr=document.getElementById('nr-prob-res-val');if(hr)hr.value=vr?String(vr):'';
      el.textContent=vr?_labelProbRiesgo(vr):'—';
    }else if(elId==='nr-imp-inh'){
      var vi=_nivelRiesgoValor(val);el.value=vi?String(vi):'';
      var d=document.getElementById('nr-imp-inh-calc');if(d)d.textContent=vi?_labelImpactoRiesgo(vi):'—';
    }else el.value=val||'';
  };
  ...
  set('nr-tipo-impl-val',r.tipoImpl||'');
  set('nr-criterio-prob',r.criterioProb||'');
  set('nr-criterio-imp-econ',r.criterioImpEcon||'');
  set('nr-criterio-imp-reput',r.criterioImpReput||'');
...
function guardarRiesgo(){
  const get = id => {
    if(id==='nr-prob-inh') return ((document.getElementById('nr-prob-inh-val')||{}).value||'').trim();
    if(id==='nr-prob-res') return ((document.getElementById('nr-prob-res-val')||{}).value||'').trim();
    return ((document.getElementById(id)||{}).value||'').trim();
  };
  try{calcProbInherenteAuto();calcImpactoInherenteAuto();window.calcEfectividadAuto&&window.calcEfectividadAuto();}catch(e){}
  ...
  const riesgo = {
    ...
    frecuencia: get('nr-frecuencia'), tipoImpl:get('nr-tipo-impl-val'),
    criterioProb:get('nr-criterio-prob'), criterioImpEcon:get('nr-criterio-imp-econ'), criterioImpReput:get('nr-criterio-imp-reput'),
    ...
    probResRaw: (window._nrResidualRaw||{}).probabilidad||0, impResRaw: (window._nrResidualRaw||{}).impacto||0,
...
function _calcCtrlValoracion(resp){
  ...
  // ── Pesos por atributo para el cuestionario de 6 campos ──
  // Se elimina la dependencia del séptimo atributo del formato anterior.
  // Los seis pesos suman exactamente 1.00, por lo que 6 respuestas "Sí" = 100%.
  // 1 Implementado 15% · 2 Documentado 15% · 3 Asignado 15%
  // 4 Divulgado 15% · 5 Evidencia 20% · 6 Monitoreado 20%
  var w1 = resp.a1==='Si'?0.15:(resp.a1==='Parcial'?0.05:0);
  var w2 = resp.a2==='Si'?0.15:0;
  var w3 = resp.a3==='Si'?0.15:0;
  var w4 = resp.a4==='Si'?0.15:0;
  var w5 = resp.a5==='Si'?0.20:0;
  var w6 = resp.a6==='Si'?0.20:0;
  var cumpl = Math.round((w1+w2+w3+w4+w5+w6)*100)/100; // 6 síes → 1.00
...
function renderReportesAC(){
  ...
  if(window._sgrt42Active){
    // El consolidado final calcula progreso/madurez por contrato. Aquí solo se necesita
    // saber si existen terceros aprobados para conservar la tabla de contratos.
    Object.keys(TERCEROS_DB||{}).forEach(function(nit){
      var t=TERCEROS_DB[nit];if(!t||t.bloqueado)return;
      var estado=String(t.estado||'Sin iniciar');
      if(estado==='Completado'||estado.toLowerCase().indexOf('aprob')>=0)totalTerceros++;
    });
  }else{
    Object.keys(TERCEROS_DB||{}).forEach(function(nit){
  ...
  html+='<div id="rpt-tabla-wrap" class="card"><div class="card-hdr">'
    ...
    // El filtro de tercero/contrato se gestiona en el resumen consolidado superior.
    +'</div></div>'
    ...
    +'</tr></thead><tbody>';
  var rowCount=0;
  // Rendimiento: el detalle completo solo se materializa para el tercero elegido en el filtro superior.
  // Así no se crean cientos/miles de filas ocultas cada vez que se abre Ambiente de Control.
  var _rptNits = filTercero ? [filTercero] : [];
  _rptNits.forEach(function(nit){
    if(!nit || nit.startsWith('__')) return;
    var filContrato = window._rptFiltroContrato || '';
    var t=TERCEROS_DB[nit]; if(!t) return;
    var _respSet = (filContrato && t.respuestasACPorContrato && t.respuestasACPorContrato[filContrato])
      ? t.respuestasACPorContrato[filContrato]
      : (t.respuestasAC || CUEST_RESPUESTAS[nit] || {});
    var _dimsSet = (filContrato && t.dimsPorContrato && Array.isArray(t.dimsPorContrato[filContrato]) && t.dimsPorContrato[filContrato].length)
      ? t.dimsPorContrato[filContrato] : (t.dims||[]);
    _dimsSet.forEach(function(d){
      if(filTip&&d.key!==filTip) return;
      var ctrls=(typeof window._ctrlsCuest==='function' ? (window._ctrlsCuest(nit,d.key,filContrato)||[]) : (CUESTIONARIO_CONTROLES[d.key]||[]));
      ctrls.forEach(function(c,ci){
        var resp=(_respSet[d.key]&&_respSet[d.key][c.n])||{};
        var val=_calcCtrlValoracion(resp);
        ...
        html+='<tr style="background:'+bg+';border-bottom:1px solid #F3F4F6;">'
          ...
          +'<td style="padding:6px 8px;font-size:11px;vertical-align:top;">'+(function(){
            var cons = t.contratos || [];
            var _shownContrato = filContrato || ((t.modoEval==='contrato' && t.contratoEval)?t.contratoEval:'');
            if(_shownContrato){
              var idx = cons.findIndex(function(c){ return String(c.num||c.numero||'')===String(_shownContrato); });
              var etq = idx>=0 ? 'Contrato '+(idx+1) : _shownContrato;
              return '<span style="display:inline-block;padding:3px 8px;border-radius:6px;font-weight:700;background:#fef3c7;color:#78350f;border:1px solid #fde68a;white-space:nowrap;" title="'+_shownContrato+'">'+etq+' ('+_shownContrato+')</span>';
            }
            if(cons.length){
          ...
          +(function(){
            ...
            var oIdx = window._obsDetalleReg.push({
              ...
              contrato: (function(){
                var _obsContrato = filContrato || ((t.modoEval==='contrato' && t.contratoEval)?t.contratoEval:'');
                if(_obsContrato){
                  var cons = t.contratos || [];
                  var idx = cons.findIndex(function(x){ return String(x.num||x.numero||'')===String(_obsContrato); });
                  return (idx>=0 ? 'Contrato '+(idx+1)+' — ' : '')+'('+_obsContrato+')';
                }
                var cons = t.contratos || [];
  ...
  if(!rowCount) html+='<tr><td colspan="14" style="text-align:center;padding:24px;color:var(--muted);">'+(filTercero?'No hay respuestas registradas para este filtro.':'Selecciona un tercero en el filtro superior para cargar el detalle de valoración.')+'</td></tr>';
...
// 🔄 AUTOGUARDADO GLOBAL OPTIMIZADO
var _SGRT_AUTOSAVE_CACHE={};
function _guardarSiCambio(key,obj,force){
  if(!obj || (typeof obj==='object' && Object.keys(obj).length===0))return;
  var txt=JSON.stringify(obj);
  if(force || _SGRT_AUTOSAVE_CACHE[key]!==txt){
    if(force || localStorage.getItem(key)!==txt)localStorage.setItem(key,txt);
    _SGRT_AUTOSAVE_CACHE[key]=txt;
  }
}
function autoguardarTodo(force) {
  if(document.hidden && !force)return;
  try{
    _guardarSiCambio('sgrt_terceros_db_shared',window.TERCEROS_DB,force);
    _guardarSiCambio('sgrt_cls_db_auto',window.CLS_DB,force);
    _guardarSiCambio('sgrt_reportes_fases_auto',window.REPORTES_POR_FASE,force);
  }catch(e){console.warn('⚠️ Error en autoguardado:',e);}
}
// Autoguardado de respaldo. Los cambios de formularios se persisten por evento/debounce.
window._AUTOSAVE_INTERVALO=setInterval(function(){autoguardarTodo(false);},60000);
window.addEventListener('beforeunload',function(){try{autoguardarTodo(true);}catch(e){}});
document.addEventListener('visibilitychange',function(){if(document.hidden){try{autoguardarTodo(true);}catch(e){}}});

// 🔄 SINCRONIZACIÓN GLOBAL ENTRE ROLES
window.SINCRONIZACION_ACTIVA=true;
var _SGRT_LAST_SYNC_TEXT='';
function sincronizarTercerosGlobal() {
  if(!window.SINCRONIZACION_ACTIVA || document.hidden)return;
  try{
    var txt=localStorage.getItem('sgrt_terceros_db_shared')||'{}';
    if(txt!==_SGRT_LAST_SYNC_TEXT){
      var localDB=JSON.parse(txt||'{}');
      if(Object.keys(localDB).length>0){if(!window.TERCEROS_DB)window.TERCEROS_DB={};Object.assign(window.TERCEROS_DB,localDB);}
      _SGRT_LAST_SYNC_TEXT=txt;
    }
    if(window.TERCEROS_DB && Object.keys(window.TERCEROS_DB).length>0){
      var actual=JSON.stringify(window.TERCEROS_DB);
      if(actual!==_SGRT_LAST_SYNC_TEXT){localStorage.setItem('sgrt_terceros_db_shared',actual);_SGRT_LAST_SYNC_TEXT=actual;}
    }
  }catch(e){console.warn('⚠️ Error en sincronización:',e);}
}
// BroadcastChannel/storage de otros módulos propaga cambios locales inmediatamente;
// este intervalo queda solo como respaldo, reduciendo CPU y escrituras de localStorage.
window._INTERVALO_SINCRO=setInterval(sincronizarTercerosGlobal,60000);
function detenerSincronizacion(){window.SINCRONIZACION_ACTIVA=false;if(window._INTERVALO_SINCRO){clearInterval(window._INTERVALO_SINCRO);window._INTERVALO_SINCRO=null;}}
function iniciarSincronizacion(){window.SINCRONIZACION_ACTIVA=true;if(!window._INTERVALO_SINCRO)window._INTERVALO_SINCRO=setInterval(sincronizarTercerosGlobal,60000);}
