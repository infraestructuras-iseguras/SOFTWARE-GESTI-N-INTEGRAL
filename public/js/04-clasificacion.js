...
// Sincronización ligera: los eventos locales son inmediatos; sondeo de respaldo cada 10 s
...
(function syncTercerosFromAPI(){
  ...
  setInterval(syncTercerosFromAPI, 60000);
})();
...
// Antes se serializaba toda la base cada 2 s. Ahora solo si cambió, cada 10 s.
window._AUTOSAVE_INTERVALO=setInterval(function(){autoguardarTodo(false);},10000);
...
window._INTERVALO_SINCRO=setInterval(sincronizarTercerosGlobal,10000);
...
function iniciarSincronizacion(){window.SINCRONIZACION_ACTIVA=true;if(!window._INTERVALO_SINCRO)window._INTERVALO_SINCRO=setInterval(sincronizarTercerosGlobal,10000);}
