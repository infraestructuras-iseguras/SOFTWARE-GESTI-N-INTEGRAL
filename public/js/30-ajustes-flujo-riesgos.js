...
(function(){
  ...
  function answerSummary(r){
    ...
    var vals=[];for(var i=1;i<=6;i++){var v=r['a'+i];if(v)vals.push(i+': '+v);}return vals.length?esc(vals.join(' · ')):'<span style="color:#94a3b8;">Sin respuesta</span>';
  }
  ...
  // Se conserva sincronización remota periódica, pero a 20 s y se pausa en pestañas ocultas; los cambios locales se propagan por BroadcastChannel/storage inmediatamente.
  function tuneAutoRefresh(){
    try{if(window._AUTOREFRESH_INTERVAL)clearInterval(window._AUTOREFRESH_INTERVAL);}catch(e){}
    if(typeof window._sgrtRefreshCurrent==='function')window._AUTOREFRESH_INTERVAL=setInterval(function(){if(document.hidden)return;try{window._sgrtRefreshCurrent();}catch(e){}},20000);
  }
