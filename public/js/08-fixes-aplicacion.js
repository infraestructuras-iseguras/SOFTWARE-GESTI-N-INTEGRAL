...
window.odSave=function(){
  try{var data=JSON.stringify(_odFS);localStorage.setItem(_OD,data);var lbl=document.getElementById('od-uso-lbl');if(lbl)lbl.textContent='Repositorio documental disponible';}catch(e){console.warn('SGRT: guardado documental pendiente de compactación.',e);}
};
...
window.odRender=function(){
  ...
  if(!grid||!bc)return;
  try{var lbl=document.getElementById('od-uso-lbl');if(lbl)lbl.textContent='Repositorio documental disponible';}catch(e){}
...
window.adminOdRender = function(fs){
  ...
  if(lbl) lbl.textContent='Repositorio documental disponible';
  var fsObj=null;
