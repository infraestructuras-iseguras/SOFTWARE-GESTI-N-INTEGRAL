...
var _LS='sgrt_v8';
var _SGRT_LS_SERIAL_CACHE = window._SGRT_LS_SERIAL_CACHE || {};
function _sgrtWriteJSONIfChanged(key,obj){
  var txt=JSON.stringify(obj);
  if(_SGRT_LS_SERIAL_CACHE[key]===txt) return false;
  try{
    if(localStorage.getItem(key)!==txt) localStorage.setItem(key,txt);
    _SGRT_LS_SERIAL_CACHE[key]=txt;
    return true;
  }catch(e){ return false; }
}
function _sgrtActivePageId(){
  try{var p=document.querySelector('.page.active');return p&&p.id||'';}catch(e){return '';}
}
window._lsSave = function(){
  try{
    ...
    // Evita reescribir/serializar hacia el almacenamiento si el contenido no cambió.
    _sgrtWriteJSONIfChanged(_LS,data);
    _sgrtWriteJSONIfChanged('sgrt_cuest_respuestas',window.CUEST_RESPUESTAS||{});
    _sgrtWriteJSONIfChanged('sgrt_terceros_db_shared',window.TERCEROS_DB||{});

    // No reconstruir pantallas ocultas en cada guardado. Solo refrescar la vista activa.
    var pg=_sgrtActivePageId();
    try{ if(pg==='pg-aprobar-op' && typeof renderAprobarOp==='function') renderAprobarOp(); }catch(e){}
    try{ if(pg==='pg-ctrl-op' && typeof acPoblarSelectorTerceroInstruc==='function') acPoblarSelectorTerceroInstruc(); }catch(e){}
    try{ if(pg==='pg-reportes-entidad' && typeof renderReportesAC==='function') renderReportesAC(); }catch(e){}

    // Antes se creaba un setTimeout nuevo por CADA _lsSave y cada uno recorría todos
    // los terceros contra Azure. Ahora queda una sola sincronización en cola y nunca
    // se solapa con otra. En Ambiente de Control el módulo por contrato ya sincroniza
    // únicamente el tercero que se está diligenciando, por lo que no se dispara el
    // barrido global de todos los terceros desde esta función.
    if(window._lsSyncWithAzure && !window._SGRT_SKIP_AUTOSYNC && pg!=='pg-cuestionario'){
      clearTimeout(window._SGRT_AUTOSYNC_TIMER);
      window._SGRT_AUTOSYNC_TIMER=setTimeout(function runAutoSync(){
        if(window._SGRT_SKIP_AUTOSYNC || document.hidden) return;
        if(window._SGRT_AUTOSYNC_RUNNING){
          window._SGRT_AUTOSYNC_TIMER=setTimeout(runAutoSync,5000);
          return;
        }
        window._SGRT_AUTOSYNC_RUNNING=true;
        try{
          Promise.resolve(window._lsSyncWithAzure())
            .catch(function(){})
            .finally(function(){window._SGRT_AUTOSYNC_RUNNING=false;});
        }catch(e){window._SGRT_AUTOSYNC_RUNNING=false;}
      },5000);
    }
  }catch(e){}
};
