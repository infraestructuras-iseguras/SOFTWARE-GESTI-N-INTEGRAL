...
window._lsSave = function(){
  try{
    ...
    if(window._lsSyncWithAzure) {
      setTimeout(function(){ window._lsSyncWithAzure(); }, 300);
    }
  }catch(e){}
};
window._lsLoad = function(){
  try{
    ...
    if(Array.isArray(d.MATRIZ_DB)){ MATRIZ_DB.length=0; d.MATRIZ_DB.forEach(function(r){MATRIZ_DB.push(r);}); window.MATRIZ_DB=MATRIZ_DB; }
    ...
    if(Array.isArray(d.tercerosPendientesCuestionario)){
      if(typeof tercerosPendientesCuestionario!=='undefined'){
        tercerosPendientesCuestionario.length=0;
    ...
    try{
      var _tdRaw=localStorage.getItem('sgrt_terceros_db_shared');
      if(_tdRaw!==null){
        var _td=JSON.parse(_tdRaw)||{};
        if(typeof TERCEROS_DB==='object')Object.keys(TERCEROS_DB).forEach(function(k){delete TERCEROS_DB[k];});
        if(typeof TERCEROS_DB==='object')Object.assign(TERCEROS_DB,_td);
        window.TERCEROS_DB=typeof TERCEROS_DB==='object'?TERCEROS_DB:_td;
      }
    }catch(e2){}
...
window.renderAprobarOp=function(){
  ...
  try{ window._lsLoad && window._lsLoad(); }catch(e){}
  try{
    var _svRaw=localStorage.getItem('sgrt_terceros_db_shared');
    if(_svRaw!==null)window.TERCEROS_DB=JSON.parse(_svRaw)||{};
  }catch(e){}
  try{
    ...
    lista.forEach(function(t,i){
      ...
      if(contratos.length){
        ...
        contratos.forEach(function(c, cidx){
          ...
          var dimsC = ((t.dimsPorContrato||{})[c.num]||[]);
          var promObjC=((t.promPorContrato||{})[c.num]||{});
          var promC=parseFloat(typeof promObjC==='object'?promObjC.prom:promObjC);
          if(isNaN(promC)){var valsC=dimsC.map(function(d){return parseFloat(d.val);}).filter(function(v){return !isNaN(v);});promC=valsC.length?valsC.reduce(function(a,b){return a+b;},0)/valsC.length:0;}
          ...
          h+='<span style="flex:1;font-weight:600;color:#0f172a;">• '+(c.num||'s/n')+' <b style="color:#1e6bb8;margin-left:6px;">Prom. '+promC.toFixed(2)+'</b></span>';
          h+='<button onclick="window._aprToggleContrato(\''+nit+'\',\''+(c.num||'').replace(/\'/g,"\\\\'")+'\')" style="padding:4px 12px;border-radius:4px;font-size:10px;font-weight:800;cursor:pointer;border:1px solid '+(habC?'#86efac':'#fca5a5')+';background:'+(habC?'#f0fdf4':'#fef2f2')+';color:'+(habC?'#166534':'#991b1b')+';font-family:inherit;white-space:nowrap;margin-right:6px;">'+(habC?'✓ Aprobado':'Aprobar')+'</button>';
          ...
          h+='</div>';
          h+='<div style="display:flex;gap:5px;flex-wrap:wrap;padding:2px 0 8px 14px;">'+(dimsC.length?dimsC.map(function(d){return '<span style="padding:3px 7px;border-radius:10px;background:#fff;color:#1e40af;border:1px solid #bfdbfe;font-size:9.5px;"><b>'+(window._nombreTipologia?window._nombreTipologia(d):(d.nombre||d.key))+'</b>: '+(d.val||'Sin calificar')+'</span>';}).join(''):'<span style="font-size:10px;color:#9a3412;">Sin tipologías calificadas en este contrato.</span>')+'</div>';
        });
        h+='</div>';
      }
...
window.guardarClasif = async function(){
  ...
  // Capturar la identidad ANTES de saveClasifForm(). Esa función limpia el
  // formulario al terminar; leer estos campos después dejaba nit0 vacío.
  ...
  if(nit0){
    ...
    TERCEROS_DB[nit0].nit = nit0;
    TERCEROS_DB[nit0].nombre = TERCEROS_DB[nit0].nombre || nom0;
    TERCEROS_DB[nit0].sincronizado = false;
    TERCEROS_DB[nit0]._changed = true;
    TERCEROS_DB[nit0].localOnly = true;
