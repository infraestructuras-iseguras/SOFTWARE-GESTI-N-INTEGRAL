/* SGRT V6 — flujo individual por contrato
...
(function(){
  ...
  function toast(m,t,d){ try{ window.showToast(m,t||'success',d||2600); }catch(e){} }
  function persist(){
  ...
  function catalogoTipologias(){
    try{
      if(typeof TIPOLOGIA_CATALOG!=='undefined' && TIPOLOGIA_CATALOG) return TIPOLOGIA_CATALOG;
    }catch(e){}
    var out={};
    try{
      (window.getDBTips?window.getDBTips(window.getEntidad?window.getEntidad():''):[]).forEach(function(t){
        var k=t.clave||String(t.id_tipologia);out[k]={nombre:t.nombre_tipologia||t.nombre||k,hasNA:!!t.hasNA,hints:t.hints||{},soloImpar:!!t.soloImpar};
      });
    }catch(e){}
    return out;
  }
  ...
  function guardarActual(){
    ...
    var sel=document.getElementById('cls-contrato-actual'),num=(sel&&sel.value)||t.contratoEval||'';
    if(!num)return;
  ...
  window._clasifCambiarContratoActual=function(num){
    ...
    if(anterior&&anterior!==num) guardarActual();
  ...
  window.sgrtAgregarTipologiaContrato=function(){
    ...
    if(!t){toast('Primero selecciona el tercero que vas a clasificar','warning',3500);return;}
    if(!num){toast('Primero selecciona el contrato que vas a calificar','warning',3500);return;}
    if(!key){toast('Selecciona una tipología en el desplegable','warning',3000);return;}
    t.modoEval='contrato';t.contratoEval=num;
    var arr=trabajo(),idx=-1;arr.forEach(function(d,i){if(String(d.key)===String(key))idx=i;});
    if(idx>=0){arr.splice(idx,1);toast('Tipología retirada de este contrato','info',1800);}
    else{
  ...
  ['agregarTipologiaSeleccionada','setDimVal','quitarDim'].forEach(function(n){
  ...
  window.finalizarClasificacionContratos=function(){
    guardarActual();var x=terceroActual(),t=x.t;if(!t){toast('Selecciona un tercero','error');return;}
    var cons=contratos(t),faltan=[];
    if(!cons.length){toast('El tercero debe tener al menos un contrato','warning');return;}
  ...
  function htmlAprobacion(lista){
    var total=0,ok=0;lista.forEach(function(t){total+=contratos(t).length;ok+=aprobados(t).length;});
    ...
    if(!lista.length)return h+'<div class="card" style="padding:35px;text-align:center;color:#64748b">No hay clasificaciones finalizadas.</div>';
    lista.forEach(function(t){
      ...
      contratos(t).forEach(function(c){var n=contratoNum(c),ds=dimsContrato(t,n),p=promContrato(t,n),a=aprobados(t).some(function(x){return contratoNum(x)===n;}),v=window._validarAprobacionSGRT(t,n);
        ...
        h+='<button onclick="window._verTipologiasContrato(\''+jsq(t.nit)+'\',\''+jsq(n)+'\')" class="btn btn-outline" style="font-size:11px">Ver tipologías y puntajes</button>';
        h+='<button onclick="window._aprToggleContrato(\''+jsq(t.nit)+'\',\''+jsq(n)+'\')" class="btn '+(a?'btn-success':'btn-primary')+'" style="font-size:11px;opacity:'+(v.ok||a?'1':'.55')+'">'+(a?'✓ Aprobado':'Aprobar')+'</button></div>';
        h+='<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">'+ds.map(function(d){return '<span style="padding:4px 8px;border-radius:12px;background:#eff6ff;color:#1d4ed8;font-size:10px"><b>'+esc(window._nombreTipologia?window._nombreTipologia(d):(d.nombre||d.key))+'</b>: '+esc(d.val||'—')+'</span>';}).join('')+'</div></div>';
      });h+='</div>';
    });return h;
  }
  window.renderAprobarOp=function(){
    ...
    var lista=Object.keys(db()).map(function(k){return db()[k];}).filter(function(t){return t&&t.clasificacionLista===true&&(!q||String(t.nombre||'').toLowerCase().indexOf(q)>=0||String(t.nit||'').toLowerCase().indexOf(q)>=0);});
