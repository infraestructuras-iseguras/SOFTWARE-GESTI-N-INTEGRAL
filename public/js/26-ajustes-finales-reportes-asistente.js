...
(function(){
  ...
  function supervisorNames(t){
    var names=arr(t&&t.supervisores).map(function(x){return String(x&&x.nombre||'').trim();}).filter(Boolean);
    if(!names.length&&t&&t.supervisor)names=[String(t.supervisor).trim()];
    return names.filter(function(x,i,a){return a.indexOf(x)===i;}).join(', ');
  }
  function registrationRows(s){
    return arr(s.terceros).map(function(t){return{NIT:t.nit||'',Tercero:t.nombre||'',Domicilio:t.domicilio||'',Contratos:arr(t.contratos).length,Supervisores:arr(t.supervisores).length,Nombres_Supervisores:supervisorNames(t)||'—',Estado:t.estado||'Activo'};});
  }
  function classificationRows(s){
    var out=[];arr(s.terceros).forEach(function(t){
      var cs=arr(t.contratos);if(!cs.length&&(t.nocontrato||allDims(t).length))cs=[{num:t.nocontrato||'',estado_aprobacion:''}];
      cs.forEach(function(c){var cn=c.num||c.numero||'',ds=arr(t.dimsPorContrato&&t.dimsPorContrato[cn]);if(!ds.length)ds=arr(t.dims);var pc=t.promPorContrato&&t.promPorContrato[cn]||{};
        if(ds.length)ds.forEach(function(d){out.push({NIT:t.nit||'',Tercero:t.nombre||'',Contrato:cn,Tipologia:d.nombre||d.nombre_tipologia||d.tipologia||d.key||'',Calificacion:d.val!=null?d.val:(d.calificacion!=null?d.calificacion:d.nivel||''),Promedio_Contrato:pc.prom!=null?pc.prom:(t.prom||0),Clasificacion:pc.zona||t.clasificacion||t.zona||'',Estado_Aprobacion:c.estado_aprobacion||((t.aprobadoPorContrato&&t.aprobadoPorContrato[cn])?'APROBADO':'PENDIENTE')});});
      });
    });return out;
  }
  function acDetailedRows(s){
    var out=[];arr(s.terceros).forEach(function(t){var resp=s.respuestas&&s.respuestas[t.nit]||{},cnt=responseCount(resp),cs=approvedContracts(t);
      cs.forEach(function(c){var cn=c.num||c.numero||'',ds=arr(t.dimsPorContrato&&t.dimsPorContrato[cn]);if(!ds.length)ds=allDims(t);if(!ds.length)out.push({NIT:t.nit||'',Tercero:t.nombre||'',Contrato:cn,Tipologia:'—',Respuestas:cnt,Estado:cnt?'En progreso / diligenciado':'Pendiente'});else ds.forEach(function(d){out.push({NIT:t.nit||'',Tercero:t.nombre||'',Contrato:cn,Tipologia:d.nombre||d.nombre_tipologia||d.tipologia||d.key||'',Respuestas:cnt,Estado:cnt?'En progreso / diligenciado':'Pendiente'});});});
    });return out;
  }
  function seguimientoRows(m){return arr(m.risks).map(function(r){return{Ref:r.Ref,NIT:r.NIT,Tercero:r.Tercero,Contrato:r.Contrato,Riesgo:r.Riesgo,Tratamiento:r.Tratamiento,Responsable:r.Responsable,Estado:r.Estado,Fecha_Seguimiento:r.Fecha_Seguimiento};});}
  function powerBIRows(s,m){
    var out=[];registrationRows(s).forEach(function(r){out.push({Fase:'Registro de Terceros',NIT:r.NIT,Tercero:r.Tercero,Contrato:'',Tipologia:'',Indicador:'Supervisores',Valor:r.Supervisores,Estado:r.Estado,Riesgo:'',Inherente:'',Residual:'',Tratamiento:'',Responsable:r.Nombres_Supervisores});});
    classificationRows(s).forEach(function(r){out.push({Fase:'Clasificación de Terceros',NIT:r.NIT,Tercero:r.Tercero,Contrato:r.Contrato,Tipologia:r.Tipologia,Indicador:'Calificación',Valor:r.Calificacion,Estado:r.Estado_Aprobacion,Riesgo:'',Inherente:r.Clasificacion,Residual:'',Tratamiento:'',Responsable:''});});
    acDetailedRows(s).forEach(function(r){out.push({Fase:'Ambiente de Control',NIT:r.NIT,Tercero:r.Tercero,Contrato:r.Contrato,Tipologia:r.Tipologia,Indicador:'Respuestas',Valor:r.Respuestas,Estado:r.Estado,Riesgo:'',Inherente:'',Residual:'',Tratamiento:'',Responsable:''});});
    arr(m.risks).forEach(function(r){out.push({Fase:'Análisis de Riesgos',NIT:r.NIT,Tercero:r.Tercero,Contrato:r.Contrato,Tipologia:r.Tipologia,Indicador:'Riesgo',Valor:r.Ref,Estado:r.Estado,Riesgo:r.Riesgo,Inherente:r.Zona_Inherente,Residual:r.Zona_Residual,Tratamiento:r.Tratamiento,Responsable:r.Responsable});});
    seguimientoRows(m).forEach(function(r){out.push({Fase:'Seguimiento',NIT:r.NIT,Tercero:r.Tercero,Contrato:r.Contrato,Tipologia:'',Indicador:'Plan de acción',Valor:r.Fecha_Seguimiento||'',Estado:r.Estado,Riesgo:r.Riesgo,Inherente:'',Residual:'',Tratamiento:r.Tratamiento,Responsable:r.Responsable});});
    return out;
  }
  function renderSimpleTable(rows){
    if(!rows.length)return '<div style="padding:24px;text-align:center;color:#718096;">No hay información registrada para esta fase.</div>';
    var cols=Object.keys(rows[0]);return '<div style="overflow:auto;max-height:430px;"><table style="width:100%;border-collapse:collapse;font-size:10.8px;"><thead><tr style="background:#173b5f;color:white;position:sticky;top:0;z-index:1;">'+cols.map(function(c){return'<th style="padding:8px;text-align:left;white-space:nowrap;">'+esc(c.replace(/_/g,' '))+'</th>';}).join('')+'</tr></thead><tbody>'+rows.map(function(r,i){return'<tr style="border-bottom:1px solid #e5ebf1;background:'+(i%2?'#f8fafc':'#fff')+'">'+cols.map(function(c){return'<td style="padding:8px;vertical-align:top;">'+esc(r[c]==null?'':r[c])+'</td>';}).join('')+'</tr>';}).join('')+'</tbody></table></div>';
  }
  function phasePanel(s,m,phase){
    var map={registro:['Registro de Terceros',registrationRows(s)],clasificacion:['Clasificación de Terceros',classificationRows(s)],ambiente:['Ambiente de Control',acDetailedRows(s)],analisis:['Análisis de Riesgos',m.risks],seguimiento:['Seguimiento',seguimientoRows(m)]};
    if(phase==='resumen'||!map[phase]){var rows=[{Fase:'Registro de Terceros',Registros:m.total},{Fase:'Clasificación de Terceros',Registros:m.classified},{Fase:'Aprobación',Registros:m.approved},{Fase:'Ambiente de Control',Registros:m.withAC},{Fase:'Análisis de Riesgos',Registros:m.withRisk},{Fase:'Seguimiento',Registros:seguimientoRows(m).length}];return'<div class="card"><div style="padding:12px 14px;border-bottom:1px solid #e2e8f0;"><b style="color:#173b5f;">📌 Resumen por fases</b><div style="font-size:10.5px;color:#718096;margin-top:2px;">Selecciona una fase en el desplegable para consultar su detalle.</div></div>'+renderSimpleTable(rows)+'</div>'; }
    return '<div class="card"><div style="padding:12px 14px;border-bottom:1px solid #e2e8f0;"><b style="color:#173b5f;">📑 '+esc(map[phase][0])+'</b><div style="font-size:10.5px;color:#718096;margin-top:2px;">Información actual de esta fase del flujo.</div></div>'+renderSimpleTable(map[phase][1])+'</div>';
  }
  function summary(s){
  ...
  function printable(s,m){
    var css='body{font-family:Arial,Calibri;margin:28px;color:#263238}h1{color:#173b5f;border-bottom:3px solid #2b6fae;padding-bottom:8px}h2{color:#2b6fae;margin-top:28px}table{width:100%;border-collapse:collapse;font-size:9.5px;margin:12px 0 22px}th{background:#173b5f;color:white;padding:7px;text-align:left}td{border:1px solid #d6dee6;padding:6px}.k{display:inline-block;padding:12px;margin:4px;background:#f3f7fb;border:1px solid #cbd9e6;border-radius:6px;min-width:120px}.n{font-size:22px;font-weight:800;color:#173b5f}@media print{button{display:none}}';
    return '<!doctype html><html><head><meta charset="utf-8"><title>Reporte SGRT</title><style>'+css+'</style></head><body><button onclick="window.print()">Imprimir / Guardar PDF</button><h1>Reporte ejecutivo de Riesgos de Terceros</h1><p>Generado con la información actual del sistema · '+new Date().toLocaleString('es-CO')+'</p><div class="k"><div class="n">'+m.total+'</div>Terceros</div><div class="k"><div class="n">'+m.contracts+'</div>Contratos</div><div class="k"><div class="n">'+m.approvedContracts+'</div>Contratos aprobados</div><div class="k"><div class="n">'+m.risks.length+'</div>Riesgos</div><div class="k"><div class="n">'+m.docs+'</div>Evidencias</div>'+exportHTMLTable('Registro de Terceros',registrationRows(s))+exportHTMLTable('Clasificación de Terceros',classificationRows(s))+exportHTMLTable('Ambiente de Control',acDetailedRows(s))+exportHTMLTable('Análisis de Riesgos',m.risks)+exportHTMLTable('Seguimiento',seguimientoRows(m))+'</body></html>';
  }
  window.sgrtExportarReporte=function(kind){
    ...
    if(k==='powerbi'||k==='csv'||k==='riesgos'||k==='terceros'||k==='ac'){
      download('SGRT_PowerBI_Fases_'+d+'.csv',toCSV(powerBIRows(s,m)),'text/csv;charset=utf-8');
      try{showToast('✅ Datos de todas las fases preparados para Power BI','success',2200);}catch(e1){}
    }else if(k==='pdf'){
      var w=window.open('','_blank');if(w){w.document.open();w.document.write(printable(s,m));w.document.close();setTimeout(function(){try{w.focus();w.print();}catch(e){}},450);}else try{showToast('Habilita ventanas emergentes para imprimir/guardar PDF','warning',3000);}catch(e2){}
    }else{
      try{showToast('En Reportes están disponibles únicamente PDF y Power BI.','info',2300);}catch(e3){}
    }
  };
  // Compatibilidad con botones antiguos: cualquier descarga de datos se unifica para Power BI.
  window.descargarDatosPowerBIAdmin=function(kind){return window.sgrtExportarReporte(String(kind||'').toLowerCase()==='pdf'?'pdf':'powerbi');};


  function normalizarPowerBIUrl(raw){
    var url=String(raw||'').trim();
    if(!url)return '';
    // Si el usuario pega el iframe completo, extraer src automáticamente.
    var m=url.match(/<iframe[^>]+src=["']([^"']+)["']/i);
    if(m&&m[1])url=m[1];
    url=url.replace(/&amp;/g,'&').trim();
    // También aceptar el vínculo normal del reporte y convertirlo al formato reportEmbed.
    var n=url.match(/^https:\/\/app\.powerbi\.com\/groups\/([^\/?#]+)\/reports\/([^\/?#]+)/i);
    if(n)url='https://app.powerbi.com/reportEmbed?reportId='+encodeURIComponent(n[2])+'&groupId='+encodeURIComponent(n[1])+'&autoAuth=true';
    if(url.indexOf('https://app.powerbi.com/reportEmbed?')===0 && !/[?&]autoAuth=/.test(url))url += (url.indexOf('?')>=0?'&':'?')+'autoAuth=true';
    return url;
  }

  function ensurePowerBIStable(){
    var pg=document.getElementById('pg-reportes-entidad');
    var wrap=document.getElementById('rpe-wrap-op');
    if(!pg||!wrap)return;
    var shell=document.getElementById('sgrt-powerbi-stable-shell');
    if(!shell){
      shell=document.createElement('div');
      shell.id='sgrt-powerbi-stable-shell';
      shell.className='card';
      shell.style.cssText='margin:0 0 16px 0;overflow:hidden;border:1px solid #d8e2ec;box-shadow:0 4px 18px rgba(15,45,70,.08);background:#fff;';
      wrap.parentNode.insertBefore(shell,wrap);
    }
    var raw='';try{raw=localStorage.getItem('sgrt_powerbi_embed_url')||'';}catch(e){}
    var url=normalizarPowerBIUrl(raw);
    var current=shell.getAttribute('data-powerbi-url')||'';
    if(current===url && shell.querySelector('#sgrt-powerbi-stable-frame'))return;
    shell.setAttribute('data-powerbi-url',url);
    shell.innerHTML='<div style="padding:12px 14px;background:linear-gradient(90deg,#173b5f,#2b6fae);color:#fff;display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;">'
      +'<div><div style="font-size:14px;font-weight:900;">📊 Power BI Ejecutivo · Vista integrada</div><div style="font-size:10.5px;opacity:.82;margin-top:2px;">El reporte permanece abierto dentro del SGRT y no se reinicia cuando se actualizan los demás reportes.</div></div>'
      +'<div style="display:flex;gap:7px;flex-wrap:wrap;"><button class="btn btn-sm" style="background:#fff;color:#173b5f;font-weight:800;" onclick="window.sgrtConfigurarPowerBI()">'+(url?'Cambiar conexión':'Conectar Power BI')+'</button>'
      +(url?'<button class="btn btn-sm" style="background:#fff;color:#173b5f;font-weight:800;" onclick="window.sgrtRecargarPowerBI()">↻ Recargar Power BI</button>':'')+'</div></div>'
      +(url?'<div id="sgrt-powerbi-frame-wrap" style="height:680px;min-height:520px;position:relative;background:#f7f9fc;">'
        +'<div id="sgrt-powerbi-loader" style="position:absolute;inset:0;z-index:2;display:flex;align-items:center;justify-content:center;background:#f7f9fc;color:#52606d;font-size:12px;"><div style="text-align:center;"><div style="font-size:28px;margin-bottom:8px;">📊</div><b>Conectando con Power BI…</b><div style="font-size:10.5px;margin-top:4px;color:#8291a1;">La primera carga puede tardar unos segundos.</div></div></div>'
        +'<iframe id="sgrt-powerbi-stable-frame" title="Power BI Ejecutivo SGRT" src="'+esc(url)+'" style="width:100%;height:100%;border:0;display:block;background:#fff;" allow="fullscreen; clipboard-write" allowfullscreen="true"></iframe></div>'
        :'<div style="padding:28px;text-align:center;color:#718096;background:#f8fafc;"><div style="font-size:30px;margin-bottom:7px;">📈</div><b>Conecta tu reporte publicado de Power BI</b><div style="font-size:11px;margin-top:5px;">Pega el vínculo que comienza por https://app.powerbi.com/reportEmbed?...</div></div>');
    var fr=shell.querySelector('#sgrt-powerbi-stable-frame');
    if(fr){
      fr.addEventListener('load',function(){var l=document.getElementById('sgrt-powerbi-loader');if(l)l.style.display='none';},{once:true});
      // Respaldo visual: no volver a tocar el src; solo retirar nuestro indicador.
      setTimeout(function(){var l=document.getElementById('sgrt-powerbi-loader');if(l)l.style.display='none';},12000);
    }
  }

  window.sgrtRecargarPowerBI=function(){
    var shell=document.getElementById('sgrt-powerbi-stable-shell');
    if(shell)shell.setAttribute('data-powerbi-url','');
    ensurePowerBIStable();
  };

  window.sgrtConfigurarPowerBI=function(){
    var old='';try{old=localStorage.getItem('sgrt_powerbi_embed_url')||'';}catch(e){}
    var url=prompt('Pega el enlace de inserción de Power BI (el que empieza por https://app.powerbi.com/reportEmbed?...). También puedes pegar el iframe completo:',old);
    if(url===null)return;
    url=normalizarPowerBIUrl(url);
    if(url&&url.indexOf('https://app.powerbi.com/')!==0){try{showToast('Usa el enlace de inserción de Power BI (app.powerbi.com/reportEmbed).','warning',3200);}catch(e2){}return;}
  ...
  function selectedFilters(){
    return{third:(document.getElementById('sgrt-rpt-third')||{}).value||'',contract:(document.getElementById('sgrt-rpt-contract')||{}).value||'',type:(document.getElementById('sgrt-rpt-type')||{}).value||'',state:(document.getElementById('sgrt-rpt-state')||{}).value||'',phase:(document.getElementById('sgrt-rpt-phase')||{}).value||'resumen'};
  }
  ...
  function renderEnhancedReports(){
    ...
    var html=''
      ...
      +'<div style="display:flex;gap:6px;flex-wrap:wrap;"><button class="btn btn-sm" style="background:white;color:#173b5f;font-weight:800;" onclick="window.sgrtExportarReporte(\'powerbi\')">📊 Power BI</button><button class="btn btn-sm" style="background:white;color:#173b5f;font-weight:800;" onclick="window.sgrtExportarReporte(\'pdf\')">🖨 PDF</button></div></div>'
      ...
      +'<div class="card" style="padding:12px 14px;display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;"><div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;"><div><b style="color:#173b5f;">🧭 Ver información por fase</b><div style="font-size:10.5px;color:#718096;">Consulta cada etapa sin salir del tablero.</div></div><select id="sgrt-rpt-phase" onchange="window.sgrtRefrescarReporte()" style="min-width:235px;padding:8px;border:1px solid #cbd5e1;border-radius:6px;background:white;"><option value="resumen" '+(f.phase==='resumen'?'selected':'')+'>— Todas las fases —</option><option value="registro" '+(f.phase==='registro'?'selected':'')+'>Registro de Terceros</option><option value="clasificacion" '+(f.phase==='clasificacion'?'selected':'')+'>Clasificación de Terceros</option><option value="ambiente" '+(f.phase==='ambiente'?'selected':'')+'>Ambiente de Control</option><option value="analisis" '+(f.phase==='analisis'?'selected':'')+'>Análisis de Riesgos</option><option value="seguimiento" '+(f.phase==='seguimiento'?'selected':'')+'>Seguimiento</option></select></div><div style="display:flex;align-items:center;gap:8px;"><span style="font-size:10.5px;color:#15803d;font-weight:700;">● Actualización automática activa</span><span style="font-size:10px;color:#718096;">'+new Date().toLocaleTimeString('es-CO')+'</span><button class="btn btn-outline btn-sm" onclick="window.sgrtRefrescarReporte()">↻ Actualizar</button></div></div>'
      +phasePanel(s,m,f.phase)
  ...
  window.sgrtLimpiarFiltrosReporte=function(){['sgrt-rpt-third','sgrt-rpt-contract','sgrt-rpt-type','sgrt-rpt-state'].forEach(function(id){var e=document.getElementById(id);if(e)e.value='';});var ph=document.getElementById('sgrt-rpt-phase');if(ph)ph.value='resumen';renderEnhancedReports();};
  ...
  function installReports(){
    ...
    var oldOpen=window.abrirReportesPowerBIAdmin;if(typeof oldOpen==='function'&&!oldOpen._sgrt26){
      var n=function(el){var r=oldOpen.apply(this,arguments);setTimeout(function(){ensurePowerBIStable();renderEnhancedReports();},100);return r;};n._sgrt26=true;window.abrirReportesPowerBIAdmin=n;
    }
  }
  ...
  var pageGuides={
    ...
    'pg-reportes-entidad':{name:'Reportes y Power BI',next:'Revisión y cierre',text:'Consulta el tablero consolidado, revisa cada fase desde el desplegable, compara riesgo inherente y residual y genera únicamente PDF o datos para Power BI.'},
  ...
  function assistantAnswer(q){
    ...
    if(n.indexOf('reporte')>=0||n.indexOf('power bi')>=0||n.indexOf('descargar')>=0)return '<b>Reportes:</b> abre “Reportes y Power BI”. Allí puedes filtrar el tablero, consultar cada fase y generar únicamente el reporte PDF o el conjunto de datos preparado para Power BI.';
  ...
  function install(){
    ...
    if(!window._sgrtReportRealtimeTimer){window._sgrtReportRealtimeTimer=setInterval(function(){if(document.hidden)return;try{var pg=document.getElementById('pg-reportes-entidad');if(pg&&pg.offsetParent!==null)renderEnhancedReports();}catch(e){}},20000);}
    if(!window._sgrtReportStorageListener){window._sgrtReportStorageListener=true;window.addEventListener('storage',function(e){if(!e||!e.key||String(e.key).indexOf('sgrt_')===0){try{var pg=document.getElementById('pg-reportes-entidad');if(pg&&pg.offsetParent!==null)renderEnhancedReports();}catch(er){}}});}
    if(typeof window._lsSave==='function'&&!window._lsSave._sgrt26ReportLive){var oldSaveReport=window._lsSave;var liveSave=function(){var r=oldSaveReport.apply(this,arguments);setTimeout(function(){try{var pg=document.getElementById('pg-reportes-entidad');if(pg&&pg.offsetParent!==null)renderEnhancedReports();}catch(e){}},25);return r;};liveSave._sgrt26ReportLive=true;window._lsSave=liveSave;}
