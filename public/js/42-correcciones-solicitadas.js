...
(function(){
  ...
  function videoSlidesFromText(text){
    var raw=norm(text).replace(/\r/g,''),title=assistantAnswerTitle(raw),clean=raw.replace(/^\s*#{1,4}\s+.*$/m,'').trim();
    var blocks=clean.split(/\n\s*\n+/).map(function(x){return x.replace(/^#{1,4}\s+/gm,'').replace(/^[-*•]\s+/gm,'• ').trim();}).filter(Boolean);
    if(blocks.length<2){var sentences=clean.split(/(?<=[.!?])\s+/).filter(Boolean);blocks=[];for(var i=0;i<sentences.length;i+=2)blocks.push(sentences.slice(i,i+2).join(' '));}
    blocks=blocks.slice(0,6);if(!blocks.length)blocks=['Resumen generado por el Asistente SGRT.'];
    return {title:title||'Video SGRT',slides:blocks};
  }
  async function createAssistantVideo(text,titleValue){
    if(!window.MediaRecorder||!HTMLCanvasElement.prototype.captureStream){toast('Este navegador no permite crear el video directamente. Puedes usar el guion generado por el asistente.','warning',4500);return;}
    var parsed=videoSlidesFromText(text),title=norm(titleValue)||parsed.title||'Video SGRT',slides=parsed.slides;
    var canvas=document.createElement('canvas');canvas.width=1280;canvas.height=720;var ctx=canvas.getContext('2d'),stream=canvas.captureStream(30);
    ...
    var rec=new MediaRecorder(stream,{mimeType:mime}),chunks=[];rec.ondataavailable=function(e){if(e.data&&e.data.size)chunks.push(e.data);};
    ...
    var totalScenes=slides.length+1,duration=2600,totalMs=totalScenes*duration;rec.start(250);var started=performance.now();
    await new Promise(function(resolve){function frame(now){var elapsed=now-started,scene=Math.min(totalScenes-1,Math.floor(elapsed/duration)),local=(elapsed%duration)/duration;draw(scene,elapsed/totalMs);if(elapsed<totalMs)requestAnimationFrame(frame);else{draw(totalScenes-1,1);setTimeout(resolve,180);}}requestAnimationFrame(frame);});
    var stopped=new Promise(function(resolve){rec.onstop=resolve;});rec.stop();await stopped;var blob=new Blob(chunks,{type:mime}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=(title||'Video_SGRT').replace(/[^a-z0-9_-]+/gi,'_').slice(0,70)+'_'+new Date().toISOString().slice(0,10)+'.webm';a.click();setTimeout(function(){URL.revokeObjectURL(a.href);},4000);toast('Video visual generado. Incluye el contenido resumido en escenas.','success',4200);
  }
  window.sgrt42CrearVideoIA=createAssistantVideo;
  function enhanceAssistantMessageTools(div,text){
    if(!div||!text)return;var tools=document.createElement('div');tools.style.cssText='display:flex;gap:6px;flex-wrap:wrap;margin-top:9px;padding-top:7px;border-top:1px solid #edf2f7;';
    [['Copiar',function(){navigator.clipboard&&navigator.clipboard.writeText(text).then(function(){toast('Respuesta copiada.','success');}).catch(function(){});}],['Documento',function(){openAssistantDocument(text,assistantAnswerTitle(text));}],['Video resumen',function(){createAssistantVideo(text,assistantAnswerTitle(text));}]].forEach(function(it){var b=document.createElement('button');b.type='button';b.textContent=it[0];b.style.cssText='padding:4px 8px;border:1px solid #c8d7e5;border-radius:6px;background:#f8fbfd;color:#1e4f78;font-size:9.5px;font-weight:700;cursor:pointer;';b.onclick=it[1];tools.appendChild(b);});div.appendChild(tools);
  }
  function enhanceChat(){
    ...
    [['Pregunta libre','Tengo una pregunta: '],['SGRT','Explícame de forma sencilla: '],['Riesgo','Explícame qué significa este riesgo y dame un ejemplo: '],['Informe','Genera un informe ejecutivo con los datos actuales del SGRT y muéstralo como documento editable.'],['Adjuntos','Voy a adjuntar archivos. Analiza únicamente su contenido y dime hallazgos, diferencias y acciones.'],['Video IA','Crea un guion breve para un video explicativo sobre: ']].forEach(function(q){var b=document.createElement('button');b.type='button';b.textContent=q[0];b.style.cssText='padding:5px 9px;border:1px solid #bdd2e5;border-radius:999px;background:white;color:#1e4f78;font-size:10px;font-weight:700;cursor:pointer;';b.onclick=function(){inp.value=q[1];inp.focus();inp.setSelectionRange(inp.value.length,inp.value.length);};quick.appendChild(b);});chat.parentElement.insertBefore(quick,chat);
    ...
    window.enviarMensajeAsistente=async function(){
      ...
      try{
        ...
        var lm=low(msg),action=/(genera|generar|crea|crear|haz|redacta|redactar|prepara|preparar|descarga|descargar|elabora|elaborar|convierte|convertir)/.test(lm),ppt=/(powerpoint|\bpptx?\b|presentaci[oó]n)/.test(lm),word=/(\bword\b|\bdocx\b)/.test(lm),report=/(informe|reporte|resumen ejecutivo)/.test(lm),video=/(\bvideo\b|\bvídeo\b|video resumen|vídeo resumen)/.test(lm),freeDoc=/(documento|acta|carta|minuta|concepto|procedimiento|plan de acci[oó]n|\bplan\b|propuesta|correo|memorando|checklist|lista de chequeo)/.test(lm);
        if(action){
          if(video){addChat('assistant','El contenido está listo. Voy a convertir esta respuesta en un video resumen visual.');setTimeout(function(){createAssistantVideo(ans,assistantAnswerTitle(ans));},120);}
