...
(function(){
  ...
  function localGuideAnswer(prompt,error){
    ...
    if(/^(hol+a+|hello|buenas|buenos d[ií]as|buenas tardes|buenas noches)([,.! ]+(c[oó]mo est[aá]s|como estas|qu[eé] tal|c[oó]mo vas|como vas))?[?.! ]*$/.test(p)){
    ...
    if(/(?:qu[eé] es|que significa|expl[ií]came).*?(?:riesg|riegso|riegos|rieso)/.test(p)||/^(?:riesg|riegso|riegos|rieso)[o\s?¿!¡]*$/.test(p)){
      return '### Qué es un riesgo\nUn **riesgo** es la posibilidad de que un evento o situación incierta afecte los objetivos de una organización. En el SGRT se identifica y analiza para entender **qué podría pasar, qué impacto tendría, qué tan probable es y qué controles existen**.\n\nPor ejemplo: si un proveedor crítico deja de prestar su servicio, puede afectar la continuidad de la empresa. Ese escenario se registra, se valora y se trata dentro de la gestión de riesgos.';
    }
    if(/(?:qu[eé] es|que significa|expl[ií]came).*?dimensi[oó]n|^dimensi[oó]n[\s?¿!¡]*$/.test(p)){
      return '### Qué es una dimensión\nEn este SGRT, una **dimensión** es un área o dominio que agrupa aspectos relacionados que se quieren evaluar. Sirve para ordenar preguntas y controles por temas, por ejemplo **seguridad de la información, continuidad, cumplimiento o riesgo operacional**.\n\nLas dimensiones ayudan a que cada contrato sea evaluado con los temas que realmente le aplican y a medir el avance por cada área.';
    }
  ...
  async function aiSend(prompt,files){
    ...
    var payload={prompt:prompt,context:ctx,history:chatHistory.slice(-24),files:[]},total=0;
    ...
    if(arr.length===1&&/^(audio|video)\//i.test(arr[0].type||'')){
      ...
      var tr=await fetch(apiBase()+'/api/ai/transcribe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fileName:mf.name,mimeType:mf.type,dataUrl:md.dataUrl,prompt:prompt,context:ctx,history:chatHistory.slice(-24)})});
  ...
  async function aiStatus(){var dot=document.getElementById('sgrt42-ai-status');if(!dot)return;try{var r=await fetch(apiBase()+'/api/ai/status',{cache:'no-store'}),j=await r.json();if(r.ok&&j.ok&&j.configured){var provider=j.provider==='gemini'?'Gemini':j.provider==='openai'?'OpenAI':'IA';dot.textContent=provider+' conectado'+(j.model?' · '+j.model:'');dot.title='Proveedor activo: '+provider+(j.model?' / '+j.model:'')+(j.providers&&j.providers.openai&&j.providers.gemini?' · OpenAI disponible como alternativa':'');dot.style.color='#bbf7d0';}else{dot.textContent='Servicio pendiente de configuración';dot.style.color='#fde68a';}}catch(e){dot.textContent='Servicio sin conexión al servidor';dot.style.color='#fecaca';}}
  ...
  function enhanceChat(){
    ...
    window.enviarMensajeAsistente=async function(){
      ...
      var wait=addThinkingIndicator(files.length?'Leyendo '+files.length+' archivo(s)':'Pensando'),sentOk=false;
      ...
      try{
        ...
        if(action){
          var kind=imageIntent?'Imagen':podcast?'Podcast':infografia?'Infografía':video?'Video':ppt?'Presentación':excel?'Excel':word?'Word':report?'Reporte':studioSummary?'Resumen':'Documento';
