...
(function(){
  ...
  function isSGRTDomainPrompt(prompt,hasFiles){
    ...
    var p=low(prompt);
    if(!p)return true;
    if(/^(?:(?:hola|hello|buenas|buenos d[ií]as|buenas tardes|buenas noches)(?:[,.! ]+(?:c[oó]mo est[aá]s|como estas|qu[eé] tal|c[oó]mo vas|como vas))?|c[oó]mo est[aá]s|como estas|qu[eé] tal|c[oó]mo vas|como vas|bien|muy bien|todo bien|y t[uú]|gracias|muchas gracias|listo|ok|vale)[\s,.!?¿¡]*$/.test(p))return true;
    return /sgrt|riesg|tercer|proveedor|contrat|tipolog|clasific|control|ambiente|evidenc|document|cumplim|auditor|continuidad|ciberseg|seguridad de la informaci[oó]n|matriz|probabilidad|impacto|inherente|residual|tratamiento|mitig|seguimiento|hallazgo|incidente|vulnerab|amenaza|iso 27001|iso 31000|power ?bi|reporte|informe|evaluador|administrador de riesgos|usuario|rol|sharepoint|azure|base de datos|pregunta|cuestionario|no aplica|observaci[oó]n|soporte|expediente|hoja de vida|apetito|tolerancia|kri|kpi|d[oó]nde|c[oó]mo se usa|como se usa|no entiendo|ay[uú]dame|gu[ií]a|siguiente paso|qu[eé] hago aqu[ií]|mi rol|m[oó]dulo|pantalla|no entiendo (?:este |el )?sistema|c[oó]mo se usa (?:este |el )?sistema|presentaci[oó]n|powerpoint|podcast|infograf[ií]a|video|resumen|acta|plan de acci[oó]n|flashcards?|tarjetas de estudio|quiz|juego|evaluaci[oó]n|pr[aá]ctica|repaso|aprender|estudiar|capacitar|capacitaci[oó]n|me siento triste|estoy triste|me siento mal|estoy frustrad|me frustra|estoy cansad|me siento cansad|me abruma|estoy estresad|me siento estresad/.test(p);
  }
  ...
  function localGuideAnswer(prompt,error){
    ...
    if(/^(hola|hello|buenas|buenos d[ií]as|buenas tardes|buenas noches)([,.! ]+(c[oó]mo est[aá]s|como estas|qu[eé] tal|c[oó]mo vas|como vas))?[?.! ]*$/.test(p)){
      return 'Hola, muy bien, gracias por preguntar. ¿Cómo estás tú? Si hoy necesitas ayuda con el SGRT, cuéntame qué estás intentando hacer y lo vemos contigo paso a paso.';
    }
  ...
  async function aiStatus(){var dot=document.getElementById('sgrt42-ai-status');if(!dot)return;try{var r=await fetch(apiBase()+'/api/ai/status',{cache:'no-store'}),j=await r.json();if(r.ok&&j.ok&&j.configured){dot.textContent='Servicio conectado · guía especializada SGRT';dot.style.color='#bbf7d0';}else{dot.textContent='Servicio pendiente de configuración';dot.style.color='#fde68a';}}catch(e){dot.textContent='Servicio sin conexión al servidor';dot.style.color='#fecaca';}}
  ...
  function enhanceChat(){
    ...
    if(attach){
      ...
      if(attach.previousElementSibling&&attach.previousElementSibling.tagName==='BUTTON'){attachBtn=attach.previousElementSibling;attachBtn.textContent='Adjuntar';attachBtn.title='Adjuntar documentos o imágenes';}
      var attachButtons=Array.from(panel.querySelectorAll('button')).filter(function(b){return /adjunt/i.test(norm(b.textContent));});
      attachButtons.forEach(function(b){if(!attachBtn)attachBtn=b;else if(b!==attachBtn)b.style.display='none';});
    }
    var sendBtn=inp.parentElement&&Array.from(inp.parentElement.querySelectorAll('button')).find(function(b){return b!==attachBtn&&b.id!=='sgrt42-mic'&&!/adjunt/i.test(norm(b.textContent));});if(sendBtn){sendBtn.id='sgrt42-send';sendBtn.textContent='Enviar';sendBtn.title='Enviar mensaje';sendBtn.style.background='#1e6bb8';sendBtn.style.minWidth='68px';}
    inp.placeholder='Pregunta sobre SGRT, riesgos o adjunta un archivo…';inp.style.fontSize='12px';installVoiceInput(inp);
    if(attach){attach.onchange=function(){var fs=Array.from(attach.files||[]).slice(0,4),lab=document.getElementById('sgrt41-ai-file-label');if(lab){lab.style.display=fs.length?'block':'none';lab.innerHTML=fs.length?('<strong>Listo para enviar:</strong> '+fs.map(function(f){return esc(f.name);}).join(' · ')):'';lab.style.color='#1e5f91';}};}
    window.enviarMensajeAsistente=async function(){
      ...
      if(!isSGRTDomainPrompt(msg,files.length>0)){
        var scoped=outOfScopeLocal();addChat('user',msg);var scopeDiv=addChat('assistant',scoped);enhanceAssistantMessageTools(scopeDiv,scoped);chatHistory.push({role:'user',content:msg},{role:'assistant',content:scoped});saveHistory();inp.value='';return;
      }
      ...
      var wait=addChat('assistant',files.length?'Estoy leyendo los archivos y relacionándolos con el SGRT…':'Estoy revisando tu solicitud…'),sentOk=false;
      ...
      try{
        ...
        var ans=await aiSend(sendPrompt,files);sentOk=true;if(wait)wait.remove();var answerDiv=addChat('assistant',ans);enhanceAssistantMessageTools(answerDiv,ans);chatHistory.push({role:'assistant',content:ans});saveHistory();var statusOk=document.getElementById('sgrt42-ai-status');if(statusOk){statusOk.textContent='Guía SGRT conectada · análisis por rol activo';statusOk.style.color='#bbf7d0';}
        var lm=low(msg),action=/(genera|generar|crea|crear|haz|redacta|redactar|prepara|preparar|descarga|descargar|elabora|elaborar|convierte|convertir)/.test(lm),ppt=/(powerpoint|\bpptx?\b|presentaci[oó]n)/.test(lm),word=/(\bword\b|\bdocx\b)/.test(lm),report=/(informe|reporte|resumen ejecutivo)/.test(lm),excel=/(\bexcel\b|\bxlsx\b|hoja de c[aá]lculo)/.test(lm),video=/(\bvideo\b|\bvídeo\b|video resumen|vídeo resumen)/.test(lm),podcast=/(podcast|audio resumen|resumen de audio)/.test(lm),infografia=/(infograf[ií]a|resumen visual)/.test(lm),studioSummary=/(resumen inteligente|expl[ií]came lo que estoy viendo)/.test(lm),freeDoc=/(documento|acta|carta|minuta|concepto|procedimiento|plan de acci[oó]n|\bplan\b|propuesta|correo|memorando|checklist|lista de chequeo)/.test(lm);
        if(action){
          var kind=podcast?'Podcast':infografia?'Infografía':video?'Video':ppt?'Presentación':excel?'Excel':word?'Word':report?'Reporte':studioSummary?'Resumen':'Documento';
          addChat('assistant','Ya preparé el contenido para '+kind+'. Por seguridad no abriré ni reproduciré nada automáticamente. Usa los botones debajo de mi respuesta para revisar el resultado y decidir cuándo reproducir, descargar o generar el archivo.');
        }
      }catch(e){
        if(wait)wait.remove();
      ...
      }finally{if(sendBtn)sendBtn.disabled=false;if(sentOk&&fi)fi.value='';var lab=document.getElementById('sgrt41-ai-file-label');if(lab){if(sentOk){lab.style.display='none';lab.textContent='';}else if(files.length){lab.style.display='block';lab.innerHTML='<strong>Listo para reintentar:</strong> '+files.map(function(f){return esc(f.name);}).join(' · ');}}}
    };
    aiStatus();
  }
...
(function(){
  ...
  function enhanceCopyBox(){
    ...
    // Siempre reconstruir: el archivo 40 puede volver a dibujar este bloque al cambiar contrato.
    var thirds=sourceThirds(target.nit);if(!thirds.length){old.style.display='none';return;}old.style.display='block';old.dataset.sgrt42extended='1';
    ...
    function fillContracts(){var sn=norm(thirdSel.value),st=db()[sn]||{},cs=allContracts(st);contractSel.innerHTML=cs.length?cs.map(function(c){return '<option value="'+esc(c)+'">Contrato '+esc(c)+'</option>';}).join(''):'<option value="">Sin contratos disponibles</option>';fillTips();}
    ...
    function fillTips(){var list=currentMatches();tipSel.innerHTML=list.length?'<option value="__all__">Todas las tipologías coincidentes ('+list.length+')</option>'+list.map(function(x){return '<option value="'+esc(x.targetKey)+'" data-source-key="'+esc(x.sourceKey)+'">'+esc(x.name)+'</option>';}).join(''):'<option value="">Sin tipologías coincidentes con respuestas</option>';btn.disabled=!list.length;btn.style.opacity=list.length?'1':'.55';help.textContent=list.length?'Hay '+list.length+' tipología(s) compatible(s). Las preguntas que no existan en el destino no se copian.':'No encontré tipologías con respuestas compatibles entre el origen y el destino.';}
    check.onchange=async function(){opts.style.display=check.checked?'block':'none';if(!check.checked)return;check.disabled=true;try{await refreshFromServer();var refreshed=sourceThirds(target.nit);thirdSel.innerHTML=refreshed.map(function(n){var t=db()[n]||{};return '<option value="'+esc(n)+'"'+(n===target.nit?' selected':'')+'>'+esc((t.nombre||n)+' · '+n)+'</option>';}).join('');target=currentTarget();fillTargets();fillContracts();}finally{check.disabled=false;}};
