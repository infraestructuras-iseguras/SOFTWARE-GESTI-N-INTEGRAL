...
(function(){
  ...
  async function aiSend(prompt,files){
    ...
    var payload={prompt:prompt,context:ctx,history:chatHistory.slice(-10),files:[]},total=0;
    ...
    if(arr.length===1&&/^(audio|video)\//i.test(arr[0].type||'')){
      ...
      var tr=await fetch(apiBase()+'/api/ai/transcribe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fileName:mf.name,mimeType:mf.type,dataUrl:md.dataUrl,prompt:prompt,context:ctx,history:chatHistory.slice(-10)})});
  ...
  function enhanceChat(){
    var panel=document.getElementById('chat-panel'),chat=document.getElementById('mensajes-chat'),inp=document.getElementById('input-asistente');if(!panel||!chat||!inp||panel.dataset.sgrt42)return;panel.dataset.sgrt42='1';panel.style.width='440px';panel.style.maxHeight='650px';
    ...
    chat.innerHTML='<div style="background:white;padding:12px;border-radius:8px;border-left:4px solid #1e6bb8;font-size:11.5px;line-height:1.55;"><b>Hola, ¿cómo estás?</b><br>Soy el Asistente SGRT. ¿En qué te puedo colaborar?<br><span style="color:#64748b;">Puedo explicarte conceptos del sistema, revisar dimensiones y riesgos, analizar varios adjuntos, resumir reuniones y ayudarte a crear informes, actas, cartas, procedimientos, matrices, planes y otros documentos relacionados con la gestión de terceros y sistemas.</span></div>';
    var oldQuick=document.getElementById('sgrt42-quick');if(oldQuick)oldQuick.remove();var quick=document.createElement('div');quick.id='sgrt42-quick';quick.style.cssText='display:flex;gap:5px;flex-wrap:wrap;padding:8px 10px;background:#f5f9fd;border-bottom:1px solid #e2e8f0;';
    [['Informe','Genera un informe ejecutivo práctico con los datos actuales del SGRT y muéstralo como documento dentro del sistema.'],['Documento','Ayúdame a crear un documento profesional con lo que te indique y muéstralo editable dentro del SGRT.'],['Adjuntos','Voy a adjuntar archivos. Analízalos juntos, relaciónalos con el SGRT y dime hallazgos y acciones.'],['Riesgos','Analiza los riesgos actuales y prioriza acciones concretas.'],['Acta','Redacta un acta profesional. Si adjunto una grabación, usa únicamente su contenido.'],['Explicar','Explícame de forma sencilla qué es una dimensión, una tipología y cómo se relacionan con el Ambiente de Control.']].forEach(function(q){var b=document.createElement('button');b.type='button';b.textContent=q[0];b.style.cssText='padding:5px 8px;border:1px solid #bdd2e5;border-radius:999px;background:white;color:#1e4f78;font-size:10px;font-weight:700;cursor:pointer;';b.onclick=function(){inp.value=q[1];inp.focus();};quick.appendChild(b);});chat.parentElement.insertBefore(quick,chat);
    ...
    inp.placeholder='Ej.: ¿qué es una dimensión?, revisa estos archivos o créame un informe…';
    ...
    window.enviarMensajeAsistente=async function(){
      ...
      try{
        var ans=await aiSend(msg||'Analiza los archivos adjuntos y genera una salida práctica para SGRT.',files);if(wait)wait.remove();addChat('assistant',ans);chatHistory.push({role:'assistant',content:ans});saveHistory();
        var lm=low(msg),action=/(genera|generar|crea|crear|haz|redacta|redactar|prepara|preparar|descarga|descargar|elabora|elaborar)/.test(lm),ppt=/(powerpoint|\bpptx?\b|presentaci[oó]n)/.test(lm),word=/(\bword\b|\bdocx\b)/.test(lm),report=/(informe|reporte|resumen ejecutivo)/.test(lm),freeDoc=/(documento|acta|carta|minuta|concepto|procedimiento|plan de acci[oó]n|plan|propuesta|correo|memorando|checklist|lista de chequeo)/.test(lm);
        if(action){
          if(ppt&&!files.length){addChat('assistant','Prepararé la presentación con la plantilla corporativa y los datos actuales del SGRT.');setTimeout(function(){window.sgrtGenerarPowerPointFinal&&window.sgrtGenerarPowerPointFinal(msg);},80);}
