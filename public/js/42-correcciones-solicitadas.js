...
(function(){
  ...
  function saveExact(nit,contract,doRemote){
    ...
    if(vals.length){
      ...
      // Una tipología habilitada sin calificación NO equivale a cero ni a riesgo BAJO.
      // Se conserva expresamente como "sin puntaje" hasta que exista una valoración real.
      t.promPorContrato[contract]={prom:null,zona:'',sinPuntaje:true};
    }
...
(function(){
  ...
  async function aiSend(prompt,file){
    ...
    if(file){var dataUrl=await fileToDataUrl(file);if(/^(audio|video)\//i.test(file.type||'')){if(file.size>18*1024*1024)throw new Error('El audio/video debe pesar máximo 18 MB para análisis directo.');var tr=await fetch(apiBase()+'/api/ai/transcribe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fileName:file.name,mimeType:file.type,dataUrl:dataUrl,prompt:prompt,context:ctx,history:chatHistory.slice(-10)})}),tj=await tr.json().catch(function(){return{};});if(!tr.ok||!tj.ok)throw new Error(tj.error||('HTTP '+tr.status));saveMeeting(file,dataUrl,tj.summary||'',tj.transcript||'');return tj.summary||tj.transcript||'Transcripción completada.';}if(file.size>18*1024*1024)throw new Error('El archivo debe pesar máximo 18 MB para analizarlo directamente desde el asistente.');payload.file={name:file.name,type:file.type||'application/octet-stream',dataUrl:dataUrl};}
  ...
  function enhanceChat(){
    ...
    var floatBtn=document.getElementById('btn-asistente');if(floatBtn){floatBtn.textContent='SGRT';floatBtn.style.fontSize='11px';floatBtn.style.fontWeight='800';floatBtn.style.letterSpacing='.4px';floatBtn.style.animation='none';}
    var header=panel.firstElementChild;if(header){header.style.background='linear-gradient(135deg,#0d2740,#1e6bb8)';var title=header.firstElementChild;if(title)title.innerHTML='<span>Asistente SGRT<br><small id="sgrt42-ai-status" style="font-size:9px;font-weight:600;color:#fde68a;">Verificando servicio…</small></span>';}
    chat.innerHTML='<div style="background:white;padding:10px;border-radius:8px;border-left:4px solid #1e6bb8;font-size:11.5px;line-height:1.5;"><b>Asistente SGRT</b><br>Escribe tu solicitud con tus palabras. Puedes pedir redacción de objetos, revisión de riesgos, análisis de archivos, resumen de reuniones o informes. Los informes pueden abrirse como documento dentro del SGRT y, si lo necesitas, descargarse en Word o PowerPoint.</div>';
    var oldQuick=document.getElementById('sgrt42-quick');if(oldQuick)oldQuick.remove();var quick=document.createElement('div');quick.id='sgrt42-quick';quick.style.cssText='display:flex;gap:5px;flex-wrap:wrap;padding:8px 10px;background:#f5f9fd;border-bottom:1px solid #e2e8f0;';[['Informe','Genera un informe ejecutivo práctico con los datos actuales del SGRT y muéstralo como documento dentro del sistema.'],['Word','Genera y descarga un informe Word con los datos actuales del SGRT.'],['PowerPoint','Genera y descarga una presentación PowerPoint ejecutiva con los datos actuales del SGRT.'],['Riesgos','Analiza los riesgos actuales y prioriza acciones concretas.'],['Acta','Redacta un acta profesional. Si adjunto una grabación, usa únicamente su contenido.'],['Objeto','Ayúdame a redactar el objeto del informe o contrato con lenguaje técnico y claro.']].forEach(function(q){var b=document.createElement('button');b.type='button';b.textContent=q[0];b.style.cssText='padding:5px 8px;border:1px solid #bdd2e5;border-radius:999px;background:white;color:#1e4f78;font-size:10px;font-weight:700;cursor:pointer;';b.onclick=function(){inp.value=q[1];inp.focus();};quick.appendChild(b);});chat.parentElement.insertBefore(quick,chat);
    var attach=document.getElementById('sgrt41-ai-file');if(attach&&attach.previousElementSibling&&attach.previousElementSibling.tagName==='BUTTON')attach.previousElementSibling.textContent='Adjuntar';
    var sendBtn=inp.parentElement&&Array.from(inp.parentElement.querySelectorAll('button')).find(function(b){return b!==attach&&b.textContent!=='Adjuntar';});if(sendBtn)sendBtn.textContent='Enviar';
    inp.placeholder='Ej.: revisa este contrato, redacta el informe o resume esta reunión…';
    window.enviarMensajeAsistente=async function(){
      var msg=norm(inp.value),fi=document.getElementById('sgrt41-ai-file'),file=fi&&fi.files&&fi.files[0];if(!msg&&!file)return;
      var shown=msg||(file?'Analiza el archivo adjunto: '+file.name:'');addChat('user',shown);chatHistory.push({role:'user',content:shown});saveHistory();inp.value='';
      var wait=addChat('assistant','Procesando…');
      try{
        var ans=await aiSend(msg||'Analiza el archivo adjunto y genera una salida práctica para SGRT.',file);if(wait)wait.remove();addChat('assistant',ans);chatHistory.push({role:'assistant',content:ans});saveHistory();
        var lm=low(msg),action=/(genera|generar|crea|crear|haz|redacta|redactar|prepara|preparar|descarga|descargar)/.test(lm),ppt=/(powerpoint|\bpptx?\b|presentaci[oó]n)/.test(lm),word=/(\bword\b|\bdocx\b)/.test(lm),doc=/(informe|reporte|documento|acta|resumen ejecutivo)/.test(lm);
        if(!file&&action){
          if(ppt){addChat('assistant','La presentación se generará con la plantilla corporativa y los datos actuales.');setTimeout(function(){window.sgrtGenerarPowerPointFinal&&window.sgrtGenerarPowerPointFinal(msg);},80);}
          else if(word){addChat('assistant','El documento Word se generará con la plantilla corporativa y los datos actuales.');setTimeout(function(){window.sgrtGenerarWordFinal&&window.sgrtGenerarWordFinal(msg);},80);}
          else if(doc){addChat('assistant','El contenido también quedó disponible como documento dentro del SGRT. Puedes editarlo, imprimirlo o guardarlo en PDF.');setTimeout(function(){var title=/(acta)/.test(lm)?'ACTA / DOCUMENTO DE SEGUIMIENTO':'INFORME EJECUTIVO SGRT';window.sgrt42AbrirDocumento&&window.sgrt42AbrirDocumento(ans,title);},100);}
        }
      }catch(e){
...
(function(){
  ...
  function allContracts(t){
    ...
    [t&&t.dimsPorContrato,t&&t.tipologiasPorContrato,t&&t.respuestasACPorContrato,t&&t.respuestasPorContrato,t&&t._respuestasPorContrato,t&&t.promPorContrato,t&&t.acPorContrato,t&&t.borradoresACPorContrato].forEach(function(m){Object.keys(m||{}).forEach(add);});
  ...
  function tipMatchList(srcT,srcC,targetT,targetC){
    ...
    tarD.forEach(function(td){var tc=dimCanon(td),sd=srcD.find(function(x){return dimCanon(x)===tc;});if(!sd)return;var sk=dimKey(sd),tk=dimKey(td),bucket=srcR[sk]||srcR[dimName(sd)]||{};if(bucket&&Object.keys(bucket).length)out.push({targetKey:tk,sourceKey:sk,name:dimName(td)});});return out;
  }
  ...
  function enhanceCopyBox(){
    ...
    check.onchange=function(){opts.style.display=check.checked?'flex':'none';if(check.checked)fillContracts();};thirdSel.onchange=fillContracts;contractSel.onchange=fillTips;
  ...
  var observer=new MutationObserver(function(){setTimeout(enhanceCopyBox,25);});observer.observe(document.documentElement,{childList:true,subtree:true});
