...
(function(){
  ...
  window.sgrtGenerarWordFinal=async function(customInstruction){
    ...
    try{
      ...
      var blob=await d.Packer.toBlob(doc),a=document.createElement('a');var name=(data.third?((db()[data.third]||{}).nombre||data.third):ent).replace(/[^a-z0-9_-]+/gi,'_'),fileName='SGRT_Informe_'+name+'_'+new Date().toISOString().slice(0,10)+'.docx';try{if(window.sgrt45RegistrarDocumentoGenerado)await window.sgrt45RegistrarDocumentoGenerado(blob,fileName,{nit:data.third||'',contract:data.contract||''});}catch(_reg){}a.href=URL.createObjectURL(blob);a.download=fileName;a.click();setTimeout(function(){URL.revokeObjectURL(a.href);},2000);toast('Informe Word generado con la plantilla corporativa y guardado en Documentación.','success');
