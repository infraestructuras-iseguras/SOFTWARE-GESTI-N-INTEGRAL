...
(function(){
  ...
  function localGuideAnswer(prompt,error){
    ...
    if(/^(hola|hello|buenas|buenos d[ií]as|buenas tardes|buenas noches)([,.! ]+(c[oó]mo est[aá]s|como estas|qu[eé] tal))?[?.! ]*$/.test(p)){
      return '### Hola\nEstoy bien, gracias. **Estoy aquí para acompañarte dentro del SGRT.**\n\nAhora estás en **'+mi.actual.nombre+'** como **'+roleName+'**. ¿Cómo estás tú? Puedes preguntarme **qué hacer aquí**, **dónde encontrar algo** o **cuál es el siguiente paso**.';
    }
  ...
  function learningCards(source,focus){
    var all=uniqueLearningCards([].concat(learningModuleCards(),learningPhaseCards(),learningConceptCards(),sourceLearningCards(source)));
    var f=low(focus||'');
  ...
  function enhanceChat(){
    ...
    window.enviarMensajeAsistente=async function(){
      ...
      var learnIntent=low(msg),learnMode='';if(!files.length){if(/flashcards?|tarjetas de estudio|fichas de estudio/.test(learnIntent))learnMode='flashcards';else if(/juego|reto|niveles|gamific/.test(learnIntent))learnMode='game';else if(/evaluaci[oó]n|examen|prueba de conocimiento/.test(learnIntent))learnMode='exam';else if(/quiz|preguntas para practicar|pr[aá]ctica de preguntas/.test(learnIntent))learnMode='quiz';}
      ...
      try{
        ...
        if(action){
          var kind=podcast?'Podcast':infografia?'Infografía':video?'Video':ppt?'Presentación':word?'Word':report?'Reporte':studioSummary?'Resumen':'Documento';
