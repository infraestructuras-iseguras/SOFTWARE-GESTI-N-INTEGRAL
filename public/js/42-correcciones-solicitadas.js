...
(function(){
  ...
  async function aiStatus(){var dot=document.getElementById('sgrt42-ai-status');if(!dot)return;try{var r=await fetch(apiBase()+'/api/ai/status',{cache:'no-store'}),j=await r.json();if(r.ok&&j.ok&&j.configured){var both=j.providers&&j.providers.openai&&j.providers.gemini,auto=(j.providerMode||'auto')==='auto';if(both&&auto){dot.textContent='OpenAI + Gemini conectados · modo automático'+(j.webGrounding?' · búsqueda SGRT activa':'');dot.title='Conversación: OpenAI/Gemini con fallback. Búsqueda web solo para conceptos de gestión de riesgos; datos internos salen únicamente del SGRT.';}else{var provider=j.provider==='gemini'?'Gemini':j.provider==='openai'?'OpenAI':'IA';dot.textContent=provider+' conectado'+(j.model?' · '+j.model:'')+(j.webGrounding?' · búsqueda SGRT activa':'');dot.title='Proveedor activo: '+provider+(j.model?' / '+j.model:'');}dot.style.color='#bbf7d0';}else{dot.textContent='Servicio pendiente de configuración';dot.style.color='#fde68a';}}catch(e){dot.textContent='Servicio sin conexión al servidor';dot.style.color='#fecaca';}}
