...
(function(){
  ...
  var lastPull=0,pullRunning=false;
  ...
  function safePull(force){
    if(!window.sgrtCargarDesdeServidor||pullRunning)return;
    var dirty=Object.values(db()).some(function(t){return t&&t._changed;});
    if(dirty&&!force)return;
    var now=Date.now();if(!force&&now-lastPull<15000)return;lastPull=now;
    pullRunning=true;
    try{
      Promise.resolve(window.sgrtCargarDesdeServidor({selectNit:getCtx().nit||''}))
        .then(function(){restoreRemoteConfig();})
        .catch(function(){})
        .finally(function(){pullRunning=false;});
    }catch(e){pullRunning=false;}
  }
  ...
  setInterval(function(){if(!document.hidden)safePull(false);},120000);
